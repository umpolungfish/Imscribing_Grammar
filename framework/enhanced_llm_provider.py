...
from .ig_devices import cpu_forced as _cpu_forced, devices as ig_devices, device_plan as ig_device_plan, attn_implementation as _attn_impl, warmup_devices as _warmup_devices  # noqa: F401
    ...
    def _ensure_loaded(self) -> None:
        ...
        with LocalProvider._load_lock:
            ...
            load_kwargs: dict = {
                ...
                "attn_implementation": _attn_impl(),  # flash_attention_2 > sdpa > eager; auto-detected from GPU
                "dtype": torch.float16,
            ...
            # ── Toroidal attention integration ──────────────────────────
            # IG_TOROIDAL_ATTENTION=1 replaces flash_attn/sdpa with
            # FFT-based circulant convolution on Z_N or Z_m×Z_n.
            # Config: IG_TOROIDAL_SIGMA=0.15 (kernel width), 
            #         IG_TOROIDAL_MODE=1d|2d (geometry)
            if os.getenv("IG_TOROIDAL_ATTENTION", "").strip() not in ("", "0"):
                _sigma = float(os.getenv("IG_TOROIDAL_SIGMA", "0.15"))
                _mode = os.getenv("IG_TOROIDAL_MODE", "1d").strip()
                _layers_s = os.getenv("IG_TOROIDAL_LAYERS", "").strip()
                _layers = None
                if _layers_s:
                    _layers = [int(x.strip()) for x in _layers_s.split(",") if x.strip().isdigit()]
                try:
                    from framework.ig_toroidal_attention import patch_with_toroidal
                    patch_with_toroidal(mdl, sigma=_sigma, mode=_mode, 
                                       layers_to_patch=_layers)
                    logger.info(
                        f"Toroidal attention active (σ={_sigma}, {_mode}"
                        + (f", layers={_layers}" if _layers else ", all layers")
                        + ")"
                    )
                except Exception as _tor_err:
                    logger.warning(
                        f"Toroidal attention patch failed ({_tor_err}); "
                        f"using default attention ({_attn_impl()})."
                    )
            # ─────────────────────────────────────────────────────────
