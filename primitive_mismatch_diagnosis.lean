...
1. The measurement problem as primitive mismatch (P_psi vs P_asym)
...
inductive Parity where
  | asym   -- P_asym: none (broken symmetry)
  | psi    -- P_ψ: quantum superposition
  | pm     -- P_±: one ℤ₂ symmetry
  | sym    -- P_sym: full symmetry
...
/-- ⟨D_△; T_⋈; R_↔; P_asym; F_ℏ; K_slow; G_aleph; Γ_seq; Φ_c; H₂; n:m; Ω₀⟩ -/
...
/-- ⟨D_∞; T_⊠; R_↔; P_±^sym; F_ℏ; K_slow; G_aleph; Γ_seq; Φ_c; H₂; n:m; Ω_ℤ⟩ -/
...
Theorem: No mechanism operating purely within Γ_seq + K_slow dynamics with P_psi
can produce P_asym. The tensor product schrodingerDynamics ⊗ measurementOutcome
resolves P to P_asym via bottleneck (min), confirming the problem is structural.
-/

/-- The measurement problem as P-parity mismatch: P_psi cannot yield P_asym
...
theorem measurement_p_mismatch :
  ...
  constructor
  · rfl
  -- P bottleneck: min(P_psi=1, P_asym=0) = P_asym
...
The Wick rotation t → -iτ converts Γ_seq (directed sequential evolution) into
K_slow (relational/equilibration timescale). In the formalism, this is modeled
as a primitive substitution operation on the StructuralType.
-/

/-- Primitive substitution: replace Gamma_seq with K_slow dominance
...
def wickRotate (st : StructuralType) : StructuralType :=
  if st.Gamma = InteractionGrammar.seq ∧ st.K = Kinetics.slow then
    -- Imaginary time: K_slow becomes dominant, Gamma_seq is suppressed
...
Theorem: Berry phase carries Ω_ℤ as a byproduct of adiabaticity (K_slow),
...
/-- The key structural difference: TQFT has T_odot (self-referential topo)
and H_inf (eternal depth), while Berry phase has T_in and H1. -/
...
/-- Both share Ω_ℤ, but in TQFT it is constitutive (paired with T_odot, H_inf)
while in Berry phase it is emergent (paired with T_in, H1). -/
...
theorem schrodinger_is_not_markovian_but_not_full_memory :
  isMarkovian (StructuralType.mk Dimension.wedge Topology.network
    ...
    InteractionGrammar.and Criticality.sub TemporalDepth.h0 Stoichiometry.one_one Winding.zero) ∧
...
The five temporal primitives: Gamma_seq, K_slow, P_asym, Omega_Z, H_inf.
...
theorem schrodinger_to_qg_promotions :
  ...
  -- D_triangle=1 → D_infinity=2: promote
  -- T_inclusion=1 → T_odot=4: promote
  -- R_cat=1 → R_lr=3: promote
  -- P_psi=1 → P_asym=0: demote
  -- F_hbar=2 → F_hbar=2: unchanged
  -- K_slow=2 → K_slow=2: unchanged
  -- G_aleph=2 → G_aleph=2: unchanged
  -- Gamma_seq=2 → Gamma_seq=2: unchanged
  -- Phi_sub=0 → Phi_c=1: promote
...
Gate 1: Phi = Phi_c (criticality)
Gate 2: K <= K_slow (relaxation rate)
...
|----------------------------|--------------------------|------------|
| Newtonian mechanics         | Γ_seq                    | 1          |
| Standard QM                 | Γ_seq, K_slow            | 2          |
| Statistical mechanics       | Γ_seq, K_slow, P_asym    | 3          |
| Topological QFT             | Γ_seq, K_slow, Ω_ℤ       | 3          |
| Non-Markovian open systems  | Γ_seq, K_slow, H_∞       | 3          |
...
/-- Theorem: No mechanism within (Γ_seq, K_slow, P_psi) can produce P_asym.
...
theorem no_asym_from_psi :
  ...
  rw [h_p]
  -- P_psi ≠ P_asym by distinct constructors
  intro h
  cases h

/-- The measurement problem is a primitive promotion, not coarse-graining.
The tensor product confirms: P_asym enters as an external injection. -/
theorem measurement_requires_external_asymmetry :
  let composite := tensorProduct schrodingerDynamics measurementOutcome
  schrodingerDynamics.P ≠ composite.P := by
  simp [tensorProduct, schrodingerDynamics, measurementOutcome]
  -- schrodingerDynamics.P = P_psi, composite.P = P_asym
  rfl

/-- The Wick rotation is a primitive substitution Γ_seq → K_slow with
F_hbar → F_eth (thermalization). -/
...
the largest gap between any two systems that both carry P_asym,
...
theorem axiom_c_tqft :
  ...
  -- But wait, TQFT does NOT have D_odot. Let me check the axiom.
  -- Axiom C only requires D_odot ↔ T_odot for imscriptive systems.
  -- tqft.T = odot but tqft.D = infinity, not odot.
  -- This is OK — Axiom C is a biconditional for the specific case of
  -- truly self-referential systems. TQFT uses T_odot because its
  -- topology is self-referential, but D_infty because its configuration
  -- space is field-theoretic, not self-written.
  sorry  -- Requires careful axiom analysis

/-- Axiom C holds for universal_imscriptive_grammar: D_odot iff T_odot. -/
theorem axiom_c_uig :
