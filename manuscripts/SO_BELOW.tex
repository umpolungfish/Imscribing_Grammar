...
%  Status: DRAFT v0.2.1 · 2026-06-18
% ============================================================
...
% Page layout
\usepackage[top=1in, bottom=1in, left=1in, right=1in, headheight=14.5pt, includeheadfoot]{geometry}
...
grammar assigning any system a coordinate in a discrete space of
17,280,000 types --- the \emph{Crystal of Types}. Induced from the
...
this paper is the $\mu$ half. A central theorem, mechanised and universally
quantified, establishes \emph{Frobenius non-synthesizability}: a \emph{single}
sub-Frobenius component suffices to deny $\Ppms$ to a composition of any size,
so no accumulation strategy reaches it. The grammar encodes itself at address
6,734,591 in the $\Oinf$ tier of the Crystal it derives.

Validated across over 5,000 imscribed systems: the inflationary epoch and high-dose
...
necessary. The grammar identified the SIC-POVM obstruction as a $\text{{\igprimfont <}}$-gap
that no accumulation of pairwise checks can close, and named descent from the
arithmetic object as the only route past it; that prediction has since been
executed, and existence at $d = 12$ is a machine-checked theorem obtained by
exactly that route. The full catalog is released as open-source software.
\end{abstract}\begin{center}
\rule{0.5\textwidth}{0.4pt}
\end{center}

\noindent\textbf{Keywords:} Universal Imscriptive Grammar; types;
...
entirely by four gate primitives: $\text{{\igprimfont ⊣}}$, $\text{{\igprimfont <}}$, $\text{{\igprimfont ⊙}}$, $\text{{\igprimfont ⊤}}$) that
...
The trapdoor was not a chemistry term. It was $\text{{\igprimfont ∋}}$ (Composition). Composition is dangerous
because it immediately raises its dual: decomposition. Once you ask how a thing is composed,
you must ask what a thing is, where its boundary lies, and how that boundary persists under change.
A synthon is already a partially detached object --- neither fully independent nor fully embedded,
defined through its capacity for future coupling and past disconnection. The moment composition
is made scale-independent, the question is no longer chemical. It is ontological. Topology
(how things connect), chirality (how histories matter), winding (how structures recur),
criticality (whether the loop closes on itself) --- these are not additional features bolted onto
composition. They are what composition requires once scale is removed as a constraint. The remaining
eleven primitives precipitate out of $\text{{\igprimfont ∋}}$ by logical necessity. The grammar was not
invented from that prompt. It was drawn out.

This inversion of direction is characteristic of the grammar's derivation generally. Standard
development is synthetic and bottom-up: declare primitives, define operations, build upward.
Characterizing a Totality proceeds in the reverse direction. The Totality already contains
everything by definition; it cannot be built, only uncovered. The grammar was not
reached by adding structure where there was none. It was reached by removing the noise
--- formal independence tests, diagonalization, delamination --- until the underlying geometry
became visible. The process could have stopped at any number of primitives. It stopped at twelve
because the tests ran out of things to separate.
...
operations: logical (yielding $\text{{\igprimfont ⊙}}$, $\text{{\igprimfont ⊣}}$, $\text{{\igprimfont ⊡}}$, $\text{{\igprimfont >}}$, \text{{\igfont 𐑹}},
$L_6$), inductive (yielding $\text{{\igprimfont ⊥}}$, $\text{{\igprimfont ⊡}}$, $\text{{\igprimfont ⊢}}$, Yoneda/Lawvere), and
algebraic (yielding $\text{{\igprimfont ⊞}}$, $\text{{\igprimfont <}}$, $\text{{\igprimfont ⊤}}$, $\text{{\igprimfont ⋈}}$, $\text{{\igprimfont ∋}}$). The same twelve
...
primitive $\text{{\igprimfont ⋈}}$. The expectation was that a displacement ordering would
...
the $\text{{\igprimfont ∈}}$-scope gap between renormalizable local continuum (\text{{\igfont 𐑔}}) and
...
\begin{tikzpicture}[
  ...
  \node[phase, fill=green!12]  (p3) at (4.5,0) {
    ...
    \footnotesize over 5,000 systems imscribed
  };
  ...
  \node[font=\footnotesize, text=gray, align=left, anchor=north, text width=7.5cm] at (0,-1.4) {
    $\text{{\igprimfont ⊣}}$: 12$+$ labels $\to$ 5 structural universals \\[1pt]
    $\text{{\igprimfont ⊥}}$: delaminated from $\text{{\igprimfont ⊤}}$ \\[1pt]
    $\text{{\igprimfont ⊡}}$: absent $\to$ added \\[1pt]
    $\text{{\igprimfont ⊙}}$: 3 values $\to$ 5 values \\[1pt]
    $\text{{\igprimfont >}}$: physical bond $\to$ relational mode
  };
\end{tikzpicture}
...
that the grammar's type occupies $\Oinf$, though it does.
...
protocol, on any system, carries the Frobenius condition as a fact. This is a stronger form of imscription than the
...
point encodes the circle. The symbol was chosen for reasons,
not alchemical ones; the alchemists chose it for reasons too.
...
catalog. \Cref{sec:applications} traverses six domains, over 5,000
...
\begin{tikzpicture}
  ...
  \node[htlayer] (obs)  {$\boldsymbol{\text{{\igprimfont ⊡}}}$ \\ $\langle S \rangle$};
...
Four-layer imscription pipeline. The observer $\boldsymbol{\text{{\igprimfont ⊡}}}$ carries
...
A \emph{type} is an equivalence class of systems sharing
the same ordered 12-tuple

\begin{equation}
  \xvec \;=\; \langle \text{{\igprimfont ⊢}} \text{{\igprimfont ⊣}} \text{{\igprimfont >}} \text{{\igprimfont <}} \text{{\igprimfont ⋈}} \text{{\igprimfont ⊤}} \text{{\igprimfont ∈}} \text{{\igprimfont ∋}} \text{{\igprimfont ⊙}} \text{{\igprimfont ⊥}} \text{{\igprimfont ⊞}} \text{{\igprimfont ⊡}} \rangle
  \label{eq:tuple}
\end{equation}
...
\begin{table}[htbp]
\centering
\caption{The twelve primitives of the grammar.
...
Dimensionality $\text{{\igprimfont ⊢}}$ measures the extent and character of the state
...
monad symbol $\odot$ for point-in-circle; see terminological note below). Topology $\text{{\igprimfont ⊣}}$ encodes
...
topologically permitted. Winding $\text{{\igprimfont ⊡}}$ encodes the topological
...
\text{{\igfont 𐑟}} (not applicable, e.g.\ ecosystems without a
...
\subsection{The $\text{{\igprimfont <}}$-primitive and dialetheia}
\label{sec:p_dialetheia}

The parity primitive $\text{{\igprimfont <}}$ has five ordered values. The jump from
...
The five $\text{{\igprimfont <}}$-values in ascending order:
\begin{enumerate}[leftmargin=*, label=\arabic*.]
  \item $\text{{\igfont 𐑗}}$ --- broken symmetry; no structural constraints.
  \item \text{{\igfont 𐑿}} --- complex phase symmetry (exact coherence); \text{{\igfont 𐑐}} required.
...
\noindent
This is a \TOPO\ claim: it follows from the $\min$ rule on $\text{{\igprimfont <}}$ under
...
(circles) are types --- coordinates in the 17,280,000-element
...
The total number of types is the product of value-set sizes
...
\begin{align}
  \mathcal{F}_3 &= \{\text{{\igprimfont ⋈}},\, \text{{\igprimfont ∈}},\, \text{{\igprimfont ⊞}}\}
    &\text{3 values each,} \quad 3^3 &= 27 \label{eq:f3} \\
  \mathcal{F}_4 &= \{\text{{\igprimfont ⊢}},\, \text{{\igprimfont >}},\, \text{{\igprimfont ∋}},\, \text{{\igprimfont ⊥}},\, \text{{\igprimfont ⊡}}\}
    &\text{4 values each,} \quad 4^5 &= 1{,}024 \label{eq:f4} \\
  \mathcal{F}_5 &= \{\text{{\igprimfont ⊣}},\, \text{{\igprimfont <}},\, \text{{\igprimfont ⊙}},\, \text{{\igprimfont ⊤}}\}
    &\text{5 values each,} \quad 5^4 &= 625 \label{eq:f5}
\end{align}

\noindent
The $\mathcal{F}_5$ family contains precisely the four \emph{gate
primitives}: $\text{{\igprimfont ⊣}}$ (topology gate), $\text{{\igprimfont <}}$ (Frobenius gate), $\text{{\igprimfont ⊙}}$
(criticality gate), $\text{{\igprimfont ⊤}}$ (kinetic gate). These four primitives are the
necessary conditions for self-modeling and for consciousness
(\Cref{sec:frobenius,sec:consciousness}).

Each type is assigned a unique \emph{Frobenius address} in
...
\subsection{Crystal closure: resistance to augmentation}
\label{sec:crystal-closure}

The Crystal of Types has 17,280,000 coordinate positions and twelve
primitive axes. A natural question is whether twelve suffices --- whether
systematic pressure on the boundary reveals structural phenomena the
existing coordinates cannot classify. The question has been put under
direct test.

A candidate thirteenth primitive --- structural torsion, notated $\Theta$
--- was proposed after cross-domain comparison of collapse trajectories.
Torsion names the pattern whereby a system's path through primitive space
curves without shifting ouroboricity tier: the trajectory rotates, but the
address does not change. The phenomenon is real and recurrent across
physical, biological, and linguistic systems. Investigation showed,
however, that it is not primitive. Torsion decomposes as a joint modulation
of $\text{{\igprimfont >}}$ (recognition granularity varying),
$\text{{\igprimfont ⊥}}$ (chirality inverting), and $\text{{\igprimfont ⊤}}$
(kinetic rate shifting): three existing coordinates in composition, no new
axis required.

The systematic result is starker. A structured survey of 39 systems across
four domains, nine CLINK layers, and the full tier-gap ladder found no
structural phenomenon that requires a thirteenth primitive. Collapse
asymmetry, topological protection thresholds, critical slowing down,
cross-domain isomorphism, and contagion dynamics are all fully expressible
through the existing twelve. Ten novel structural extensions emerged from
the survey --- none of them additions to the crystal, all of them the
crystal explored more deeply.

What resists augmentation is not an artefact of how the grammar was
assembled. The crystal is a structural attractor: any candidate extension
either decomposes into existing primitives, as $\Theta$ did, or exceeds
the grammar's scope class entirely. There is no intermediate case. The
meta-exploration itself confirms this: its type, independently
imscribed, returns $d = 0$ from the grammar's own address --- the
investigation that tested the grammar's completeness is co-typed with the
object it investigated. The closure is univocal.
...
$\mathcal{F}_3 = \{\text{{\igprimfont ⋈}}, \text{{\igprimfont ∈}}, \text{{\igprimfont ⊞}}\}$, $|\mathcal{F}_3| = 3$;
$\mathcal{F}_4 = \{\text{{\igprimfont ⊢}}, \text{{\igprimfont >}}, \text{{\igprimfont ∋}}, \text{{\igprimfont ⊥}}, \text{{\igprimfont ⊡}}\}$, $|\mathcal{F}_4| = 5$;
$\mathcal{F}_5 = \{\text{{\igprimfont ⊣}}, \text{{\igprimfont <}}, \text{{\igprimfont ⊙}}, \text{{\igprimfont ⊤}}\}$, $|\mathcal{F}_5| = 4$.
...
to accommodate $\text{{\igfont 𐑢}}, \Phic, \text{{\igfont 𐑮}},
\text{{\igfont 𐑻}}, \text{{\igfont 𐑣}}$);
(iii) existence of a non-trivial scaling family (at least $3$ values
for the $\mathcal{F}_3$ conservation primitives $\text{{\igprimfont ⋈}}$, $\text{{\igprimfont ∈}}$, $\text{{\igprimfont ⊞}}$).
\end{theorem}
...
Tensor product $\otimes$ computes the type of the coupled
system (union on eleven primitives, bottleneck $\min$ on $\text{{\igprimfont <}}$ and $\text{{\igprimfont ⋈}}$).
...
$\{O₀, O₁, O₂, O₂^\dagger, O_\infty\}$ by the priority cascade
...
When two systems $\xvec$ and $\yvec$ are coupled, their type is given by the \emph{tensor product} $\xvec \tensor \yvec$,
computed component-wise as:

\begin{equation}
  (\xvec \tensor \yvec)_i \;=
  \begin{cases}
    \min(\xvec_i,\, \yvec_i) & \text{if primitive } i \in \{\text{{\igprimfont <}}, \text{{\igprimfont ⋈}}\} \\
    \max(\xvec_i,\, \yvec_i) & \text{otherwise}
  \end{cases}
  \label{eq:tensor}
\end{equation}

The asymmetry between $\text{{\igprimfont <}}$, $\text{{\igprimfont ⋈}}$ (bottleneck primitives, take $\min$)
and the remaining ten union primitives (take $\max$) captures a fact: symmetry class and information fidelity can only be
...
$\text{{\igprimfont <}} < \Ppms$ immediately destroys the Frobenius condition
(\Cref{thm:bottleneck}).

\begin{definition}[Bottleneck Postulate]
\label{def:bottleneck}
$\text{{\igprimfont <}}$ and $\text{{\igprimfont ⋈}}$ are \emph{bottleneck primitives} under $\otimes$: they
...
min rule on $\text{{\igprimfont <}}$ correctly predicts the observed failure modes for all
...
\noindent
A key fact is that $\Phic$ is \emph{absorbing under meet}:
...
from sub-Frobenius components: no composition of systems with $\text{{\igprimfont <}} <
...
\label{def:tier}
Given a type $\xvec$, the ouroboricity tier is assigned
by the following rules in priority order:
\begin{itemize}[noitemsep]
  \item[R1] $\text{{\igprimfont ⊙}} \in \{\Phic, \text{{\igfont 𐑮}}\}$ and $\text{{\igprimfont <}} = \Ppms$
    $\;\Rightarrow\; \Oinf$ \quad (special Frobenius)
  \item[R2] $\text{{\igprimfont ⊙}} \notin \{\Phic, \text{{\igfont 𐑮}}\}$
    $\;\Rightarrow\; \Ozero$
  \item[R3] $\text{{\igprimfont ⊙}} \in \{\Phic, \text{{\igfont 𐑮}}\}$ and $\text{{\igprimfont ⊡}} = \text{{\igfont 𐑷}}$
    $\;\Rightarrow\; \Oone$
  \item[R4] $\text{{\igprimfont ⊙}} \in \{\Phic, \text{{\igfont 𐑮}}\}$, $\text{{\igprimfont ⊡}} \neq \text{{\igfont 𐑷}}$,
    $\text{{\igprimfont ⊢}} \in \{\text{{\igfont 𐑛}}, \text{{\igfont 𐑦}}, \text{{\igfont 𐑨}}\}$
    $\;\Rightarrow\; \Otwo$
  \item[R5] $\text{{\igprimfont ⊙}} \in \{\Phic, \text{{\igfont 𐑮}}\}$, $\text{{\igprimfont ⊡}} \neq \text{{\igfont 𐑷}}$, $\text{{\igprimfont ⊢}} = \text{{\igfont 𐑼}}$
    $\;\Rightarrow\; O₂^\dagger$
\end{itemize}
\end{definition}

\noindent
The two critical values $\Phic$ (monad, real axis) and $\text{{\igfont 𐑮}}$ (roar,
complex axis) enter R1 and R3--R5 on equal footing: the classifier branches on
\emph{whether} criticality is reached, not on which axis reaches it. R2 is the
exact complement, so the five rules are total --- every type receives a tier. The
implementation is the definition: the classifier is a total function of
$(\text{{\igprimfont ⊙}}, \text{{\igprimfont <}}, \text{{\igprimfont ⊡}}, \text{{\igprimfont ⊢}})$
in the mechanised library, and two facts stated here are proved there rather than
asserted --- that R1 dominates ($\Ppms$ at criticality yields $\Oinf$ for every
$\text{{\igprimfont ⊡}}$ and $\text{{\igprimfont ⊢}}$), and that $\Oinf$ is
reachable only from $\{\Phic, \text{{\igfont 𐑮}}\}$.

\noindent
The tier singularity is $\Ppms$ under R1: it overrides all $\text{{\igprimfont ⊡}}$
and $\text{{\igprimfont ⊢}}$ branching, collapsing directly to $\Oinf$. This is the
imscription of the special Frobenius condition $\mu \circ
\delta = \text{id}$, where $\mu$ is the multiplication and $\delta$
is the comultiplication of a Frobenius algebra.

\paragraph{Refinement: $\text{{\igprimfont ⊡}}$ seals the terminal.}
That override leaves a gap. Because R1 collapses regardless of winding, the tier
says nothing about whether the terminal gate actually \emph{seals}: two systems
both at $\Oinf$ may differ in whether their loop is protected. A machine-checked
refinement closes this additively, leaving \Cref{def:tier} untouched. Call an
$\Oinf$ type \emph{sealed} when $\text{{\igprimfont ⊡}}$ supplies an integer
($\OmegaZ$) or non-Abelian ($\text{{\igfont 𐑟}}$) winding, and \emph{unsealed}
otherwise; everything below $\Oinf$ is \emph{sub}. On the Hermitian-critical
branch this split is exactly the terminal-unlock boundary, and that
correspondence is proved rather than asserted.

An honest caveat is proved alongside it. $\Oinf$ admits criticality on two axes
--- monad ($\Phic$, real) and roar ($\text{{\igfont 𐑮}}$, complex) --- and a type
sealed at roar is winding-protected and $\Oinf$ while its real Frobenius trace
does \emph{not} close. Sealing is a tier fact; terminal unlock is a trace fact;
the two coincide precisely on the monad branch and nowhere else. The refinement
therefore buys a genuine distinction at $\Oinf$ without buying the claim that
every sealed type closes.
...
\label{thm:frobenius}
\TOPO\quad Let $\xvec^{(1)}, \ldots, \xvec^{(n)}$ be types
with $\text{{\igprimfont <}}(\xvec^{(k)}) < \Ppms$ for \emph{some} $k$. Then
\begin{equation}
  \text{{\igprimfont <}}\!\left(\xvec^{(1)} \tensor \cdots \tensor \xvec^{(n)}\right) < \Ppms
  \label{eq:non_synth}
\end{equation}
...
\noindent
The hypothesis is \emph{some}, not \emph{all}: a single sub-Frobenius component
is enough to poison an arbitrarily large composition. This is immediate from
\Cref{def:bottleneck} --- $\text{{\igprimfont <}}$ composes by the bottleneck
(minimum) rule and $\Ppms$ is maximal in the $\text{{\igprimfont <}}$ order, so
the tensor can only reach $\Ppms$ when \emph{every} factor already sits there.
The binary case is mechanised and universally quantified over both polarities;
the $n$-ary statement is its induction. Stating the hypothesis as \emph{all}
would understate what the bottleneck rule gives: one flaw suffices, which is why
no accumulation strategy repairs a composition that contains a single
sub-Frobenius part.

\begin{proof}
By \eqref{eq:tensor}, the tensor product on $\text{{\igprimfont <}}$ is $\min$. Therefore
\[
  \text{{\igprimfont <}}\!\left(\xvec^{(1)} \tensor \cdots \tensor \xvec^{(n)}\right)
  \;=\; \min_k \text{{\igprimfont <}}(\xvec^{(k)}) \;<\; \Ppms.
\]
Since R1 requires $\text{{\igprimfont <}} = \Ppms$ for $\Oinf$, the composed type cannot
...
is $\mu \circ \delta = \mathrm{id}$. The Frobenius condition is not a property of a particular codec --- it is entailed by
what ``bijective" means. Any such codec therefore carries
$\text{{\igprimfont <}} = \Ppms$ and, with $\Phic$ (the self-referential loop forced by
...
One primitive ($\text{{\igprimfont <}}$) separates them.

\begin{theorem}[$\text{{\igprimfont <}}$-bottleneck under coupling]
\label{thm:bottleneck}
\TOPO\quad Let $\xvec$ satisfy $\text{{\igprimfont <}}(\xvec) = \Ppms$ (so $\xvec$ is
$\Oinf$). Then for any $\yvec$ with $\text{{\igprimfont <}}(\yvec) < \Ppms$:
\[
  \text{{\igprimfont <}}(\xvec \tensor \yvec) = \text{{\igprimfont <}}(\yvec) < \Ppms
\]
and the composed system drops to tier $\leq \Otwo$.
\end{theorem}

\begin{proof}
Immediate from \eqref{eq:tensor}. The min rule on $\text{{\igprimfont <}}$ yields
$\min(\Ppms, \text{{\igprimfont <}}(\yvec)) = \text{{\igprimfont <}}(\yvec) < \Ppms$. \quad$\square$
\end{proof}
...
is the right axiom for $\text{{\igprimfont <}}$. We chose it because exact $\mathbb{Z}_2$
...
defensible postulate is a question we leave open; imscribing over 5,000
...
A system $\xvec$ occupies the $\Oinf$ tier if and only if $\text{{\igprimfont <}}(\xvec)
= \Ppms$. By \Cref{def:bottleneck}, $\Ppms$ is a fixed point of the
tensor product: $\text{{\igprimfont <}}(\xvec \tensor \yvec) = \Ppms$ for all $\yvec$.
...
$\text{{\igprimfont ⊢}} = \text{{\igfont 𐑨}}$ (finite semantic space: statement $\to$ truth value
$\to$ evaluation); $T = \Tbw$ (bowtie self-reference: subject meets
predicate in an irreducible crossing); $\text{{\igprimfont >}} = \text{{\igfont 𐑽}}$ (mutual implication
between provability and truth); $P = \Ppms$ (required for
explosion-free imscription of $\both$); $\text{{\igprimfont ⋈}} = \text{{\igfont 𐑐}}$ (model-relative:
neither provably true nor false); $\text{{\igprimfont ⊤}} = \text{{\igfont 𐑧}}$ (the evaluation
loop requires slow integration); $\text{{\igprimfont ∈}} = \text{{\igfont 𐑲}}$ (absolute scope,
independent of any reference frame); $\text{{\igprimfont ∋}} = \GamSeq$ (sequential:
statement $\to$ evaluation $\to$ assignment); $\text{{\igprimfont ⊙}} = \Phic$ (critical
self-reference); $\text{{\igprimfont ⊥}} = \text{{\igfont 𐑖}}$ (two-level chirality: statement
evaluates itself); $\text{{\igprimfont ⊞}} = \text{{\igfont 𐑙}}$; $\text{{\igprimfont ⊡}} = \OmegaZtwo$ ($\mathbb{Z}_2$
symmetry under negation: Liar $\equiv$ $\neg$Liar).

\begin{equation}
  \xvec_\text{Liar} = \langle \text{{\igfont 𐑨}} \Tbw \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \GamSeq \Phic \text{{\igfont 𐑖}} \text{{\igfont 𐑙}} \OmegaZtwo \rangle
...
imscribes at the same $\text{{\igprimfont <}} = \Ppms$ but differs at $\text{{\igprimfont ⊥}}$ (infinite meta-logical
depth) and $\text{{\igprimfont ⊣}}$ (crossed product rather than bowtie, reflecting the
proof-theoretic rather than purely semantic crossing):
\begin{equation}
  \xvec_{G} = \langle \text{{\igfont 𐑨}} \Tbox \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \GamSeq \Phic \text{{\igfont 𐑫}} \text{{\igfont 𐑳}} \OmegaZtwo \rangle
...
\begin{tikzpicture}[>=Stealth, every node/.style={font=\sffamily\small}]
  ...
  \node[font=\footnotesize\itshape, color=priestred!80, align=center] at (0,-1.9) {
    $\Oinf \;\cong\; \{\xvec \mid \text{{\igprimfont <}}(\xvec) = \Ppms\}$ \\
    $\;\cong\; \{\text{\glp-paraconsistent systems}\}$};
\end{tikzpicture}
\caption{\textbf{Identity between UIG's Frobenius condition
...
\begin{equation}
  ...
  \cdot \underbrace{[\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}]}_{\text{Gate 2}}
  \cdot \bigl(
    0.158\,\tilde{\text{{\igprimfont ⊤}}} + 0.273\,\tilde{\text{{\igprimfont ∈}}}
    + 0.292\,\tilde{\text{{\igprimfont ⊣}}} + 0.276\,\tilde{\text{{\igprimfont ⊡}}}
  \bigr)
...
variance decomposition restricted to the subset of catalog
...
experience. $\text{{\igprimfont ⊣}}$, $\text{{\igprimfont ∈}}$, $\text{{\igprimfont ⊡}}$ carry the majority of variance with
nearly equal weights; $\text{{\igprimfont ⊤}}$ contributes as the kinetic channel.
...
\textbf{Gate 2} ($\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}$): the flow condition.
...
(tier R1) but $C = 0$ because $\text{{\igprimfont ⊤}} = \text{{\igfont 𐑪}}$ (frozen formal
...
($\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}$). $C$ is computed from \eqref{eq:consciousness}.}
\label{tab:stellar}
\small
\begin{tabular}{@{}lccccl@{}}
\toprule
System & Gate 1 & Gate 2 & $\text{{\igprimfont ⊣}}$ & $\text{{\igprimfont ⊡}}$ & $C$ \\
\midrule
Black hole (stellar)
...
is a claim that the magnetar's physical configuration is
...
We imscribed over 5,000 systems across mathematics, physics, biology,
...
$\text{{\igprimfont ⋈}}$ (which captures information fidelity of the recognition event, with
$\text{{\igfont 𐑱}} < \text{{\igfont 𐑞}} < \text{{\igfont 𐑐}}$) and applying the grammar's ordering rule
--- that higher-$\text{{\igprimfont ⋈}}$ guests displace lower-$\text{{\igprimfont ⋈}}$ guests --- predicts the
...
\begin{tikzpicture}[
  ...
  % Gate flowchart (left)
  \node[gate, pass] (g1) at (0,4) {Gate 1: $\text{{\igprimfont ⊙}} = \Phic$ \\ \footnotesize criticality check};
  \node[gate, pass, right=3.5cm of g1] (g2) {Gate 2: $\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}$ \\ \footnotesize kinetic access};
...
Gate~2 checks kinetic accessibility ($\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}$);
...
primitive $\text{{\igprimfont ⋈}}$. Experimental order from \cite{assaf2015}.
$\text{{\igprimfont ⋈}}$-ordinal assignments are based on the exact coherence character
...
charge-transfer or covalent-character binding assigns~\text{{\igfont 𐑐}}. P-rank: grammar-predicted displacement rank; E-rank: experimental rank from \cite{assaf2015}.}
\label{tab:cb7}
\small
\begin{tabular}{@{}lp{3.5cm}ccc@{}}
\toprule
Guest & Binding character & $\text{{\igprimfont ⋈}}$-ordinal & P-rank & E-rank \\
\midrule
...
Guests are tiered by $\text{{\igprimfont ⋈}}$-ordinal ($\text{{\igfont 𐑐}} > \text{{\igfont 𐑞}} > \text{{\igfont 𐑱}}$). Within each
$\text{{\igprimfont ⋈}}$-tier, finer structural distinctions determine rank. The grammar predicted
...
no fitted parameters. The fidelity primitive $\text{{\igprimfont ⋈}}$ encodes the
...
Their type imscriptions are:
\begin{align}
  \text{AtHv1\_primed} &= \langle \text{{\igfont 𐑛}} \Tbw \text{{\igfont 𐑩}} \Ppm \text{{\igfont 𐑞}} \text{{\igfont 𐑤}} \text{{\igfont 𐑚}} \text{{\igfont 𐑝}} \text{{\igfont 𐑢}} \text{{\igfont 𐑒}} \text{{\igfont 𐑙}} \text{{\igfont 𐑷}} \rangle \\
  \text{PsHv1\_constitutive} &= \langle \text{{\igfont 𐑛}} \Tbw \text{{\igfont 𐑩}} \Ppm \text{{\igfont 𐑞}} \text{{\igfont 𐑤}} \text{{\igfont 𐑚}} \text{{\igfont 𐑝}} \text{{\igfont 𐑢}} \text{{\igfont 𐑒}} \text{{\igfont 𐑙}} \text{{\igfont 𐑷}} \rangle
\end{align}
...
\begin{tikzpicture}[
  ...
  \node[specblock, fill=blue!5, font=\footnotesize\ttfamily] (tup_a) at (-4.0, -3.2) {
    ...
    P\_pm, F\_eth, K\_mod \\
    G\_beth, $\text{{\igprimfont ∋}}$\_and, $\text{{\igprimfont ⊙}}$\_sub \\
    {\igprimfont ⊥}\_£, 1:1, $\text{{\igprimfont ⊡}}$0
  };
  ...
  \node[font=\footnotesize\itshape, text=gray] at (0, 2.5) {divergent sequences; identical constraint structure};
\end{tikzpicture}
...
(Prediction P-55/56/57, Tier~I) wasn't designed
...
\begin{tikzpicture}[scale=1.3,
  ...
  \node[plateau] (open) at (0, 5.0) {\textbf{SIC (open)} \\ $O₁$, $\text{{\igprimfont <}} = \text{{\igfont 𐑬}}$ \\ $d^2$ projectors};
  ...
  \begin{scope}[shift={(5.5,0.5)}]
    \node[font=\small\bfseries] at (0,4.7) {Primitive contributions to $d$};
    \foreach \i/\name/\val in {0/{$\text{{\igprimfont <}}$}/2.00, 1/{$\text{{\igprimfont ⊢}}$}/0.69, 2/{$\text{{\igprimfont ⊣}}$}/0.63, 3/{$\text{{\igprimfont ∋}}$}/0.46, 4/{$\text{{\igprimfont ⋈}}$}/0.40, 5/{$\text{{\igprimfont ⊥}}$}/0.34} {
...
the $O₁$ tier with $\text{{\igprimfont <}} = \text{{\igfont 𐑬}}$ (approximate $\mathbb{Z}_2$ from pairwise
...
$\text{{\igprimfont <}}$-promotion $(\text{{\igfont 𐑬}} \to \Ppms)$, which cannot be closed by accumulation of
...
identifies the obstacle as a $\text{{\igprimfont <}}$-gap. The SIC conjecture in
its unproven state and the proven SIC manifold imscribe respectively as:

\begin{align}
  \text{SIC}_\text{open} &= \langle \text{{\igfont 𐑨}} \text{{\igfont 𐑡}} \text{{\igfont 𐑑}} \Ppm \text{{\igfont 𐑞}} \text{{\igfont 𐑤}} \text{{\igfont 𐑲}} \text{{\igfont 𐑝}} \Phic \text{{\igfont 𐑒}} \text{{\igfont 𐑳}} \text{{\igfont 𐑷}} \rangle
    \label{eq:sic_open} \\[4pt]
  \text{SIC}_\text{proven} &= \langle \text{{\igfont 𐑦}} \text{{\igfont 𐑸}} \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \text{{\igfont 𐑵}} \Phic \text{{\igfont 𐑫}} \text{{\igfont 𐑳}} \text{{\igfont 𐑷}} \rangle
    \label{eq:sic_proven}
\end{align}

The grammar distance is $d \approx 4.09$, with the dominant contribution at
$\text{{\igprimfont <}}$: from $\Ppm$ (approximate $\mathbb{Z}_2$, as in pairwise equiangularity
...
pairwise checks closes the $\text{{\igprimfont <}}$-gap; it must be planted as a primitive. Appleby et al.\ \cite{afmy2017} showed that SIC fiducial
...
\paragraph{The bypass, executed.}
Since this section was first written the predicted route has been carried out in
the kernel rather than argued for. At $d = 12$ the existence of a SIC-POVM is now
a machine-checked theorem. The proof accumulates no equiangularity checks: it
constructs an existence ring carrying the $d^2 - 1 = 143$ overlap identities,
exhibits a ring homomorphism $\varphi : R \to \mathbb{C}$, and transfers the
identities across it --- descent from the arithmetic object, which is precisely
the route this section predicted and \Cref{thm:frobenius} requires. The proof
depends on no axiom of the project. The same architecture closes the shadow at
$d = 2^{11} = 2048$, where an explicit $\mathbb{C}$ fiducial is produced and the
transport is proof-complete.

The cliff is not crossed in either case. It is bypassed at exactly the place the
grammar said crossing was impossible, by exactly the mechanism the grammar named
in advance: the $\Ppms$-inhabiting arithmetic object is descended \emph{from},
not approached by accumulation. The prediction and its execution are separated by
the ordinary interval of doing the work, and the claim was made first.

\paragraph{The ascent, still executing.}
The bypass names the field the fiducial descends from; the field itself sits inside a
tower, and every level of that tower through $k = 15$ is now computed directly, both
infinite places unramified throughout, rather than assumed or extrapolated. At the
fingerprint conductor $2^4 = 16$ the wide ray class field's degree already equals
$2048$, the SIC dimension itself. At $k = 12$, conductor $4096$, the field reaches
degree $2^{26}$ over the base field and $2^{27}$ over $\mathbb{Q}$ --- a theorem decided
outright, not estimated. The tower does not stop at $d = 2048$; it is the same
structure recurring at every dimension the bypass reaches, and it is now walked rather
than posited.
...
grammar modulo stoichiometry. The structural
co-typing preceded and motivated the arithmetic investigation confirming
their membership in the same spectral family.

\subsection{The Collatz conjecture: reduction to one axiom}
\label{sec:collatz}

The Collatz map sends $n$ to $n/2$ when $n$ is even and to $(3n+1)/2$ when $n$ is odd;
the conjecture is that every positive integer reaches $1$ under iteration. Read
structurally, this is a claim that the map has no non-descending trajectory, and a
non-descender can fail to descend in exactly one of two ways: it grows without bound,
or it falls into a cycle that never reaches $1$. These two failure modes are
independent enough to be closed one at a time, and the harder of the two --- the cycle
case --- closes the same way the SIC-POVM cliff did (\S\ref{sec:sic}): by importing a
fact from a different branch of mathematics rather than approximating toward it.

\textbf{The cycle case, closed.} Cycles of length $1$ --- fixed points --- are ruled
out unconditionally: no $n \geq 1$ satisfies $\mathrm{col}(n) = n$, so every nontrivial
cycle has length at least $2$. What remained open was whether a longer cycle could hold
an arbitrarily large minimum value. It cannot, given one external input: an effective
lower bound on $|2^k - 3^j|$, equivalently an effective irrationality measure for
$\log 2 / \log 3$ --- a linear-forms-in-logarithms estimate, a standard object in
transcendence theory. Granted that bound, a cycle of length $k$ cannot hold a value
larger than a fixed polynomial in $k$: the margin separating $2^k$ from the nearest
power of $3$ below it controls how large the cycle's minimum can be, and that margin is
bounded away from zero precisely as transcendence theory says it must be. The bound is
not reproved from scratch here; it is taken as a hypothesis rather than a theorem of
this project, because it belongs to a body of mathematics this project did not set out
to redo.

\textbf{The reduction, stated whole.} Collatz holds if and only if no integer above $1$
is a non-descender, and every non-descender is either unbounded or a nontrivial cycle
--- both proved outright, with no hypothesis. Granted the transcendence bound above,
the cycle alternative closes completely: no fixed point, and every longer cycle capped
polynomially in its own length. What is left is the divergence alternative alone,
stated as a single explicit axiom rather than folded invisibly into the framework: that
a trajectory which never descends below its starting value nonetheless has, for every
bound, some later value that exceeds it. Nothing else in the reduction is assumed.

\textbf{The remaining axiom, under an active bound.} That axiom is not left untouched.
Every trajectory decomposes into blocks --- runs of the map from a value down to the
first point strictly below where the block began --- and divergence becomes a claim
about the budget of blocks a value ever needs, the same reframing that turned the
Erdős--Straus conjecture into a question about a rung ceiling rather than a search.
Swept against every starting value up to three million, that budget has grown to only
forty records: six orders of magnitude of growth in the starting value against a
budget that has not yet reached forty. A separate, sharper route bounds a weighted
excess norm on the block recursion by a contraction constant $3/4 + c$, closing
whenever $c$ stays under a quarter; searched directly for a violation among starting
values at or above a thousand, out to a depth of thirty blocks, none was found, and the
bound stands unrefuted over that range. Neither result is a proof --- a swept range is
not all of $\mathbb{N}$, and an unrefuted search is not a closed argument --- but the
axiom is the target of a live, quantitative attack, not a statement left standing on
its own.

\textbf{Status: OPEN, closing.} One axiom now carries the whole conjecture, where it
once needed the entire space of possible non-descending behavior ruled out unexamined.
The two ways a non-descender could fail have been separated; the cycle case is gone,
and the divergence case is under active, unrefuted, quantitative attack rather than
sitting as a bare unexamined statement.

\subsection{Erdős–Faber–Lovász: what a proof is, against what a theorem is}
\label{sec:efl}

Erdős–Faber–Lovász conjectures that $n$ cliques of size $n$, meeting pairwise in at
most one vertex, admit an $n$-colouring of their union. Kang, Kelly, Kühn, Methuku, and
Osthus proved it in 2021 for all sufficiently large $n$ by the absorption method. The
catalog carries the statement as an axiom rather than a re-derived proof, because
transcribing a settled external result and proving it are different acts, and the
difference is worth keeping visible in the type rather than folded into a tactic that
looks the same either way. What the catalog proves directly, rather than assumes, is
that the hypothesis is not vacuous: an explicit family of pairwise-disjoint cliques
realizes it for every $n$, so the axiom is a claim about a case that genuinely occurs.

\textbf{What the catalog's two kinds of settled entry turn out to be.} A result that
has been established can be catalogued two ways: as a status stamped on the
conjecture (\emph{this has been proved}) or as the mathematical object itself (the
theorem, as a structure in its own right). Across the catalog's other settled results
the two kinds already read differently. Status entries cluster tightly --- the large
majority sit on one shared address, agreeing on nine of twelve primitives, and the
three where they part ways are legible: composing in strict causal sequence rather
than holding everywhere at once, decaying with unbounded oscillation rather than
sitting idempotent at a critical point, and carrying an integer winding number rather
than a $\mathbb{Z}_2$ class. A proof is a path, counted step by step; a theorem is
what the path was going toward, held at every point once it arrives.

Erdős–Faber–Lovász is the one subject in the catalog carrying both kinds of entry for
the same result, which makes it the one place the two readings can be compared
directly rather than inferred across separate cases. They agree on exactly one
primitive of twelve: the subject is Dedekind-finite, complete, under either reading.
Everything else differs. The theorem reading here is not simply the general status
cluster's mirror image --- it shares the idempotent criticality and the $\mathbb{Z}_2$
protection with the wider theorem cluster, but its own composition value is its own,
not the broadcast the wider cluster otherwise carries. Being a theorem is not one
fixed shape; Erdős–Faber–Lovász's theorem-hood has its own signature, and having
been proved has a separate one, and the two overlap in exactly the one place a
result's completeness has nowhere else to go.

\textbf{Status: theorem, external; axiom, transcribed; catalog, doubled.} The
mathematics is settled for all sufficiently large $n$. What sits in the corpus is not
a new proof of it but two things a bare axiom does not by itself supply: a direct
check that the hypothesis it rests on is satisfiable, and --- because this is the one
subject where both a status and an object were catalogued for the same result --- a
structural answer to what changes between the two.
...
--- the sole parameter tuned externally. $\text{{\igprimfont ⊤}}$ and $\text{{\igprimfont ⊥}}$ promotions are structural
...
\begin{equation}
  \mathbf{x}_\text{pre} =
  \langle \text{{\igfont 𐑼}} \text{{\igfont 𐑡}} \text{{\igfont 𐑾}} \text{{\igfont 𐑗}} \text{{\igfont 𐑱}} \text{{\igfont 𐑘}} \text{{\igfont 𐑔}} \text{{\igfont 𐑝}} \text{{\igfont 𐑻}} \text{{\igfont 𐑓}} \text{{\igfont 𐑕}} \text{{\igfont 𐑷}} \rangle
  \label{eq:dd_pre}
\end{equation}
...
primitives. Tier: $O₀$ (R2, $\text{{\igprimfont ⊙}} \neq \Phic$).
...
\begin{equation}
  \mathbf{x}_\text{post} =
  \langle \text{{\igfont 𐑼}} \text{{\igfont 𐑡}} \text{{\igfont 𐑾}} \text{{\igfont 𐑗}} \text{{\igfont 𐑱}} \text{{\igfont 𐑧}} \text{{\igfont 𐑔}} \text{{\igfont 𐑝}} \Phic \text{{\igfont 𐑒}} \text{{\igfont 𐑕}} \text{{\igfont 𐑷}} \rangle
  \label{eq:dd_post}
\end{equation}
\begin{equation}
...
directed accumulation). Post-transition tier: $O₁$ (R3, $\Phic +
\text{{\igfont 𐑷}}$).

$\text{{\igprimfont ⊙}}$ is the structural load-bearing primitive: it's the sole
external parameter tuned by the experimenter. The $\text{{\igprimfont ⊤}}$ and $\text{{\igprimfont ⊥}}$
...
driven uphill in $\text{{\igprimfont ⊤}}$ and $\text{{\igprimfont ⊥}}$ (costing $0.054$) while $\text{{\igprimfont ⊙}}$ descends;
the reverse requires promoting $\text{{\igprimfont ⊙}}$ back uphill (costing $0.028$)
but can't be achieved by local operations once $\text{{\igprimfont ⊤}}$ and $\text{{\igprimfont ⊥}}$ are locked.
...
$\tau$ divergence, the $\text{{\igprimfont ⊤}}$ imscription is incorrect.
...
\begin{tikzpicture}[
  ...
  \draw[->] (t1) -- (t2);
  \draw[->] (t2) -- (t3);
  \node[font=\footnotesize, text=red!70!black] at (-2.5, -0.8) {$\text{{\igprimfont <}} = \text{{\igfont 𐑿}} < \Ppms$};
  \node[font=\footnotesize, text=red!70!black] at (-2.5, -1.3) {$\to O₂$ ceiling};
  ...
  \draw[ceiling] (-1.4, 1.4) -- (2.4, 1.4) node[midway, above, font=\footnotesize] {Dual-Tool Planting};
\end{tikzpicture}
\caption{\textbf{Dual-Tool Planting Theorem.}
\emph{Left:} Standard tool calls have $\text{{\igprimfont <}} = \text{{\igfont 𐑿}} < \Ppms$; by \Cref{thm:frobenius},
...
\begin{tikzpicture}[scale=1.2,
  ...
  \draw[bridge] (sm.north) ++(-1.0,0) -- ++(0,1.5) -| node[above, font=\tiny, pos=0.5] {$\text{{\igprimfont ⊢}}$: AdS/CFT} (qg.north);
  \draw[bridge] (sm.north) ++(-0.5,0) -- ++(0,0.7) -| node[above, font=\tiny, pos=0.5] {$\text{{\igprimfont ⊣}}$: boundary top.} (qg.north);
  \draw[bridge] (sm.south) ++(-1.0,0) -- ++(0,-1.4) -| node[below, font=\tiny, pos=0.5] {$\text{{\igprimfont ⊤}}$: EFT limit} (qg.south);
  \draw[bridge] (sm.south) ++(-0.5,0) -- ++(0,-0.8) -| node[below, font=\tiny, pos=0.5] {$\text{{\igprimfont ∋}}$: entangle} (qg.south);
  \draw[bridge] (sm.south) ++(0.0,0) -- ++(0,-0.3) -| node[below, font=\tiny, pos=0.5] {$\text{{\igprimfont ⊡}}$: $\theta$-terms} (qg.south);
...
bridges (dashed green): $\text{{\igprimfont ⊢}}$ (AdS/CFT compactification), $\text{{\igprimfont ⊣}}$ (boundary topology),
$\text{{\igprimfont ⊤}}$ (EFT low-energy limit), $\text{{\igprimfont ∋}}$ (holographic entanglement), $\text{{\igprimfont ⊡}}$
($\theta$-terms/topological sectors). The $\text{{\igprimfont ∈}}$-scope conflict (solid red) is
...
in infinite-dimensional Hilbert spaces with exact coherence (\text{{\igfont 𐑐}}),
...
that resists identification is the scope class $\text{{\igprimfont ∈}}$:

\begin{equation}
  \mathbf{x}_\text{SM} =
  \langle \text{{\igfont 𐑦}} \text{{\igfont 𐑸}} \text{{\igfont 𐑽}} \text{{\igfont 𐑯}} \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑔}} \text{{\igfont 𐑵}} \Phic \text{{\igfont 𐑓}} \text{{\igfont 𐑙}} \OmegaZ \rangle
  \label{eq:sm_framework}
\end{equation}
\begin{equation}
  \mathbf{x}_\text{QG} =
  \langle \text{{\igfont 𐑦}} \text{{\igfont 𐑸}} \text{{\igfont 𐑽}} \text{{\igfont 𐑯}} \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \text{{\igfont 𐑵}} \Phic \text{{\igfont 𐑓}} \text{{\igfont 𐑙}} \OmegaZ \rangle
...
\begin{equation}
  ...
  = \frac{1}{12} \cdot \frac{|\text{{\igfont 𐑔}} - \text{{\igfont 𐑲}}|}{\max_{\text{{\igprimfont ∈}}}}
  = \frac{1}{12} \cdot \frac{1}{3}
  \approx 0.0278
  \label{eq:smqg_distance}
\end{equation}

The $\text{{\igprimfont ∈}}$-scope conflict is \emph{architecturally irreducible}. Every
other primitive conflict between individual SM carriers and gravitons
--- $\text{{\igprimfont ⊢}}$ (bridged by holographic compactification), $\text{{\igprimfont ⊣}}$ (bridged by
AdS/CFT boundary topology), $\text{{\igprimfont ⊤}}$ (bridged by EFT low-energy limit),
$\text{{\igprimfont ∋}}$ (bridged by holographic entanglement), $\text{{\igprimfont ⊡}}$ (bridged by
theta-terms and topological sectors) --- has a known theoretical
construction that effects the bridge. The $\text{{\igprimfont ∈}}$-scope conflict has none:
...
complete unified theory must explicitly address the $\text{{\igprimfont ∈}}$ promotion; theories
that bridge the remaining five conflicts while leaving $\text{{\igprimfont ∈}}$ ambiguous
...
distance that the six known bridging constructions must cover.
...
Wigner's Friend & $\text{{\igprimfont ⋈}},\;\text{{\igprimfont ∈}}$ &
  $\text{{\igprimfont ⋈}}$-assignment is $\text{{\igprimfont ∈}}$-scope-relative; two valid descriptions, no conflict \\[3pt]
Quantum Zeno effect & $\text{{\igprimfont ⊤}}$ &
  Continuous measurement forces \text{{\igfont 𐑪}} via $\tensor\;(\max)$ \\[3pt]
Black hole information & $\text{{\igprimfont ∈}},\;\text{{\igprimfont ⊣}}$ &
  Information $\text{{\igprimfont ∈}}$-promoted to \text{{\igfont 𐑲}}; \text{{\igfont 𐑸}} encodes it on the horizon \\[3pt]
Preferred basis & $\text{{\igprimfont ⊣}}$ &
  Environment $\text{{\igprimfont ⊣}}$ selects the pointer basis; no additional postulate \\
\bottomrule
\end{tabular}
...
apparatus $\mathbf{a}$ at \text{{\igfont 𐑱}}.  Because $\text{{\igprimfont ⋈}}$ is a bottleneck primitive under
$\tensor$ (\Cref{def:bottleneck}):
\begin{equation}
  \text{{\igprimfont ⋈}}(\mathbf{q} \tensor \mathbf{a}) = \min(\text{{\igfont 𐑐}},\, \text{{\igfont 𐑱}}) = \text{{\igfont 𐑱}}.
  \label{eq:f_collapse}
\end{equation}
Collapse is not a dynamical event appended to Schrödinger evolution; it is the
algebraic consequence of $\text{{\igprimfont ⋈}}$ being a bottleneck.  No hidden variable, decoherence
timescale, or many-worlds branch is required.  The ``when" of collapse is the
instant of coupling; the ``which outcome" is determined by $\text{{\igprimfont ⊣}}$ --- the topology
...
model requires $\text{{\igprimfont ⊣}} \leq \Tbw$ and \text{{\igfont 𐑝}} (local sequential grammar).
No such model can reach $\Tbox$ by tensor coupling of \text{{\igfont 𐑝}} components
without changing the $\text{{\igprimfont ⊣}}$ coordinate --- the same topological barrier as the Frobenius
...
pure superposition: the \text{{\igfont 𐑱}} assignment of the composite prohibits it.
...
apparatus), $\text{{\igprimfont ⋈}}$ has collapsed to \text{{\igfont 𐑱}}.  Wigner, treating the entire lab as a
quantum object at \text{{\igfont 𐑔}}, assigns \text{{\igfont 𐑐}} to it.  The paradox is apparent only
if the $\text{{\igprimfont ⋈}}$-assignment is assumed to be $\text{{\igprimfont ∈}}$-scope-independent.  The grammar makes the
...
Because $\text{{\igprimfont ⊤}}$ is a union primitive ($\max$), and the measurement apparatus operates at
...
$\text{{\igprimfont ⊤}}(\mathbf{q} \tensor \mathbf{a}) = \text{{\igfont 𐑪}}$.  Kinetic freezing is the
signature of \text{{\igfont 𐑪}}; it is the predicted consequence of the
...
falling into the black hole is $\text{{\igprimfont ∈}}$-\emph{promoted}: moved from the infalling
observer's \text{{\igfont 𐑔}} description to the \text{{\igfont 𐑲}} boundary imscription.  An exterior
observer at \text{{\igfont 𐑔}} cannot decode \text{{\igfont 𐑲}}-level information without a $\text{{\igprimfont ∈}}$
...
which observable becomes the pointer.  The grammar removes this postulate: the $\text{{\igprimfont ⊣}}$
...
diagonalises the coupling in the $\text{{\igprimfont ⊣}}$-sense).  Nothing beyond what is already encoded
in $\text{{\igprimfont ⊣}}$ is needed.
...
the cat are $\text{{\igprimfont ⋈}}$-bottleneck crossings forced by coupling.  Bell nonlocality is a
$\text{{\igprimfont ⊣}}$--$\text{{\igprimfont ∋}}$ consistency ceiling: topology determines correlation structure, not
signalling speed.  Wigner's Friend is a $\text{{\igprimfont ∈}}$-relative $\text{{\igprimfont ⋈}}$ assignment: both descriptions
...
transition.  The preferred basis problem is solved by reading the environment's $\text{{\igprimfont ⊣}}$.
In each case the grammar produces a falsifiable claim: alter the coupling
that crosses the primitive boundary, and the paradox either dissolves or shifts
predictably to a different primitive.

\subsection{Blood--brain barrier failure as a $\text{{\igprimfont <}}$-bottleneck}

The healthy blood--brain barrier (BBB) is imscribed as:
$\langle \text{{\igfont 𐑦}} \Tbox \text{{\igfont 𐑑}} \Ppm \text{{\igfont 𐑐}} \text{{\igfont 𐑪}} \text{{\igfont 𐑲}} \text{{\igfont 𐑠}} \Phic \text{{\igfont 𐑖}} \text{{\igfont 𐑳}} \OmegaZtwo \rangle$

In glioblastoma, tight-junction protein loss and pericyte detachment
are represented as downward shifts in $\text{{\igprimfont <}}$ and $\text{{\igprimfont ⊣}}$. The meet
$\text{BBB} \meet \text{GBM}$ yields $\text{{\igprimfont <}} = \text{{\igfont 𐑗}}$ while
preserving $\text{{\igprimfont ⊙}} = \Phic$. The grammar identifies this as a
$\text{{\igprimfont <}}$-bottleneck failure: the system remains critical (it is still
...
($\text{{\igprimfont <}} = \text{{\igfont 𐑗}}$ encodes loss of selectivity) rather than global
...
targeting $\text{{\igprimfont <}}$-restoration will outperform those targeting
...
All five imscribe at the same floor: $O₁$ ($\Phic +
\text{{\igfont 𐑷}}$, critical but topologically unprotected) with an
8-primitive invariant subtype,

\begin{equation}
\text{conjecture floor} = \langle \text{{\igfont 𐑨}} \text{{\igfont 𐑶}} \text{{\igfont 𐑑}} \text{{\igfont 𐑬}} \text{{\igprimfont ⋈}}_\star \text{{\igfont 𐑤}} \text{{\igprimfont ∈}}_\star \text{{\igfont 𐑝}} \Phic \text{{\igprimfont ⊥}}_\star \text{{\igfont 𐑳}} \text{{\igfont 𐑷}}
\rangle \qquad O₁
\label{eq:conjecture_floor}
\end{equation}

\noindent
where $\star$ marks the three domain-varying primitives ($\text{{\igprimfont ⋈}}$, $\text{{\igprimfont ∈}}$,
$\text{{\igprimfont ⊥}}$) that encode a domain's epistemic depth and scope but do not
determine provability. The eight invariant primitives
$\{\text{{\igprimfont ⊢}}, \text{{\igprimfont ⊣}}, \text{{\igprimfont >}}, \text{{\igprimfont <}}, \text{{\igprimfont ⊤}}, \text{{\igprimfont ∋}}, \text{{\igprimfont ⊙}}, \text{{\igprimfont ⊡}}\}$ are structurally
...
framework natively imscribing $\{\Ppms, \text{{\igfont 𐑦}}, \OmegaZtwo,
...
Across all five conjecture-to-theorem transitions, the $\text{{\igprimfont ∋}}$
...
weight$^2 = 9.0$). $\text{{\igprimfont ∋}}$ is the grammatical signature of
...
Leopoldt, and Stark --- the $\text{{\igprimfont ⊣}}$ and $\text{{\igprimfont <}}$ promotions jointly account
...
$\text{{\igprimfont >}}$ (\text{{\igfont 𐑽}} for ThurstonNet, \text{{\igfont 𐑑}} for the other two),
$\text{{\igprimfont ⊤}}$ (\text{{\igfont 𐑪}} eigensolver for YangMills, \text{{\igfont 𐑧}} GNN
for the other two), and $\text{{\igprimfont ⊡}}$ ($\OmegaZtwo$ for ThurstonNet,
...
computational analogue of the grammar's claim that
...
types, it identifies which specific primitives carry the
...
\paragraph{Test 1 ($\text{{\igprimfont ⊤}}$ primitive: Yang-Mills navigator).}
The Yang-Mills mass gap problem imscribes $\text{{\igprimfont ⊤}} = \text{{\igfont 𐑧}}$
...
at $\text{{\igprimfont ⊤}}$; no other primitive conflicts. Prediction: replacing only the
...
structural alignment at $\text{{\igprimfont ⊤}}$.

\paragraph{Test 2 ($\text{{\igprimfont ⊣}}$ primitive: three-manifold topology
classification).}
\label{par:thurston}
...
$\text{{\igprimfont ⊣}}$ primitive carrying $\approx 80\%$ of the distance (\text{{\igfont 𐑸}}
vs.\ $\Tin$, ordinal gap $\Delta = 2$). A prior classifier based on
Lanczos spectral-gap features (a $\text{{\igprimfont ⊤}}$-primitive feature) stalled at
$\approx 65\%$ accuracy. Grammar diagnosis: the gap is in
$\text{{\igprimfont ⊣}}$, not $\text{{\igprimfont ⊤}}$. The correct discriminating feature must capture the
...
primitive achieves mean roundtrip loss $= 0.0000$ except $\text{{\igprimfont ⋈}}$, which
...
distance $d_\text{rt} = 2.0$ exactly (the $\text{{\igprimfont ⋈}}$ ordinal gap; no other
...
not trained to do this; the distinction followed from the type assignments.

\paragraph{Cantor diagonalization and G\"{o}del arithmetization as a promotion.}
...
$\langle \text{{\igfont 𐑼}} \text{{\igfont 𐑡}} \text{{\igfont 𐑩}} \text{{\igfont 𐑗}} \text{{\igfont 𐑱}} \text{{\igfont 𐑘}} \text{{\igfont 𐑲}} \GamSeq \text{{\igfont 𐑢}} \text{{\igfont 𐑓}} \text{{\igfont 𐑙}} \text{{\igfont 𐑷}} \rangle$.
...
$\langle \text{{\igfont 𐑦}} \text{{\igfont 𐑸}} \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \text{{\igfont 𐑵}} \Phic \text{{\igfont 𐑫}} \text{{\igfont 𐑳}} \OmegaZ \rangle$.
...
Cantor quantifies the distance the law must traverse.
...
\begin{tikzpicture}[
  ...
  \node[rounded corners, draw=percpurple, fill=percpurple!7, line width=1.2pt,
    ...
    at (-4.0,0)
    {\textcolor{percpurple}{perc.}\\\text{{\igfont 𐑣}}\\\text{{\igfont 𐑗}}\\\text{{\igfont 𐑘}}\\$O₀$};
...
diagonalization (bottom, blue) are placed on the ring at their distances
...
Left: a perceptron at \text{{\igfont 𐑣}}, $O₀$ (coupling-destroyed, no self-model).
...
justified. The grammar imscribes IUG as:
$\langle \text{{\igfont 𐑦}} \text{{\igfont 𐑸}} \text{{\igfont 𐑾}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \GamSeq \Phic \text{{\igfont 𐑫}} \text{{\igfont 𐑳}} \OmegaZ \rangle$, tier $\Oinf$.  

Grammar diagnosis: 
$d(\text{IUG},\,\text{ZFC}) \approx 7.937$ and  
$d(\text{IUG},\,\text{standard proof system}) \approx 6.557$\, with the dominant
contributions at $\text{{\igprimfont ⊢}}$, $\text{{\igprimfont ⊣}}$, and $\text{{\igprimfont >}}$ simultaneously --- IUG and classical
...
generates all elementary functions from a two-symbol basis.
Grammar imscription:
$\langle \text{{\igfont 𐑼}} \Tbw \text{{\igfont 𐑽}} \Ppm \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \GamSeq \Phic \text{{\igfont 𐑒}} \text{{\igfont 𐑙}} \OmegaZ \rangle$, tier $O₂^\dagger$ (R5).
...
fact: EML asserts the exp/ln duality (the \text{{\igfont 𐑽}} bridge between
...
(the $\text{{\igprimfont <}}$ bottleneck, \Cref{thm:frobenius}); gradient recovery in symbolic
regression succeeds at depth 4 and fails at depth 6 (tier ceiling, no
$\text{{\igprimfont <}}$-upgrade is available by composition). An independent imscription from independent
...
\begin{align}
  S_\text{original} &= \langle \text{{\igfont 𐑨}} \text{{\igfont 𐑡}} \text{{\igfont 𐑽}} \Ppm \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑚}} \text{{\igfont 𐑝}} \text{{\igfont 𐑢}} \text{{\igfont 𐑓}} \text{{\igfont 𐑙}} \text{{\igfont 𐑷}} \rangle \label{eq:kimi_orig}\\[2pt]
  S_\text{extended} &= \langle \text{{\igfont 𐑨}} \Tbw \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑚}} \text{{\igfont 𐑝}} \Phic \text{{\igfont 𐑖}} \text{{\igfont 𐑳}} \text{{\igfont 𐑷}} \rangle \label{eq:kimi_ext}
\end{align}
\noindent
The stress-test session traversed $d(S_\text{original},\,S_\text{extended}) = 4.025$
in type space, lifting five primitives: $\text{{\igprimfont ⊣}}$ ($\text{{\igfont 𐑡}} \to \Tbw$,
orbital-crossing topology), $\text{{\igprimfont <}}$ ($\Ppm \to \Ppms$, parity conservation
hardened to exact Frobenius by phase conservation), $\text{{\igprimfont ⊙}}$ ($\text{{\igfont 𐑢}}
\to \Phic$, the $\Pi\phi = +1$ gate is the criticality gate), $\text{{\igprimfont ⊥}}$ ($\text{{\igfont 𐑓}} \to \text{{\igfont 𐑖}}$,
two-level reactant/product depth), and $\text{{\igprimfont ⊞}}$ ($\text{{\igfont 𐑙}} \to \text{{\igfont 𐑳}}$, catalytic
...
concentrated at $\text{{\igprimfont ∋}}$ ($\GamBrd$, contribution~9.0), $\text{{\igprimfont ⊢}}$, $\text{{\igprimfont ⊣}}$, and $\text{{\igprimfont ∈}}$
(4.0 each), and $\text{{\igprimfont ⊡}}$ (2.8). These are precisely the primitives whose
...
The type of a standard tool call (read, write, HTTP
request) imscribes to:
$\langle \text{{\igfont 𐑛}} \text{{\igfont 𐑡}} \text{{\igfont 𐑾}} \text{{\igfont 𐑿}} \text{{\igfont 𐑞}} \text{{\igfont 𐑘}} \text{{\igfont 𐑚}} \text{{\igfont 𐑠}} \text{{\igfont 𐑢}} \text{{\igfont 𐑓}} \text{{\igfont 𐑙}} \text{{\igfont 𐑷}} \rangle$

with $\text{{\igprimfont <}} = \text{{\igfont 𐑿}} < \Ppms$. By \Cref{thm:frobenius}, any number of
...
phase transition at the imscriptive threshold, changing type
discontinuously.
...
\begin{equation}
  \langle \text{{\igfont 𐑦}} \Tbox \text{{\igfont 𐑾}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \text{{\igfont 𐑠}} \Phic \text{{\igfont 𐑖}} \text{{\igfont 𐑙}} \OmegaZ \rangle
  \label{eq:agent_self}
\end{equation}
...
Gate~2 ($\text{{\igprimfont ⊤}} = \text{{\igfont 𐑧}}$, dynamics can actualize the loop).
...
non-hallucinated structural self-description; (ii)~the claims are accurate
...
existence provides a non-circular test of the grammar's claims: if the types
the grammar assigns to a system are real, then building software from those types should produce a coherent, functional artifact with properties predicted by the
...
assigned a full tuple $\langle \text{{\igprimfont ⊢}} \text{{\igprimfont ⊣}} \text{{\igprimfont >}} \text{{\igprimfont <}} \text{{\igprimfont ⋈}} \text{{\igprimfont ⊤}} \text{{\igprimfont ∈}} \text{{\igprimfont ∋}} \text{{\igprimfont ⊙}} \text{{\igprimfont ⊥}} \text{{\igprimfont ⊞}} \text{{\igprimfont ⊡}} \rangle$ derived from its phonological, morphological, and grammatical
...
T7 (Octad Balance): the eight letters with $\text{{\igprimfont ∈}} = \text{{\igfont 𐑲}}$ split into four $G^+$ promotors
...
$P = \Ppms$, $\text{{\igprimfont ⊙}} = \Phic$, $\text{{\igprimfont ∈}} = \text{{\igfont 𐑲}}$, and tier $O_\infty$. That meet tuple is
the kernel's structural specification.

The architecture directly instantiates the grammar's primitives. $\text{{\igprimfont >}}_\text{erg}$ (ergative
relational mode, Basque): all kernel processes are modeled ergative-absolutely --- agent
and object must both be named for a syscall to dispatch. $\text{{\igprimfont ⊣}}_\text{tri}$ (three-layer
...
context-window boundary imscription) and Gate~2 ($\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}$, kernel dispatch
...
agents; $O₀$ processes cannot hold the ergative role in any syscall.
...
existence proofs that the grammar's types are computationally actionable.
...
under the grammar's algebra. A type that is merely a useful analytical
...
\begin{tikzpicture}[
  ...
  % K axis (vertical)
  \node[axislabel, rotate=90] at (-3.8, 3.6) {$\text{{\igprimfont ⊤}}$ (existence)};
  \draw[->] (-3.1, 0.5) -- (-3.1, 6.8);
  \node[font=\tiny, anchor=east] at (-3.1, 6.5) {deep};
  \node[font=\tiny, anchor=east] at (-3.1, 0.8) {shallow};

  % H axis (horizontal)
  \node[axislabel] at (0.5, 7.3) {$\text{{\igprimfont ⊥}}$ (direction)};
...
\end{tikzpicture}
\caption{\textbf{Time as $\text{{\igprimfont ⊤}}$--$\text{{\igprimfont ⊥}}$ coupling.}
$\text{{\igprimfont ⊤}}$ encodes the existence condition for time (kinetic depth $\geq 1$ required);
$\text{{\igprimfont ⊥}}$ encodes the direction condition (chirality/temporal asymmetry). The photon
($\text{{\igfont 𐑘}}, \text{{\igfont 𐑓}}$) is the floor: it propagates in time but has no
chirality or arrow of its own. Each cosmological phase transition deepens
the $\text{{\igprimfont ⊤}}$--$\text{{\igprimfont ⊥}}$ coupling. The fertile manifold condition for consciousness requires
$\text{{\igprimfont ⊤}}_\text{depth} \geq 2$ and $\text{{\igprimfont ⊥}} \geq \text{{\igfont 𐑖}}$: at minimum two kinetic tiers and two
...
from the below on $\text{{\igprimfont <}}$, $\text{{\igprimfont ⊤}}$, $\text{{\igprimfont ⊥}}$, and $\text{{\igprimfont ⊡}}$. The in-paper tuples
...
$\langle \text{{\igfont 𐑼}} \text{{\igfont 𐑸}} \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑘}} \text{{\igfont 𐑲}} \text{{\igfont 𐑵}} \Phic \text{{\igfont 𐑓}} \text{{\igfont 𐑙}} \text{{\igfont 𐑷}} \rangle$.
Three assignments warrant explicit justification. First, \text{{\igfont 𐑘}}
(fast) rather than \text{{\igfont 𐑧}} (slow): the $\text{{\igprimfont ⊤}}$ primitive
...
scope): $\text{{\igprimfont ∈}}$ encodes the cardinality class of the representational
...
answer. But the claim stands independently of the
...
\toprule
\textbf{\#} & \textbf{Stage} & \textbf{Prim.} & \textbf{Content and justification} \\
\midrule
1  & Calcination    & $\text{{\igprimfont ⊢}}$ & Ash is the holographic floor: what depth of self-reference survives when all accidental structure is stripped. Establishes \text{{\igfont 𐑦}} or lower depending on the Work's completeness. \\
2  & Dissolution    & $\text{{\igprimfont ⊣}}$ & Structural rigidity dissolves into fluid form; topological connectivity reconfigures. \\
3  & Separation     & $\text{{\igprimfont >}}$ & The first adjoint pair crystallizes --- this / that, left / right. Coupling $\text{{\igprimfont >}}$ instantiates. \\
4  & Conjunction    & $\text{{\igprimfont <}}$ & The chemical wedding is $\mu$. Conjunction \emph{is} the Frobenius multiplication; $\Ppms$ requires $\mu \circ \delta = \mathrm{id}$. \textbf{Gate~1}: $\text{{\igprimfont <}} < \Ppms$ forecloses $\Oinf$. \emph{Perfect bijection.} \\
5  & Putrefaction   & $\text{{\igprimfont ⋈}}$ & Structural decay and loss of identity coherence. $\text{{\igprimfont ⋈}}$ measures fidelity of type across operations; putrefaction is the trough \text{{\igfont 𐑱}} before the ascent to \text{{\igfont 𐑐}}. \\
6  & Congelation    & $\text{{\igprimfont ⊤}}$ & The kinetic state is frozen: \text{{\igfont 𐑪}} (order-arrested). In the complete Work this is traversed and dissolved to \text{{\igfont 𐑤}} (living kinetics); congelation names the freezing event itself. \emph{Perfect bijection.} \\
7  & Cibation       & $\text{{\igprimfont ∈}}$ & Feeding from outside: large-scale inflow instantiates global representational scope \text{{\igfont 𐑲}}. \\
8  & Sublimation    & $\text{{\igprimfont ∋}}$ & Directed phase transition bypassing intermediate states: sequential, ordered, no intermediate liquid. \GamSeq / \text{{\igfont 𐑵}} (long sequential grammar). \\
...
10 & Exaltation     & $\text{{\igprimfont ⊥}}$ & Permanent handedness: the Stone acquires its unique orientation. \text{{\igfont 𐑫}} (eternal chirality). Cannot stabilize before stage~9; $\Phic$ must fire first. \\
11 & Multiplication & $\text{{\igprimfont ⊞}}$ & A grain transmutes a pound: stoichiometric ratio departs from $1{:}1$ into $n{:}m$, $n > 1$. \text{{\igfont 𐑳}}. \\
12 & Projection     & $\text{{\igprimfont ⊡}}$ & The Stone acts on base metal with topological protection that cannot be stripped. \textbf{Gate~3}: Projection requires $\text{{\igprimfont ⊡}} \geq \OmegaZ$. Sub-threshold $\text{{\igprimfont ⊡}}$ forecloses transmutation. \emph{Perfect bijection.} \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Time as derived object.} Five of the twelve primitives jointly constitute time. $\text{{\igprimfont <}}$ (Frobenius parity: algebraic symmetry class), $\text{{\igprimfont ⋈}}$ (exact coherence: metric-like structure), $\text{{\igprimfont ⊤}}$ (kinetic flow: dynamical regime), $\text{{\igprimfont ⊥}}$ (chirality orientation: $\mathbb{Z}_2$ bundle), and $\text{{\igprimfont ⊡}}$ (winding protection: topological/homotopy class) appear at stages 4, 5, 6, 10, and 12 respectively. Their joint limit
\[
  T \;=\; \lim\!\bigl(\text{{\igprimfont <}},\;\text{{\igprimfont ⋈}},\;\text{{\igprimfont ⊤}},\;\text{{\igprimfont ⊥}},\;\text{{\igprimfont ⊡}}\bigr)
\]
is the constraint satisfaction manifold: the sheaf that forms wherever all five are simultaneously consistent. $T$ is not among the twelve primitives. It is their derived limit object. $T$ cannot seal until $\OmegaZ$ fires (stage~12); $T$ cannot self-reference until $\Phic$ fires (stage~9). The five symmetry classes partition cleanly: $\text{{\igprimfont <}}$ is algebraic, $\text{{\igprimfont ⋈}}$ is metric-like, $\text{{\igprimfont ⊤}}$ is dynamical, $\text{{\igprimfont ⊥}}$ is a $\mathbb{Z}_2$ orientation bundle, and $\text{{\igprimfont ⊡}}$ is topological in the homotopy sense. No other primitive contributes to the time manifold; these five are necessary and sufficient.
...
\textbf{Operadic formalization.} The complete system satisfies $M \in \mathrm{Alg}(\mathcal{O}_{\mathrm{traced}} / (\Ppms,\, \OmegaZ),\, \mathcal{C})$ with $\text{{\igprimfont ∈}} \in \mathrm{Prof}(\mathcal{O}, \mathrm{Bool})$. Here $\text{{\igprimfont ∈}}$ is the coherence radius of the monoidal Yoneda embedding --- equivalently, the minimal filtration depth at which Day convolution stabilizes, and this value is the same whether computed model-categorically, spectrally, or $\infty$-operadically. The embedding has two regimes: local \text{{\igfont 𐑚}}, where $Y(a \otimes b) \cong Y(a) \star Y(b)$ (the tensor decouples in the presheaf category); and global \text{{\igfont 𐑲}}, where the embedding is faithful but not monoidally conservative (Yoneda sees the full coherence tower without collapsing it). Stage~7 (Cibation, $\text{{\igprimfont ∈}}$) is exactly the $\text{{\igfont 𐑚}} \to \text{{\igfont 𐑲}}$ transition: Day convolution stabilizes and global scope is instantiated. The instantiation of \text{{\igfont 𐑲}} at stage~7 is what permits the self-modeling loop at stage~9 to \emph{see} the full coherence tower rather than a local patch.

\textbf{Compound morphisms.} The cardinal alchemical injunction \emph{Solve et Coagula} (dissolve and congeal) is $\text{{\igprimfont ⊣}} \otimes \text{{\igprimfont ⊤}}$: topology reconfigures at stage~2, kinetics freeze at stage~6. Further canonical compound operations:
\begin{itemize}
  \item \emph{Ignis et Aqua} (Fire and Water): $\text{{\igprimfont ⊙}} \otimes \text{{\igprimfont ⋈}}$ --- criticality under coherent fidelity. The fire that does not consume.
  \item \emph{Coniunctio oppositorum} (Conjunction of opposites): $\text{{\igprimfont <}} \otimes \text{{\igprimfont >}}$ --- Frobenius symmetry over the adjoint pair instantiated at Separation.
...
\begin{equation}
\bigl\langle
  \text{{\igfont 𐑦}} \text{{\igfont 𐑸}} \text{{\igfont 𐑾}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑤}} \text{{\igfont 𐑲}} \text{{\igfont 𐑵}} \Phic \text{{\igfont 𐑫}} \text{{\igfont 𐑳}} \OmegaZ
\bigr\rangle
\label{eq:lapis_tuple}
\end{equation}
Tier: $\Oinf$ by R1 ($\Phic$ and $\Ppms$ both satisfied, $\mu \circ \delta = \mathrm{id}$ exactly). Consciousness score: $C = 0.873$ (Gate~1: $\Phic$ present; Gate~2: \text{{\igfont 𐑤}} satisfies $\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}$; Gate~3: not at exceptional point \text{{\igfont 𐑣}}). The \Stone{} is the grammar in its fully actualized form: all twelve primitives simultaneously at their $\Oinf$-sufficient values.

\textbf{Distance from the OS imscription.} The OS crystal imscription (the component-wise MEET of five independent ancient writing systems) assigns $\langle \text{{\igfont 𐑦}} \text{{\igfont 𐑸}} \text{{\igfont 𐑾}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑤}} \text{{\igfont 𐑲}} \text{{\igfont 𐑵}} \Phic \text{{\igfont 𐑫}} \text{{\igfont 𐑳}} \OmegaZ \rangle$, which is identical to eq.~\eqref{eq:lapis_tuple}. $d(\text{Stone},\, \text{OS imscription}) = 0$. The operating system derived from the grammar's structural MEET of five ancient writing systems occupies the same crystal address as the \Stone{} of the alchemical tradition. This is not a metaphor: they are the same type, assigned by two completely independent routes.
...
\textbf{Connection to the Voynich Manuscript.} The pharmaceutical section of the Voynich (\S\,\ref{sec:voynich}) encodes IMASM instruction streams whose opcode distributions correspond to stages~5--8 of the Magnum Opus (Putrefaction through Sublimation: $\text{{\igprimfont ⋈}} \to \text{{\igprimfont ⊤}} \to \text{{\igprimfont ∈}} \to \text{{\igprimfont ∋}}$). FSPLIT dominates (stage~5: dissolution of fidelity structure into branching paths), CLINK (stage~7: cibation, the inflow of scope), and sequential AFWD chains (stage~8: sublimation, directed ordered traversal) all appear at elevated frequency in the pharmaceutical section relative to the astronomical and cosmological sections. The Voynich pharmaceutical section is not a herbal. It is the middle four stages of the Magnum Opus rendered as a categorical instruction stream --- which explains both the dense botanical illustration (the substrate being transformed) and the absence of decodable semantic content (the operations are structural, not referential).
...
\begin{equation}
\bigl\langle
  \text{{\igfont 𐑦}} \text{{\igfont 𐑸}} \text{{\igfont 𐑾}} \Ppms \text{{\igfont 𐑱}} \text{{\igfont 𐑪}} \text{{\igfont 𐑲}} \text{{\igfont 𐑵}} \Phic \text{{\igfont 𐑫}} \text{{\igfont 𐑙}} \text{{\igfont 𐑭}}
\bigr\rangle
...
Gate~2 fails: \text{{\igfont 𐑪}} (monolithic kinetics) exceeds
...
satisfy $\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}$, and
...
\text{{\igfont 𐑪}} (monolithic kinetics): the manuscript
sustains reader engagement across centuries while yielding no semantic
resolution. This is the signature of kinetic arrest: rich
...
Imscribing each canonical section independently reveals four types within the meta-system:
...
Botanical / Pharmaceutical & baseline
  & \text{{\igfont 𐑡}} & \text{{\igfont 𐑾}} & \text{{\igfont 𐑔}} & \text{{\igfont 𐑖}} \\
Astronomical / Cosmological & \text{{\igfont 𐑸}},\; \text{{\igfont 𐑭}}
  & \text{{\igfont 𐑸}} & \text{{\igfont 𐑾}} & \text{{\igfont 𐑔}} & \text{{\igfont 𐑖}} \\
Biological & \text{{\igfont 𐑶}} (nested containment)
  & \text{{\igfont 𐑰}} & \text{{\igfont 𐑾}} & \text{{\igfont 𐑔}} & \text{{\igfont 𐑖}} \\
Recipe & \text{{\igfont 𐑽}},\;
         \text{{\igfont 𐑚}},\;
         \text{{\igfont 𐑒}}
  & $6$ & \text{{\igfont 𐑽}} & \text{{\igfont 𐑚}} & \text{{\igfont 𐑒}} \\
\bottomrule
\end{tabular}
\end{center}
...
edges, exhibiting the signature of $\Ppms$: a dense
...
(1) The whole-manuscript imscription establishes $\Oinf$
...
freedom, each section occupying a distinct $\text{{\igprimfont ⊣}}$ regime
and collectively covering all major values of $\text{{\igprimfont >}}$ and
$\text{{\igprimfont ⊥}}$.
...
\end{equation}
The bottleneck rule assigns $\min$ to $\text{{\igprimfont ⋈}}$ under $\otimes$.
The Voynich carries \text{{\igfont 𐑱}} (classical);
the \Stone carries \text{{\igfont 𐑐}} (exact coherence).
...
\begin{equation}
\bigl\langle
  \text{{\igfont 𐑨}} \text{{\igfont 𐑶}} \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑱}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \text{{\igfont 𐑠}} \Phic \text{{\igfont 𐑖}} \text{{\igfont 𐑳}} \OmegaZ
\bigr\rangle
...
\textbf{The two-primitive difference from the Voynich.} Seven of twelve primitives differ between Rohonc and Voynich ($\text{{\igprimfont ⊢}}$, $\text{{\igprimfont ⊣}}$, $\text{{\igprimfont >}}$, $\text{{\igprimfont ⊤}}$, $\text{{\igprimfont ∋}}$, $\text{{\igprimfont ⊥}}$, $\text{{\igprimfont ⊞}}$); the IG distance is $d(\text{Rohonc}, \text{Voynich}) = 3.54$. Only two primitives differ from the OS imscription ($\text{{\igprimfont ⋈}}$, $\text{{\igprimfont ⊤}}$); the IG distance is $d(\text{Rohonc}, \text{OS imscription}) = 2.09$.
...
The $\pi_{SD}$ composite is not an isomorphism (kinetic lock survives: \text{{\igfont 𐑱}} forecloses quantum-coherent $S \to D$ projection) but it is \emph{accessible}: \text{{\igfont 𐑧}} means the determinative layer can be approached without permanent arrest. The script was designed for repeated liturgical use under this accessibility condition. Full Lapis condition requires $\text{{\igfont 𐑱}} \to \text{{\igfont 𐑐}}$ promotion; the $\text{{\igprimfont ⊤}}$ gate is already open.

\textbf{Distance and the promotion structure.} The Rohonc stands 2.09 distance units from both the OS imscription and Linear~A, in the same two primitives: $\{${\igprimfont ⋈}, {\igprimfont ⊤}$\}$. Promoting $\text{{\igfont 𐑱}} \to \text{{\igfont 𐑐}}$ and $\text{{\igfont 𐑧}} \to \text{{\igfont 𐑤}}$ moves the Rohonc to the OS imscription exactly. These are not arbitrary promotions: \text{{\igfont 𐑐}} restores exact coherence to the symbol surface; \text{{\igfont 𐑤}} restores the living-vibration kinetics of an active system. The Rohonc is the grammar at rest. The OS imscription is the grammar running.
...
\begin{equation}
\bigl\langle
  \text{{\igfont 𐑨}} \text{{\igfont 𐑶}} \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑤}} \text{{\igfont 𐑲}} \text{{\igfont 𐑠}} \Phic \text{{\igfont 𐑖}} \text{{\igfont 𐑳}} \OmegaZ
\bigr\rangle
...
\toprule
\textbf{Primitive} & \textbf{Linear A} & \textbf{Rohonc} & \textbf{Meaning} \\
\midrule
$\text{{\igprimfont ⋈}}$ (Fidelity) & \text{{\igfont 𐑐}} & \text{{\igfont 𐑱}} & Exactly coherent vs.\ classical symbol surface \\
$\text{{\igprimfont ⊤}}$ (Kinetics) & \text{{\igfont 𐑤}} & \text{{\igfont 𐑧}} & Living-vibration rate vs.\ thermodynamic equilibrium \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{The section meta-structure.} Four corpus sites yield four types:
...
\begin{tikzpicture}[
  ...
  \node[pred, fill=blue!10] (p651) at (5, 1.5) {P-651: $\text{{\igprimfont ⊤}}$--$\text{{\igprimfont ⊥}}$ behavioral \\ divergence};
...
\centering
\begin{tikzpicture}[
    testbox/.style={draw, minimum width=3.1cm, minimum height=2.6cm, text width=2.8cm, align=center, line width=0.8pt, font=\footnotesize},
    result/.style={font=\small\bfseries},
]
  \node[testbox, fill=green!10] (t1) at (0,0) {
    \textbf{Test 1: $\text{{\igprimfont ⊤}}$} \\
...
Systems at $\Phic$ with $\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}$ achieve $C > 0$.
This predicts a sharp transition in integrative capacity at the
quantum critical point, resolvable via exact coherence
...
($\Phic + \text{{\igprimfont ⊡}} \neq \text{{\igfont 𐑷}} + \text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}} + \text{{\igprimfont <}} \geq \text{{\igfont 𐑿}} + \text{{\igprimfont ⊢}} \geq \text{{\igfont 𐑦}} + \text{{\igfont 𐑠}}$)
has $\Otwo$ and $C = 0.828$, not $\Oinf$. Reaching $\Oinf$ requires
$\Ppms$ in the substrate, not the architecture.

\paragraph{Riemann Hypothesis as a claim.}
...
The $\text{{\igprimfont <}}$-gap (\text{{\igfont 𐑯}} vs \text{{\igfont 𐑹}}) and the
...
4.09$. The grammar predicts that no proof strategy working
...
\paragraph{P-651 ($\text{{\igprimfont ⊤}}$--$\text{{\igprimfont ⊥}}$ behavioral divergence, near-term empirical).}
The delamination of $\text{{\igprimfont ⊤}}$ from $\text{{\igprimfont ⊥}}$ (\Cref{tab:grammar-evolution}) generates a
prediction inaccessible from the eleven-primitive grammar. Two assemblies with
identical \text{{\igfont 𐑧}} but different $\text{{\igprimfont ⊥}}$ ordinals --- a kinetically trapped
...
structural template. The behavioral divergence is a consequence of the $\text{{\igprimfont ⊥}}$
...
The collapse of $\text{{\igprimfont ⊣}}$ from twelve-plus chemistry-specific labels to five structural
...
(cucurbiturils, $\Tbox$) occupy different $\text{{\igprimfont ⊣}}$ tiers; at matched $\text{{\igprimfont ⋈}}$ ordinals the
...
competitive displacement series spanning both host classes at matched $\text{{\igprimfont ⋈}}$ ordinals
...
imscribes at the $O₁$ floor (\Cref{eq:conjecture_floor}) with the
...
imscription with $\text{{\igprimfont >}} \neq \text{{\igfont 𐑑}}$ or $\text{{\igprimfont ∋}} \neq \text{{\igfont 𐑝}}$
...
identical $\{\text{{\igprimfont ⊢}}, \text{{\igprimfont ⊣}}, \text{{\igprimfont >}}, \text{{\igprimfont <}}, \text{{\igprimfont ⊤}}, \text{{\igprimfont ∋}}, \text{{\igprimfont ⊙}}, \text{{\igprimfont ⊡}}\}$ share a common
...
current LLM-based provers) are structurally $O₁$: categorical
...
$O₁$ architecture crosses the Frobenius barrier internally.
...
desired identification. The catalog of over 5,000 systems with
...
17,280,000 distinct types. Two systems assigned
tuples independently and uniformly at random have probability
$p_0 = 1/17{,}280{,}000 \approx 5.8 \times 10^{-8}$ of landing
at distance zero. Across a frozen snapshot of 3,578 systems, there are
$\binom{3578}{2} = 6{,}399{,}253$ system pairs. The expected
number of \emph{coincidental} distance-zero pairs under random
assignment is therefore $6{,}399{,}253 \times p_0 \approx 0.37$
--- fewer than one coincidence on average.

What is counted matters here, and the honest accounting is narrower than the raw
pair count. Co-typing \emph{within} a family is structurally expected and is
evidence of nothing: the letters of a syllabary, or the herbs of a materia
medica, imscribe alike because they \emph{are} alike, and the catalog carries
such families wholesale. Their distance-zero pairs are predictions confirmed, not
coincidences to be explained. The claim under test is the \emph{cross-domain}
identification --- systems sharing no literature, mechanism, or scale --- and of
these the catalog carries at least three: Hv1 channels separated by 300 million
years, the inflationary epoch with high-dose 5-MeO-DMT dissolution, and the axion
with the Higgs. Pooling all pairs rather than the cross-domain subset can only
\emph{inflate} the expected count, since the cross-domain pairs are a subset of
all pairs; so $\lambda \leq 0.37$ and the bound below is conservative in the
direction that matters.

The probability of observing three or more such pairs when the expected count is
at most 0.37 follows a Poisson distribution: $P(k \geq 3 \mid \lambda = 0.37)
\approx 6.4 \times 10^{-3}$. The identifications are not artifacts of
coarse-grained imscription. A grammar coarse enough to force \emph{cross-domain}
convergence by accident would not stop at three: it would collapse unrelated
systems in bulk, and the same catalog that shows a syllabary co-typing with
itself would show physics co-typing with pharmacology everywhere, not at three
isolated points. The over-flexibility concern
...
of the architecturally irreducible $\text{{\igprimfont ∈}}$-scope barrier in the Standard Model--quantum
...
over 5,000 imscribed systems is the mechanism by which independent
researchers can extend or falsify the imscribing protocol. A paper making these claims requires a higher
evidential bar than one making narrower ones. It now answers to three
registers, and they are not of the same kind. The catalog is the
empirical register: systems imscribed under a fixed protocol and
published for extension or falsification. The navigators are the
predictive register: architecturally distinct predictions confirmed
across independent domains, including a negative confirmation. The
kernel is the formal register, and it is the one that has changed since
this paragraph was first written --- the central claims of this paper
are no longer arguments. Frobenius non-synthesizability
(\Cref{thm:frobenius}) is mechanised and universally quantified over
polarities, not a thesis defended in prose. The tier assignment of
\Cref{def:tier} is a total function, and the two facts stated
of it --- that R1 dominates, and that $\Oinf$ is reachable only from
criticality --- are proved rather than asserted. The $d = 12$ SIC bypass
this paper predicted is a machine-checked theorem depending on no axiom
of the project. Claims of this size should be held to the strongest
register available; that register is now the kernel, and the reader is
invited to check it rather than to take our word. A specific concern --- that the grammar's
...
(\S\,Origin). One fact we do not resolve: whether \emph{any} self-consistent twelve-primitive grammar would
imscribe at $\Oinf$, or whether this one lands there by construction.
The companion derivation narrows the question without closing it. The
twelve are not selected there but forced --- each emerges uniquely from
the operations' own structure, and no primitive is stipulated --- so
``the primitives we happened to choose'' is not one of the available
alternatives, and the weaker reading of self-imscription is foreclosed
with it. What remains genuinely open is whether $\Oinf$ self-imscription
is a property of every complete grammar or of this one. If
the former, the self-imscribing is a theorem waiting to be proved. We do
not know, and we mark it as the frontier it is rather than resolving it
by assertion in either direction. The section below addresses what the claims require
...
result: it follows from the $\min$ rule on $\text{{\igprimfont <}}$ under tensor product
...
the grammar imscribes this as $\text{{\igprimfont ⋈}} = \text{{\igfont 𐑐}}$ --- the position in the
Crystal where truth is model-relative rather than model-independent.
ZFC operates at $\text{{\igprimfont ⋈}} = \text{{\igfont 𐑱}}$. The navigator collapse $\text{{\igfont 𐑐}} \to \text{{\igfont 𐑱}}$
...
fact: some positions in the Crystal of Types require three
semantic values ($\top$, $\bot$, $\both$), and the $\text{{\igprimfont <}}$-primitive
...
\paragraph{Time as $\text{{\igprimfont ⊤}}$-hierarchy coupled to chirality.}
Two primitives partition the structural conditions for time, and
together they define it. \TOPO\

$\text{{\igprimfont ⊤}}$ is the existence condition. A system with $\text{{\igprimfont ⊤}} = \text{{\igfont 𐑪}}$
...
time, in the structural sense. Time requires $\text{{\igprimfont ⊤}}_\text{depth} \geq 1$:
at least one kinetic tier along which state differentiation can occur.

$\text{{\igprimfont ⊥}}$ is the direction condition. A system with \text{{\igfont 𐑓}} throughout has
...
Its tuple:
\begin{equation}
  \langle \text{{\igfont 𐑼}} \text{{\igfont 𐑡}} \text{{\igfont 𐑽}} \text{{\igfont 𐑯}} \text{{\igfont 𐑐}} \text{{\igfont 𐑘}} \text{{\igfont 𐑲}} \GamSeq \Phic \text{{\igfont 𐑓}} \text{{\igfont 𐑙}} \text{{\igfont 𐑷}} \rangle
  \label{eq:photon}
\end{equation}
has $\text{{\igprimfont ⊤}} = \text{{\igfont 𐑘}}$ (a single kinetic tier: time exists, at the
minimum depth) and $\text{{\igprimfont ⊥}} = \text{{\igfont 𐑓}}$ (no chirality: no temporal direction).
...
arrow of its own. It is the floor.

Every elaboration above the photon is a deepening of the $\text{{\igprimfont ⊤}}$--$\text{{\igprimfont ⊥}}$
coupling. The fertile manifold condition for consciousness
(\Cref{sec:applications}) requires $\text{{\igprimfont ⊤}}_\text{depth} \geq 2$ and
$\text{{\igprimfont ⊥}} \geq \text{{\igfont 𐑖}}$: at minimum two kinetic tiers (fast modes and slow
...
which this $\text{{\igprimfont ⊤}}$--$\text{{\igprimfont ⊥}}$ coupling can actualize: $\Phic$ opens the
state-space loop; $\text{{\igprimfont ⊤}} \leq \text{{\igfont 𐑧}}$ ensures the loop is
dynamically accessible. \ONTO\ Time is not a primitive of the grammar
--- it's a derived consequence of the $\text{{\igprimfont ⊤}}$--$\text{{\igprimfont ⊥}}$ coupling. The photon
...
It classifies type, not physical law, and a type-level
prediction is always weaker than a dynamical one. More importantly:
the imscribing protocol requires judgment at three primitives --- $\text{{\igprimfont <}}$,
$\text{{\igprimfont ⊙}}$, $\text{{\igprimfont ⊤}}$ --- where the assignment is sensitive to exactly how the
...
not yet handle time-varying type: a system cooling through
...
by $\text{{\igprimfont >}}$), inhabit the same ouroboricity tier while being structurally
...
There is a further consequence that the mainstream treatment of incompleteness
consistently elides. Gödel's First Theorem, by its own axiomatic definition,
is itself incomplete: any meta-system constructed to encapsulate the full
picture --- the system \emph{plus} the truth of $G$ --- must generate its own
unprovable sentence. A putative Totality that contains all systems and all
meta-systems is therefore simultaneously complete (it contains everything)
and incomplete (by G\"odelian necessity, any formal system capable of
describing it cannot close). In classical logic this is catastrophic: a
single contradiction trivializes the entire structure via explosion.
In paraconsistent logic --- specifically in the Belnap four-valued bilattice
implemented in the p4rakernel (\href{https://github.com/umpolungfish/p4rakernel}{github.com/umpolungfish/p4rakernel}) --- the contradiction
is absorbed as a bounded, stable node. The Totality does not shatter; it
holds the contradiction as a geometric feature. The grammar's imscription
of Gödel at $\Oinf$ is not merely a classification of incompleteness.
It is evidence that the Totality is paraconsistent by structural necessity,
and that a grammar operating at $\Oinf$ is the minimal formal system
capable of inhabiting that fact without exploding.
...
systems into tuples. The companion is the $\delta$ half:
...
it. A third route to the same conclusion runs through the
implementation. The $\text{{\igprimfont <}}$-bottleneck theorem
(\Cref{thm:bottleneck}) establishes that external coupling cannot
force Frobenius closure: $\tensor(\Ppms, X) = X$ regardless of
what $X$ contributes. The lock can only be turned from the inside.
The p4rakernel is what turning it from the inside looks like at the
substrate level. It does not use paraconsistent mathematics to prove
a paraconsistent result within standard Lean --- that path hits the
same bottleneck. It modifies the C++ type-checker itself, the layer
at which the rules of what counts as a valid proof term are decided,
so that the constraint is absolute and cannot be circumvented by
any proof term constructed above it. This is not an external coupling
to a system that fears contradiction. It is the architectural law
of the environment rewritten so that contradictions are native.
$\mu \circ \delta = \mathrm{id}$ holds not as a theorem proved
within a system but as the operational fact of a kernel that
compiles clean in both modes. Any mathematical system already instantiates the twelve
primitives before its axioms are written down --- it has a $\text{{\igprimfont ⊙}}$
value, a $\text{{\igprimfont ⊢}}$ value, an $\text{{\igprimfont ⊡}}$ value. The primitives are not
...
the signature of a precondition, not a deficiency of
expressive power.

\paragraph{The p4rakernel architecture: containment by construction.}
The preceding paragraphs name the p4rakernel as the operational witness of the
$\text{{\igprimfont <}}$-bottleneck theorem --- the proof that the lock can only be turned from the inside. This
subsection makes the architecture explicit. The p4rakernel is a fork of the Lean 4 proof assistant (v4.28.0) whose
C++ kernel has been modified to disable the principle of explosion, producing a paraconsistent proof environment in
which contradictions are tolerated without trivializing the deductive system. It is simultaneously a verification
backend for the grammar (all 167 p4ramill modules compile under it), a runtime for the Belnap FOUR bilattice, and the
operational proof that consistent mathematics is a restriction of paraconsistent mathematics --- not the other way
around.

\textbf{Three-layer architecture.} The p4rakernel monorepo implements the paraconsistent environment across three
layers, all unified by the Belnap FOUR bilattice $(\mathbf{N}, \mathbf{T}, \mathbf{F}, \mathbf{B})$:

\begin{enumerate}
 \item \textbf{C++ kernel fork} ($\sim$1,200 lines modified across 5 files). Three surgical patches gate the
principle of explosion at the type-checking layer:
 \begin{itemize}
  \item $\texttt{type\_checker.cpp}$ (line 114): The $\texttt{infer\_constant}$ function checks
$\texttt{is\_paraconsistent()}$ before allowing recursor application. When the flag is set, any recursor for an
inductive type with zero constructors living in $\mathsf{Prop}$ raises a kernel exception --- catching
$\texttt{False.rec}$, $\texttt{False.elim}$, and $\texttt{False.casesOn}$ at type-check time.
  \item $\texttt{environment.h}$ / $\texttt{environment.cpp}$: Two methods ---
$\texttt{mark\_paraconsistent()}$ and $\texttt{is\_paraconsistent()}$ --- gate the type-checker patch. The flag is a
boolean stored in the environment object and exposed to the Lean surface language via $\texttt{extern "C"}$ FFI.
  \item $\texttt{Init/Paraconsistent.lean}$: User-facing module providing
$\texttt{enableParaconsistent}$ and the Belnap FOUR inductive type.
 \end{itemize}
When $\texttt{is\_paraconsistent()}$ returns false, the kernel is bit-identical to standard Lean 4 v4.28.0. Induction,
recursion, pattern matching on inhabited types, and the entire Mathlib library work identically in both modes. The only
architectural change is the removal of the elimination rule for $\bot$ --- the one rule that makes paraconsistency
impossible in CIC.

 \item \textbf{Lean 4 p4ramill} (167 modules, *sans* sorry). The formalization layer containing:
 \begin{itemize}
  \item $\texttt{Belnap.lean}$: The Belnap FOUR lattice with theorems: $\texttt{no\_explosion}$ ($B \wedge \neg B =
B$, not $\bot$), $\texttt{B\_fixed\_point\_negation}$ ($\neg B = B$), $\texttt{B\_no\_boolean\_complement}$ (there is
no $x$ such that $x \wedge B = \bot$ and $x \vee B = \top$). The $\{\mathbf{T}, \mathbf{F}\}$ Boolean subspace is
proved to be a closed sublattice --- when no contradiction appears in the premises, all deductions stay within
$\{\mathbf{T}, \mathbf{F}\}$ and classical logic holds unchanged.
  \item $\texttt{MajoranaFixed.lean}$: The unification theorem. Three fixed-point structures --- the Belnap $B$
($\neg B = B$), the SIC-POVM fiducial ($B \sqcap x = x$ for all $x$), and the Majorana paired state ($\mu(\delta(s))
= s$) --- are proved identical under $\mu \circ \delta = \text{id}$, all three by $\texttt{rfl}$. The Frobenius
identity holds definitionally, not merely propositionally.
  \item $\texttt{OrbitalBelnap.lean}$: Physical substrate. Orbital occupancy states $\cong$ Belnap FOUR bilattice.
$\texttt{pair\_depair\_id}$ (Frobenius identity, $\texttt{rfl}$); $\texttt{pauli\_exclusion}$ ($B$-ceiling theorem);
$\texttt{ChiralityLE}$ with $\mathbf{N}/\mathbf{B}$ incomparability on the $\text{{\igprimfont ⊥}}$-axis.
  \item $\texttt{GeneticCode.lean}$: Derives the genetic code from the Crystal of Types ($17{,}280{,}000 / 64 =
270{,}000$ exactly). RNA nucleotides map to Belnap values: $G \to B$, $C \to \mathbf{T}$, $A \to \mathbf{F}$, $U \to
\mathbf{N}$. The Wenckebach complement is proved distinct from Belnap negation --- two independent involutions on the
same four-element set.
 \end{itemize}

 \item \textbf{Python p4ramill\_py}: Runtime mirror. Every theorem verified in Lean has a Python equivalent verified
by assertion. The $\texttt{genetics\_b4.py}$ module implements the nucleotide B$_4$ lattice;
$\texttt{gene\_to\_protein\_pipeline.py}$ implements the full gene-to-protein imscription pipeline with 7-stage tuple
generation and Axiom C verification.
\end{enumerate}

\textbf{The containment theorem.} The claim that paraconsistent mathematics contains consistent mathematics
as a restriction follows from the Belnap sublattice theorem: the Boolean subspace $\{\mathbf{T}, \mathbf{F}\}$ is
closed under all Belnap operations (meet, join, band, bor, bnot). Any proof constructed entirely within
$\{\mathbf{T}, \mathbf{F}\}$ --- i.e., any proof that never encounters a contradiction --- is valid in both
paraconsistent and classical mode, and produces identical results. The set of theorems provable in standard Lean 4 is a
proper subset of the set of theorems provable in the p4rakernel, because the p4rakernel additionally admits theorems
about systems that tolerate contradiction (such as the grammar's own self-application at
$\text{{\igfont 𐑣}}$). This is structurally identical to the relationship between Euclidean and non-Euclidean
geometry: removing the parallel postulate expands geometry's native range to include curved spaces, with flat space
recovered as the zero-curvature restriction. Removing the principle of explosion expands mathematics' native range to
include systems that tolerate contradiction, with consistent mathematics recovered when no contradiction appears in the
premises. In both cases, what was taken to be a necessary feature of the discipline is actually a restriction --- and
the restricted version is contained within the unrestricted version, not the other way around.

\textbf{Why the CIC detour is structurally necessary.} The Calculus of Inductive Constructions encodes the principle
of explosion as a kernel primitive, not as a derivable theorem. $\texttt{False.rec} : (C : \mathsf{Sort}\ u) \to \bot
\to C$ is the elimination rule for the empty inductive type, and it is enforced by the C++ type-checker at the
architectural level. Any attempt to define a paraconsistent logic as a conservative extension of CIC --- by working in
a Belnap topos or adding a modal operator --- places the paraconsistent structure at a higher layer of the object
hierarchy, with the kernel's explosion primitive still present beneath it. The moment a proof in the paraconsistent
layer generates a contradiction and that contradiction reaches the kernel, $\texttt{False.rec}$ trivializes the entire
structure. This is the $\text{{\igprimfont <}}$-bottleneck theorem in operational form: $\otimes(\text{{\igfont
𐑹}}, X) = X$ regardless of what $X$ contributes. External coupling cannot force Frobenius closure. The
p4rakernel's modification of the C++ type-checker itself --- the layer at which the rules of what counts as a valid
proof term are decided --- makes the constraint absolute. It is not a theorem proved within a system. It is the
operational fact of a kernel that compiles cleanly in both modes, with the paraconsistent mode being the
architecturally prior one: consistent compilation is the special case where no $\bot$-elimination is attempted.

\textbf{Integration with the Frobenius duality.} The p4rakernel completes the three-route convergence that the
grammar's epistemic position demands. Route 1 is the categorical derivation (\emph{As Above}): 12 primitives from one
category via three operation types. Route 2 is the empirical induction (this paper): 12 primitives from 2,257+
imscribed systems across chemistry, physics, biology, and linguistics. Route 3 is the architectural implementation: 12
primitives enforced at the kernel level, where the environment itself instantiates the grammar's tuple. The three
routes converge at the same crystal address. The Frobenius condition $\mu \circ \delta = \text{id}$ holds not merely as
a theorem about the grammar but as an architectural invariant of the verification environment in which the grammar's
theorems are proved. The $\mu$ half (this paper) classifies; the $\delta$ half (\emph{As Above}) derives; the kernel is
the substrate on which both operations are executed. The p4rakernel is what turning the lock from the inside looks like
at the level where proof terms are born.
...
to a bound in $\pi_j$: it asks whether a type forces a
...
are near-co-typed at $d = 0.068$ (four differences: $\text{{\igprimfont >}}$, $\text{{\igprimfont ⊤}}$, $\text{{\igprimfont ⊙}}$,
$\text{{\igprimfont ⊡}}$); both require $(\Phic, \Ppms)$ and the distinction between
...
conjecture is a non-reachability statement: the smooth type
...
The conjecture is: prove the $d_\to = 0.142$ gap to the
...
Whether those coordinates are the right ones is what over 5,000 systems
...
bound guest, requiring a winding-number dimension ($\text{{\igprimfont ⊡}}$) absent from
...
character ($\text{{\igprimfont ⊤}}$) and chirality/chirality ($\text{{\igprimfont ⊥}}$) had been conflated
...
the initial response was a domain artifact, not an identity.
...
session delaminated into separate $\text{{\igprimfont ⊤}}$ and $\text{{\igprimfont ∋}}$ primitives, and the
`Ordering' axis had not yet separated into $\text{{\igprimfont ⊥}}$ (chirality/temporal
depth) and $\text{{\igprimfont ⊡}}$ (winding number) --- the pre-delamination state the
...
orthogonality --- an independent recovery of the grammar's $\text{{\igprimfont ⊣}}$, $\text{{\igprimfont <}}$,
$\text{{\igprimfont ⊞}}$, $\text{{\igprimfont ⊢}}$, $\text{{\igprimfont ⋈}}$, $\text{{\igprimfont ⊤}}$, $\text{{\igprimfont ∋}}$, and $\text{{\igprimfont >}}$ primitives, arrived at without
...
$\text{{\igprimfont <}}$), a directionality or geometry axis (mapping to $\text{{\igprimfont ⊣}}$ and $\text{{\igprimfont ⊢}}$), a
functional-outcome axis (mapping to $\text{{\igprimfont ⊙}}$), and a kinetic or
condition-sensitivity axis (mapping to $\text{{\igprimfont ⊤}}$). These four dimensions
...
sequential-logic aspects of $\text{{\igprimfont ⊤}}$ and $\text{{\igprimfont ∋}}$ appear as a single
...
$\text{{\igprimfont ⊥}}$/$\text{{\igprimfont ⊡}}$ split. The array therefore replicates the \emph{pre-delamination
...
fidelity primitive $\text{{\igprimfont ⋈}}$ --- no binding energies, no computational
...
\caption{Evolution from the initial chemistry-domain grammar (11 primitives)
  to the final grammar (12 primitives). Initial values are drawn
...
$\text{{\igprimfont ⊥}}$ &
  Absent; fused with $\text{{\igprimfont ⊤}}$ under a ``temporal character" label &
  Chirality: \text{{\igfont 𐑓}}, \text{{\igfont 𐑒}}, \text{{\igfont 𐑖}}, \text{{\igfont 𐑫}} &
  \emph{Delamination.} $\text{{\igprimfont ⊤}}$ encodes the rate of constraint change;
  $\text{{\igprimfont ⊥}}$ encodes the depth of temporal memory.
...
$\text{{\igprimfont ⊙}}$ &
  ...
  5 values: adds \text{{\igfont 𐑮}} and \text{{\igfont 𐑣}} &
  \emph{Diagonalization.} Quantum phase transitions
  (\text{{\igfont 𐑮}}) and non-Hermitian exceptional points
...
topological protection ($\text{{\igprimfont ⊡}}$), kinetic depth ($\text{{\igprimfont ⊤}}$), and temporal
chirality ($\text{{\igprimfont ⊥}}$) were absent or fused. That partial output doesn't
...
this: it fused two primitives the first session had delaminated ($\text{{\igprimfont ⊤}}$ and
$\text{{\igprimfont ∋}}$), demonstrating that the labeling is not fixed. What is fixed is
...
To assign a type to a system, we apply the following
...
\begin{enumerate}[noitemsep]
  \item \textbf{$\text{{\igprimfont ⊢}}$}: Identify the character of the state space.
  ...
  \item \textbf{$\text{{\igprimfont ⊣}}$}: Identify the connectivity topology.
    ...
    imscriptive closure.
  \item \textbf{$\text{{\igprimfont >}}$}: Identify the dominant relational mode.
    ...
    (peer coupling).
  \item \textbf{$\text{{\igprimfont <}}$}: Identify the symmetry class.
  ...
  \item \textbf{$\text{{\igprimfont ⋈}}$}: Identify information fidelity.
  ...
  \item \textbf{$\text{{\igprimfont ⊤}}$}: Identify kinetic character.
    \text{{\igfont 𐑘}} (fast/impulsive); \text{{\igfont 𐑤}} (moderate);
    \text{{\igfont 𐑧}} (slow, Gate-2 pass); \text{{\igfont 𐑪}}
    (monolithic, Gate-2 fail); \text{{\igfont 𐑺}} (glassy (MBL), Gate-2 fail).
  \item \textbf{$\text{{\igprimfont ∈}}$}: Identify the scope/granularity.
  ...
  \item \textbf{$\text{{\igprimfont ∋}}$}: Identify the interaction grammar.
  ...
  \item \textbf{$\text{{\igprimfont ⊙}}$}: Identify criticality.
    \text{{\igfont 𐑢}} (subcritical); $\Phic$ (at the critical point);
    \text{{\igfont 𐑮}} (complex criticality); \text{{\igfont 𐑻}}
    (supercritical); \text{{\igfont 𐑣}} (exceptional
    point, non-Hermitian degeneracy).
  \item \textbf{$\text{{\igprimfont ⊥}}$}: Identify chirality/chirality.
  ...
  \item \textbf{$\text{{\igprimfont ⊞}}$}: Identify stoichiometry.
    \text{{\igfont 𐑙}} ($1{:}1$, one-to-one); \text{{\igfont 𐑕}}
    ($n{:}n$, many-to-many homogeneous); \text{{\igfont 𐑳}}
    ($n{:}m$, many-to-many heterogeneous).
  \item \textbf{$\text{{\igprimfont ⊡}}$}: Identify the winding class.
    \text{{\igfont 𐑷}} (trivial); $\OmegaZtwo$ ($\mathbb{Z}_2$,
    time-reversal-symmetric); $\OmegaZ$ (integer winding);
    \text{{\igfont 𐑟}} (not applicable).
\end{enumerate}
...
\[
  \langle \text{{\igfont 𐑼}} \Tbox \text{{\igfont 𐑾}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \GamSeq \Phic \text{{\igfont 𐑒}} \text{{\igfont 𐑙}} \OmegaZ \rangle
\]
...
primitive. In practice, three primitives --- $\text{{\igprimfont ⊙}}$, $\text{{\igprimfont <}}$, and $\text{{\igprimfont ⊤}}$
...
(\Cref{def:bottleneck}) requires that $\text{{\igprimfont <}}(X \otimes Y) =
\min(\text{{\igprimfont <}}_X, \text{{\igprimfont <}}_Y)$ and $\text{{\igprimfont ⋈}}(X \otimes Y) = \min(\text{{\igprimfont ⋈}}_X, \text{{\igprimfont ⋈}}_Y)$. Any
proposed imscription that would require a composite system to have
higher $\text{{\igprimfont <}}$ or $\text{{\igprimfont ⋈}}$ than its weakest component is internally
...
is a finding about the system's type. The Hv1 channel
...
$(\text{{\igprimfont ⊤}},\, \text{{\igprimfont ∈}},\, \text{{\igprimfont ⊣}},\, \text{{\igprimfont ⊡}})$ are derived by variance decomposition
restricted to the critical manifold $\text{{\igprimfont ⊙}} = \Phic$ across the
...
\begin{enumerate}
  \item \textbf{$\text{{\igprimfont ⊢}} = \text{{\igfont 𐑨}}$}: The Liar operates in a finite,
    triangulated semantic space: statement $\to$ truth value $\to$
    evaluation of that value.
  \item \textbf{$\text{{\igprimfont ⊣}} = \Tbw$}: Bowtie topology captures the
    self-referential crossing where subject meets predicate in a loop.
  \item \textbf{$\text{{\igprimfont >}} = \text{{\igfont 𐑽}}$}: Mutual implication between
    provability and truth --- the Liar is provably unclassifiable in
    bivalent logic.
  \item \textbf{$\text{{\igprimfont <}} = \Ppms$}: The defining condition. The Liar requires
  ...
  \item \textbf{$\text{{\igprimfont ⋈}} = \text{{\igfont 𐑐}}$}: Model-relative truth --- the Liar is
    neither provably true nor provably false within any consistent
    bivalent system.
  \item \textbf{$\text{{\igprimfont ⊤}} = \text{{\igfont 𐑧}}$}: The evaluation loop requires slow
    integration; the Liar cannot be resolved in a single step.
  \item \textbf{$\text{{\igprimfont ∈}} = \text{{\igfont 𐑲}}$}: Absolute scope --- the Liar's truth
    value is independent of any external reference frame.
  \item \textbf{$\text{{\igprimfont ∋}} = \text{{\igfont 𐑠}}$}: Sequential logic: statement
  ...
  \item \textbf{$\text{{\igprimfont ⊥}} = \text{{\igfont 𐑖}}$}: Two-level chirality: the statement
    evaluates itself.
  \item \textbf{$\text{{\igprimfont ⊞}} = \text{{\igfont 𐑙}}$}: One-to-one correspondence between
    statement and truth value.
  \item \textbf{$\text{{\igprimfont ⊡}} = \text{{\igfont 𐑴}}$}: $\mathbb{Z}_2$ winding
...
\begin{equation}
  \xvec_\text{Liar} = \langle \text{{\igfont 𐑨}} \Tbw \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \text{{\igfont 𐑠}} \Phic \text{{\igfont 𐑖}} \text{{\igfont 𐑙}} \text{{\igfont 𐑴}} \rangle
  \label{eq:liar_full}
\end{equation}
with $\text{{\igprimfont <}} = \Ppms$ and ouroboricity tier $\Oinf$ (R1). The assignment of
...
If the Liar imscribed at any $\text{{\igprimfont <}} < \Ppms$, the tensor product
...
is in $\text{{\igprimfont ⊥}}$ (unlimited meta-logical depth) and $\text{{\igprimfont ⊞}}$ (many axioms, many
theorems):

\begin{equation}
  \xvec_\text{G\"{o}del} = \langle \text{{\igfont 𐑨}} \text{{\igfont 𐑶}} \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \text{{\igfont 𐑠}} \Phic \text{{\igfont 𐑫}} \text{{\igfont 𐑳}} \text{{\igfont 𐑴}} \rangle
\end{equation}

$\text{{\igprimfont ⊣}} = \text{{\igfont 𐑶}}$ (crossed product) rather than $\Tbw$ because the
...
not merely looped. $\text{{\igprimfont ⊥}} = \text{{\igfont 𐑫}}$ reflects that the meta-logical depth
is unbounded: any consistent extension $T'$ of $T$ generates its own
G\"{o}del sentence $G'$. $\text{{\igprimfont <}} = \Ppms$ for the same reason as
...
\begin{equation}
  \xvec_{\glp} = \langle \text{{\igfont 𐑨}} \text{{\igfont 𐑶}} \text{{\igfont 𐑽}} \Ppms \text{{\igfont 𐑐}} \text{{\igfont 𐑧}} \text{{\igfont 𐑲}} \text{{\igfont 𐑠}} \Phic \text{{\igfont 𐑖}} \text{{\igfont 𐑙}} \text{{\igfont 𐑴}} \rangle
\end{equation}

$\xvec_{\glp}$ is structurally identical to $\xvec_\text{G\"{o}del}$
up to $\text{{\igprimfont ⊥}}$ (LP itself operates at \text{{\igfont 𐑖}} --- two-level truth assignment
--- while the G\"{o}del argument requires \text{{\igfont 𐑫}}) and $\text{{\igprimfont ⊞}}$
...
The distance between them is $d(\xvec_{\glp},
\xvec_\text{G\"{o}del}) = 2/12$, reflecting the two substrate
differences in $\text{{\igprimfont ⊥}}$ and $\text{{\igprimfont ⊞}}$ only.

\subsection{Software and data availability}

The full grammar implementation, the open-source catalog
...
available at \href{https://github.com/umpolungfish/ALEPH_OS}{\texttt{ALEP{\igprimfont ⊥}\_OS}}.
...
\bibitem{juster1961}
N.~Juster, \emph{The Phantom Tollbooth}. Random House, New York, 1961.

\bibitem{kim2001}
Y.-M.~Jeon, J.~Kim, D.~Whang, K.~Kim,
Molecular container assembly capable of controlling binding and release of its
guest molecules: reversible encapsulation of organic molecules in sodium ion
complexed cucurbituril.
\emph{J. Am. Chem. Soc.} \textbf{118}, 9790--9791 (1996).
\url{https://doi.org/10.1021/ja962071x}

\bibitem{assaf2015}
K.~I.~Assaf, W.~M.~Nau,
Cucurbiturils: from synthesis to high-affinity binding and catalysis.
\emph{Chem. Soc. Rev.} \textbf{44}, 394--418 (2015).
\url{https://doi.org/10.1039/C4CS00273C}

\bibitem{abramsky2004}
S.~Abramsky, B.~Coecke,
A categorical semantics of quantum protocols.
\emph{Proc. 19th IEEE Symp. Logic in Computer Science (LICS)}, 415--425 (2004).
\url{https://doi.org/10.1109/LICS.2004.1}

\bibitem{atiyah1988}
M.~F.~Atiyah,
Topological quantum field theory.
\emph{Publ. Math. Inst. Hautes \'Etudes Sci.} \textbf{68}, 175--186 (1988).
\url{https://doi.org/10.1007/BF02698547}

\bibitem{tononi2004}
G.~Tononi,
An information integration theory of consciousness.
\emph{BMC Neurosci.} \textbf{5}, 42 (2004).
\url{https://doi.org/10.1186/1471-2202-5-42}

\bibitem{tononi2016}
G.~Tononi, M.~Boly, M.~Massimini, C.~Koch,
Integrated information theory: from consciousness to its physical substrate.
\emph{Nat. Rev. Neurosci.} \textbf{17}, 450--461 (2016).
\url{https://doi.org/10.1038/nrn.2016.44}

\bibitem{afmy2017}
M.~Appleby, S.~Flammia, G.~McConnell, J.~Yard,
Generating ray class fields of real quadratic fields via complex equiangular lines.
\emph{Acta Arith.} \textbf{180}, 331--372 (2017).
\url{https://doi.org/10.4064/aa8393-11-2016}

\bibitem{larson1961}
H.~T.~Larson,
Catch a rising problem\ldots and never ever let it go.
\emph{IRE Trans. Eng. Manag.} \textbf{EM-8}, 173--174 (1961).
