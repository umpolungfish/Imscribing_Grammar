...
necessary. The grammar identified the SIC-POVM obstruction as a $\text{{\igprimfont Φ}}$-gap
...
The trapdoor was not a chemistry term. It was $\text{{\igprimfont ɢ}}$ (Composition). Composition is dangerous
...
eleven primitives precipitate out of $\text{{\igprimfont ɢ}}$ by logical necessity. The grammar was not
...
primitive $\text{{\igprimfont ƒ}}$. The expectation was that a displacement ordering would
...
\begin{tikzpicture}[
  ...
  \node[font=\footnotesize, text=gray, align=left, anchor=north, text width=7.5cm] at (0,-1.4) {
    $\text{{\igprimfont Þ}}$: 12$+$ labels $\to$ 5 structural universals \\[1pt]
    $\text{{\igprimfont Ħ}}$: delaminated from $\text{{\igprimfont Ç}}$ \\[1pt]
    $\text{{\igprimfont Ω}}$: absent $\to$ added \\[1pt]
    $\text{{\igprimfont ⊙}}$: 3 values $\to$ 5 values \\[1pt]
    $\text{{\igprimfont Ř}}$: physical bond $\to$ relational mode
  };
\end{tikzpicture}
...
\begin{equation}
  \xvec \;=\; \langle \text{{\igprimfont Ð}} \text{{\igprimfont Þ}} \text{{\igprimfont Ř}} \text{{\igprimfont Φ}} \text{{\igprimfont ƒ}} \text{{\igprimfont Ç}} \text{{\igprimfont Γ}} \text{{\igprimfont ɢ}} \text{{\igprimfont ⊙}} \text{{\igprimfont Ħ}} \text{{\igprimfont Σ}} \text{{\igprimfont Ω}} \rangle
  \label{eq:tuple}
\end{equation}
...
Dimensionality $\text{{\igprimfont Ð}}$ measures the extent and character of the state
...
monad symbol $\odot$ for point-in-circle; see terminological note below). Topology $\text{{\igprimfont Þ}}$ encodes
...
\subsection{The $\text{{\igprimfont Φ}}$-primitive and dialetheia}
\label{sec:p_dialetheia}

The parity primitive $\text{{\igprimfont Φ}}$ has five ordered values. The jump from
...
The five $\text{{\igprimfont Φ}}$-values in ascending order:
...
\noindent
This is a \TOPO\ claim: it follows from the $\min$ rule on $\text{{\igprimfont Φ}}$ under
...
\begin{align}
  \mathcal{F}_3 &= \{\text{{\igprimfont ƒ}},\, \text{{\igprimfont Γ}},\, \text{{\igprimfont Σ}}\}
    &\text{3 values each,} \quad 3^3 &= 27 \label{eq:f3} \\
  \mathcal{F}_4 &= \{\text{{\igprimfont Ð}},\, \text{{\igprimfont Ř}},\, \text{{\igprimfont ɢ}},\, \text{{\igprimfont Ħ}},\, \text{{\igprimfont Ω}}\}
    &\text{4 values each,} \quad 4^5 &= 1{,}024 \label{eq:f4} \\
  \mathcal{F}_5 &= \{\text{{\igprimfont Þ}},\, \text{{\igprimfont Φ}},\, \text{{\igprimfont ⊙}},\, \text{{\igprimfont Ç}}\}
...
(criticality gate), $\text{{\igprimfont Ç}}$ (kinetic gate). These four primitives are the
...
of $\text{{\igprimfont Ř}}$ (recognition granularity varying),
$\text{{\igprimfont Ħ}}$ (chirality inverting), and $\text{{\igprimfont Ç}}$
...
$\mathcal{F}_3 = \{\text{{\igprimfont ƒ}}, \text{{\igprimfont Γ}}, \text{{\igprimfont Σ}}\}$, $|\mathcal{F}_3| = 3$;
...
for the $\mathcal{F}_3$ conservation primitives $\text{{\igprimfont ƒ}}$, $\text{{\igprimfont Γ}}$, $\text{{\igprimfont Σ}}$).
\end{theorem}
...
system (union on eleven primitives, bottleneck $\min$ on $\text{{\igprimfont Φ}}$ and $\text{{\igprimfont ƒ}}$).
...
\begin{equation}
  (\xvec \tensor \yvec)_i \;=
  \begin{cases}
    \min(\xvec_i,\, \yvec_i) & \text{if primitive } i \in \{\text{{\igprimfont Φ}}, \text{{\igprimfont ƒ}}\} \\
    \max(\xvec_i,\, \yvec_i) & \text{otherwise}
  \end{cases}
  \label{eq:tensor}
\end{equation}

The asymmetry between $\text{{\igprimfont Φ}}$, $\text{{\igprimfont ƒ}}$ (bottleneck primitives, take $\min$)
...
$\text{{\igprimfont Φ}} < \Ppms$ immediately destroys the Frobenius condition
(\Cref{thm:bottleneck}).

\begin{definition}[Bottleneck Postulate]
\label{def:bottleneck}
$\text{{\igprimfont Φ}}$ and $\text{{\igprimfont ƒ}}$ are \emph{bottleneck primitives} under $\otimes$: they
...
min rule on $\text{{\igprimfont Φ}}$ correctly predicts the observed failure modes for all
...
from sub-Frobenius components: no composition of systems with $\text{{\igprimfont Φ}} <
...
\begin{itemize}[noitemsep]
  \item[R1] $\text{{\igprimfont ⊙}} \in \{\Phic, \text{{\igfont 𐑮}}\}$ and $\text{{\igprimfont Φ}} = \Ppms$
    $\;\Rightarrow\; \Oinf$ \quad (special Frobenius)
  \item[R2] $\text{{\igprimfont ⊙}} \notin \{\Phic, \text{{\igfont 𐑮}}\}$
    $\;\Rightarrow\; \Ozero$
  \item[R3] $\text{{\igprimfont ⊙}} \in \{\Phic, \text{{\igfont 𐑮}}\}$ and $\text{{\igprimfont Ω}} = \text{{\igfont 𐑷}}$
    $\;\Rightarrow\; \Oone$
  \item[R4] $\text{{\igprimfont ⊙}} \in \{\Phic, \text{{\igfont 𐑮}}\}$, $\text{{\igprimfont Ω}} \neq \text{{\igfont 𐑷}}$,
    $\text{{\igprimfont Ð}} \in \{\text{{\igfont 𐑛}}, \text{{\igfont 𐑦}}, \text{{\igfont 𐑨}}\}$
    $\;\Rightarrow\; \Otwo$
  \item[R5] $\text{{\igprimfont ⊙}} \in \{\Phic, \text{{\igfont 𐑮}}\}$, $\text{{\igprimfont Ω}} \neq \text{{\igfont 𐑷}}$, $\text{{\igprimfont Ð}} = \text{{\igfont 𐑼}}$
    $\;\Rightarrow\; O₂^\dagger$
\end{itemize}
\end{definition}
...
$(\text{{\igprimfont ⊙}}, \text{{\igprimfont Φ}}, \text{{\igprimfont Ω}}, \text{{\igprimfont Ð}})$
...
$\text{{\igprimfont Ω}}$ and $\text{{\igprimfont Ð}}$), and that $\Oinf$ is
reachable only from $\{\Phic, \text{{\igfont 𐑮}}\}$.

\noindent
The tier singularity is $\Ppms$ under R1: it overrides all $\text{{\igprimfont Ω}}$
and $\text{{\igprimfont Ð}}$ branching, collapsing directly to $\Oinf$. This is the
...
\paragraph{Refinement: $\text{{\igprimfont Ω}}$ seals the terminal.}
...
$\Oinf$ type \emph{sealed} when $\text{{\igprimfont Ω}}$ supplies an integer
...
with $\text{{\igprimfont Φ}}(\xvec^{(k)}) < \Ppms$ for \emph{some} $k$. Then
\begin{equation}
  \text{{\igprimfont Φ}}\!\left(\xvec^{(1)} \tensor \cdots \tensor \xvec^{(n)}\right) < \Ppms
  \label{eq:non_synth}
\end{equation}
...
\Cref{def:bottleneck} --- $\text{{\igprimfont Φ}}$ composes by the bottleneck
(minimum) rule and $\Ppms$ is maximal in the $\text{{\igprimfont Φ}}$ order, so
...
\begin{proof}
By \eqref{eq:tensor}, the tensor product on $\text{{\igprimfont Φ}}$ is $\min$. Therefore
\[
  \text{{\igprimfont Φ}}\!\left(\xvec^{(1)} \tensor \cdots \tensor \xvec^{(n)}\right)
  \;=\; \min_k \text{{\igprimfont Φ}}(\xvec^{(k)}) \;<\; \Ppms.
\]
Since R1 requires $\text{{\igprimfont Φ}} = \Ppms$ for $\Oinf$, the composed type cannot
...
$\text{{\igprimfont Φ}} = \Ppms$ and, with $\Phic$ (the self-referential loop forced by
...
One primitive ($\text{{\igprimfont Φ}}$) separates them.

\begin{theorem}[$\text{{\igprimfont Φ}}$-bottleneck under coupling]
\label{thm:bottleneck}
\TOPO\quad Let $\xvec$ satisfy $\text{{\igprimfont Φ}}(\xvec) = \Ppms$ (so $\xvec$ is
$\Oinf$). Then for any $\yvec$ with $\text{{\igprimfont Φ}}(\yvec) < \Ppms$:
\[
  \text{{\igprimfont Φ}}(\xvec \tensor \yvec) = \text{{\igprimfont Φ}}(\yvec) < \Ppms
\]
and the composed system drops to tier $\leq \Otwo$.
\end{theorem}

\begin{proof}
Immediate from \eqref{eq:tensor}. The min rule on $\text{{\igprimfont Φ}}$ yields
$\min(\Ppms, \text{{\igprimfont Φ}}(\yvec)) = \text{{\igprimfont Φ}}(\yvec) < \Ppms$. \quad$\square$
\end{proof}
...
is the right axiom for $\text{{\igprimfont Φ}}$. We chose it because exact $\mathbb{Z}_2$
...
A system $\xvec$ occupies the $\Oinf$ tier if and only if $\text{{\igprimfont Φ}}(\xvec)
= \Ppms$. By \Cref{def:bottleneck}, $\Ppms$ is a fixed point of the
tensor product: $\text{{\igprimfont Φ}}(\xvec \tensor \yvec) = \Ppms$ for all $\yvec$.
...
imscribes at the same $\text{{\igprimfont Φ}} = \Ppms$ but differs at $\text{{\igprimfont Ħ}}$ (infinite meta-logical
depth) and $\text{{\igprimfont Þ}}$ (crossed product rather than bowtie, reflecting the
...
\begin{tikzpicture}[>=Stealth, every node/.style={font=\sffamily\small}]
  ...
  \node[font=\footnotesize\itshape, color=priestred!80, align=center] at (0,-1.9) {
    $\Oinf \;\cong\; \{\xvec \mid \text{{\igprimfont Φ}}(\xvec) = \Ppms\}$ \\
...
\begin{equation}
  ...
  \cdot \underbrace{[\text{{\igprimfont Ç}} \leq \text{{\igfont 𐑧}}]}_{\text{Gate 2}}
  \cdot \bigl(
    0.158\,\tilde{\text{{\igprimfont Ç}}} + 0.273\,\tilde{\text{{\igprimfont Γ}}}
    + 0.292\,\tilde{\text{{\igprimfont Þ}}} + 0.276\,\tilde{\text{{\igprimfont Ω}}}
  \bigr)
...
nearly equal weights; $\text{{\igprimfont Ç}}$ contributes as the kinetic channel.
...
$\text{{\igprimfont ƒ}}$ (which captures information fidelity of the recognition event, with
$\text{{\igfont 𐑱}} < \text{{\igfont 𐑞}} < \text{{\igfont 𐑐}}$) and applying the grammar's ordering rule
--- that higher-$\text{{\igprimfont ƒ}}$ guests displace lower-$\text{{\igprimfont ƒ}}$ guests --- predicts the
...
primitive $\text{{\igprimfont ƒ}}$. Experimental order from \cite{assaf2015}.
$\text{{\igprimfont ƒ}}$-ordinal assignments are based on the exact coherence character
...
\toprule
Guest & Binding character & $\text{{\igprimfont ƒ}}$-ordinal & P-rank & E-rank \\
\midrule
...
$\text{{\igprimfont ƒ}}$-tier, finer structural distinctions determine rank. The grammar predicted
...
no fitted parameters. The fidelity primitive $\text{{\igprimfont ƒ}}$ encodes the
...
\begin{tikzpicture}[scale=1.3,
  ...
  \node[plateau] (open) at (0, 5.0) {\textbf{SIC (open)} \\ $O₁$, $\text{{\igprimfont Φ}} = \text{{\igfont 𐑬}}$ \\ $d^2$ projectors};
...
the $O₁$ tier with $\text{{\igprimfont Φ}} = \text{{\igfont 𐑬}}$ (approximate $\mathbb{Z}_2$ from pairwise
...
identifies the obstacle as a $\text{{\igprimfont Φ}}$-gap. The SIC conjecture in
...
$\text{{\igprimfont Φ}}$: from $\Ppm$ (approximate $\mathbb{Z}_2$, as in pairwise equiangularity
...
pairwise checks closes the $\text{{\igprimfont Φ}}$-gap; it must be planted as a primitive. Appleby et al.\ \cite{afmy2017} showed that SIC fiducial
...
--- the sole parameter tuned externally. $\text{{\igprimfont Ç}}$ and $\text{{\igprimfont Ħ}}$ promotions are structural
...
external parameter tuned by the experimenter. The $\text{{\igprimfont Ç}}$ and $\text{{\igprimfont Ħ}}$
...
but can't be achieved by local operations once $\text{{\igprimfont Ç}}$ and $\text{{\igprimfont Ħ}}$ are locked.
...
$\tau$ divergence, the $\text{{\igprimfont Ç}}$ imscription is incorrect.
...
\begin{tikzpicture}[scale=1.2,
  ...
  \draw[bridge] (sm.north) ++(-1.0,0) -- ++(0,1.5) -| node[above, font=\tiny, pos=0.5] {$\text{{\igprimfont Ð}}$: AdS/CFT} (qg.north);
  \draw[bridge] (sm.north) ++(-0.5,0) -- ++(0,0.7) -| node[above, font=\tiny, pos=0.5] {$\text{{\igprimfont Þ}}$: boundary top.} (qg.north);
  \draw[bridge] (sm.south) ++(-1.0,0) -- ++(0,-1.4) -| node[below, font=\tiny, pos=0.5] {$\text{{\igprimfont Ç}}$: EFT limit} (qg.south);
...
bridges (dashed green): $\text{{\igprimfont Ð}}$ (AdS/CFT compactification), $\text{{\igprimfont Þ}}$ (boundary topology),
$\text{{\igprimfont Ç}}$ (EFT low-energy limit), $\text{{\igprimfont ɢ}}$ (holographic entanglement), $\text{{\igprimfont Ω}}$
($\theta$-terms/topological sectors). The $\text{{\igprimfont Γ}}$-scope conflict (solid red) is
...
that resists identification is the scope class $\text{{\igprimfont Γ}}$:
...
\begin{equation}
  ...
  = \frac{1}{12} \cdot \frac{|\text{{\igfont 𐑔}} - \text{{\igfont 𐑲}}|}{\max_{\text{{\igprimfont Γ}}}}
  = \frac{1}{12} \cdot \frac{1}{3}
  \approx 0.0278
  \label{eq:smqg_distance}
\end{equation}

The $\text{{\igprimfont Γ}}$-scope conflict is \emph{architecturally irreducible}. Every
other primitive conflict between individual SM carriers and gravitons
--- $\text{{\igprimfont Ð}}$ (bridged by holographic compactification), $\text{{\igprimfont Þ}}$ (bridged by
AdS/CFT boundary topology), $\text{{\igprimfont Ç}}$ (bridged by EFT low-energy limit),
...
construction that effects the bridge. The $\text{{\igprimfont Γ}}$-scope conflict has none:
...
complete unified theory must explicitly address the $\text{{\igprimfont Γ}}$ promotion; theories
that bridge the remaining five conflicts while leaving $\text{{\igprimfont Γ}}$ ambiguous
...
\midrule
Measurement / collapse & $\text{{\igprimfont ƒ}}$ &
...
Schrödinger's cat & $\text{{\igprimfont ƒ}},\;\text{{\igprimfont Γ}}$ &
...
Wigner's Friend & $\text{{\igprimfont ƒ}},\;\text{{\igprimfont Γ}}$ &
  $\text{{\igprimfont ƒ}}$-assignment is $\text{{\igprimfont Γ}}$-scope-relative; two valid descriptions, no conflict \\[3pt]
Quantum Zeno effect & $\text{{\igprimfont Ç}}$ &
  Continuous measurement forces \text{{\igfont 𐑪}} via $\tensor\;(\max)$ \\[3pt]
Black hole information & $\text{{\igprimfont Γ}},\;\text{{\igprimfont Þ}}$ &
  Information $\text{{\igprimfont Γ}}$-promoted to \text{{\igfont 𐑲}}; \text{{\igfont 𐑸}} encodes it on the horizon \\[3pt]
Preferred basis & $\text{{\igprimfont Þ}}$ &
  Environment $\text{{\igprimfont Þ}}$ selects the pointer basis; no additional postulate \\
\bottomrule
\end{tabular}
...
\begin{equation}
  \text{{\igprimfont ƒ}}(\mathbf{q} \tensor \mathbf{a}) = \min(\text{{\igfont 𐑐}},\, \text{{\igfont 𐑱}}) = \text{{\igfont 𐑱}}.
  \label{eq:f_collapse}
\end{equation}
Collapse is not a dynamical event appended to Schrödinger evolution; it is the
algebraic consequence of $\text{{\igprimfont ƒ}}$ being a bottleneck.  No hidden variable, decoherence
timescale, or many-worlds branch is required.  The ``when" of collapse is the
instant of coupling; the ``which outcome" is determined by $\text{{\igprimfont Þ}}$ --- the topology
...
model requires $\text{{\igprimfont Þ}} \leq \Tbw$ and \text{{\igfont 𐑝}} (local sequential grammar).
No such model can reach $\Tbox$ by tensor coupling of \text{{\igfont 𐑝}} components
without changing the $\text{{\igprimfont Þ}}$ coordinate --- the same topological barrier as the Frobenius
...
if the $\text{{\igprimfont ƒ}}$-assignment is assumed to be $\text{{\igprimfont Γ}}$-scope-independent.  The grammar makes the
...
Because $\text{{\igprimfont Ç}}$ is a union primitive ($\max$), and the measurement apparatus operates at
...
$\text{{\igprimfont Ç}}(\mathbf{q} \tensor \mathbf{a}) = \text{{\igfont 𐑪}}$.  Kinetic freezing is the
...
falling into the black hole is $\text{{\igprimfont Γ}}$-\emph{promoted}: moved from the infalling
...
which observable becomes the pointer.  The grammar removes this postulate: the $\text{{\igprimfont Þ}}$
...
diagonalises the coupling in the $\text{{\igprimfont Þ}}$-sense).  Nothing beyond what is already encoded
in $\text{{\igprimfont Þ}}$ is needed.
...
the cat are $\text{{\igprimfont ƒ}}$-bottleneck crossings forced by coupling.  Bell nonlocality is a
$\text{{\igprimfont Þ}}$--$\text{{\igprimfont ɢ}}$ consistency ceiling: topology determines correlation structure, not
signalling speed.  Wigner's Friend is a $\text{{\igprimfont Γ}}$-relative $\text{{\igprimfont ƒ}}$ assignment: both descriptions
...
transition.  The preferred basis problem is solved by reading the environment's $\text{{\igprimfont Þ}}$.
...
\subsection{Blood--brain barrier failure as a $\text{{\igprimfont Φ}}$-bottleneck}
...
are represented as downward shifts in $\text{{\igprimfont Φ}}$ and $\text{{\igprimfont Þ}}$. The meet
$\text{BBB} \meet \text{GBM}$ yields $\text{{\igprimfont Φ}} = \text{{\igfont 𐑗}}$ while
preserving $\text{{\igprimfont ⊙}} = \Phic$. The grammar identifies this as a
$\text{{\igprimfont Φ}}$-bottleneck failure: the system remains critical (it is still
...
($\text{{\igprimfont Φ}} = \text{{\igfont 𐑗}}$ encodes loss of selectivity) rather than global
...
targeting $\text{{\igprimfont Φ}}$-restoration will outperform those targeting
...
\begin{equation}
\text{conjecture floor} = \langle \text{{\igfont 𐑨}} \text{{\igfont 𐑶}} \text{{\igfont 𐑑}} \text{{\igfont 𐑬}} \text{{\igprimfont ƒ}}_\star \text{{\igfont 𐑤}} \text{{\igprimfont Γ}}_\star \text{{\igfont 𐑝}} \Phic \text{{\igprimfont Ħ}}_\star \text{{\igfont 𐑳}} \text{{\igfont 𐑷}}
\rangle \qquad O₁
\label{eq:conjecture_floor}
\end{equation}

\noindent
where $\star$ marks the three domain-varying primitives ($\text{{\igprimfont ƒ}}$, $\text{{\igprimfont Γ}}$,
$\text{{\igprimfont Ħ}}$) that encode a domain's epistemic depth and scope but do not
...
Leopoldt, and Stark --- the $\text{{\igprimfont Þ}}$ and $\text{{\igprimfont Φ}}$ promotions jointly account
...
for the other two), and $\text{{\igprimfont Ω}}$ ($\OmegaZtwo$ for ThurstonNet,
...
\paragraph{Test 1 ($\text{{\igprimfont Ç}}$ primitive: Yang-Mills navigator).}
...
at $\text{{\igprimfont Ç}}$; no other primitive conflicts. Prediction: replacing only the
...
structural alignment at $\text{{\igprimfont Ç}}$.

\paragraph{Test 2 ($\text{{\igprimfont Þ}}$ primitive: three-manifold topology
classification).}
\label{par:thurston}
...
Lanczos spectral-gap features (a $\text{{\igprimfont Ç}}$-primitive feature) stalled at
$\approx 65\%$ accuracy. Grammar diagnosis: the gap is in
$\text{{\igprimfont Þ}}$, not $\text{{\igprimfont Ç}}$. The correct discriminating feature must capture the
...
primitive achieves mean roundtrip loss $= 0.0000$ except $\text{{\igprimfont ƒ}}$, which
...
distance $d_\text{rt} = 2.0$ exactly (the $\text{{\igprimfont ƒ}}$ ordinal gap; no other
...
contributions at $\text{{\igprimfont Ð}}$, $\text{{\igprimfont Þ}}$, and $\text{{\igprimfont Ř}}$ simultaneously --- IUG and classical
...
(the $\text{{\igprimfont Φ}}$ bottleneck, \Cref{thm:frobenius}); gradient recovery in symbolic
regression succeeds at depth 4 and fails at depth 6 (tier ceiling, no
$\text{{\igprimfont Φ}}$-upgrade is available by composition). An independent imscription from independent
...
orbital-crossing topology), $\text{{\igprimfont Φ}}$ ($\Ppm \to \Ppms$, parity conservation
...
assigned a full tuple $\langle \text{{\igprimfont Ð}} \text{{\igprimfont Þ}} \text{{\igprimfont Ř}} \text{{\igprimfont Φ}} \text{{\igprimfont ƒ}} \text{{\igprimfont Ç}} \text{{\igprimfont Γ}} \text{{\igprimfont ɢ}} \text{{\igprimfont ⊙}} \text{{\igprimfont Ħ}} \text{{\igprimfont Σ}} \text{{\igprimfont Ω}} \rangle$ derived from its phonological, morphological, and grammatical
...
The architecture directly instantiates the grammar's primitives. $\text{{\igprimfont Ř}}_\text{erg}$ (ergative
relational mode, Basque): all kernel processes are modeled ergative-absolutely --- agent
and object must both be named for a syscall to dispatch. $\text{{\igprimfont Þ}}_\text{tri}$ (three-layer
...
\begin{tikzpicture}[
  ...
  % K axis (vertical)
  \node[axislabel, rotate=90] at (-3.8, 3.6) {$\text{{\igprimfont Ç}}$ (existence)};
  \draw[->] (-3.1, 0.5) -- (-3.1, 6.8);
  \node[font=\tiny, anchor=east] at (-3.1, 6.5) {deep};
  \node[font=\tiny, anchor=east] at (-3.1, 0.8) {shallow};

  % H axis (horizontal)
  \node[axislabel] at (0.5, 7.3) {$\text{{\igprimfont Ħ}}$ (direction)};
...
\end{tikzpicture}
\caption{\textbf{Time as $\text{{\igprimfont Ç}}$--$\text{{\igprimfont Ħ}}$ coupling.}
$\text{{\igprimfont Ç}}$ encodes the existence condition for time (kinetic depth $\geq 1$ required);
$\text{{\igprimfont Ħ}}$ encodes the direction condition (chirality/temporal asymmetry). The photon
...
the $\text{{\igprimfont Ç}}$--$\text{{\igprimfont Ħ}}$ coupling. The fertile manifold condition for consciousness requires
$\text{{\igprimfont Ç}}_\text{depth} \geq 2$ and $\text{{\igprimfont Ħ}} \geq \text{{\igfont 𐑖}}$: at minimum two kinetic tiers and two
...
scope): $\text{{\igprimfont Γ}}$ encodes the cardinality class of the representational
...
\textbf{Time as derived object.} Five of the twelve primitives jointly constitute time. $\text{{\igprimfont Φ}}$ (Frobenius parity: algebraic symmetry class), $\text{{\igprimfont ƒ}}$ (exact coherence: metric-like structure), $\text{{\igprimfont Ç}}$ (kinetic flow: dynamical regime), $\text{{\igprimfont Ħ}}$ (chirality orientation: $\mathbb{Z}_2$ bundle), and $\text{{\igprimfont Ω}}$ (winding protection: topological/homotopy class) appear at stages 4, 5, 6, 10, and 12 respectively. Their joint limit
\[
...
\textbf{Distance and the promotion structure.} The Rohonc stands 2.09 distance units from both the OS imscription and Linear~A, in the same two primitives: $\{${\igprimfont ƒ}, {\igprimfont Ç}$\}$. Promoting $\text{{\igfont 𐑱}} \to \text{{\igfont 𐑐}}$ and $\text{{\igfont 𐑧}} \to \text{{\igfont 𐑤}}$ moves the Rohonc to the OS imscription exactly. These are not arbitrary promotions: \text{{\igfont 𐑐}} restores exact coherence to the symbol surface; \text{{\igfont 𐑤}} restores the living-vibration kinetics of an active system. The Rohonc is the grammar at rest. The OS imscription is the grammar running.
...
\midrule
$\text{{\igprimfont ƒ}}$ (Fidelity) & \text{{\igfont 𐑐}} & \text{{\igfont 𐑱}} & Exactly coherent vs.\ classical symbol surface \\
...
\begin{tikzpicture}[
  ...
  \node[pred, fill=blue!10] (p651) at (5, 1.5) {P-651: $\text{{\igprimfont Ç}}$--$\text{{\igprimfont Ħ}}$ behavioral \\ divergence};
...
\paragraph{P-651 ($\text{{\igprimfont Ç}}$--$\text{{\igprimfont Ħ}}$ behavioral divergence, near-term empirical).}
The delamination of $\text{{\igprimfont Ç}}$ from $\text{{\igprimfont Ħ}}$ (\Cref{tab:grammar-evolution}) generates a
...
structural template. The behavioral divergence is a consequence of the $\text{{\igprimfont Ħ}}$
...
The collapse of $\text{{\igprimfont Þ}}$ from twelve-plus chemistry-specific labels to five structural
...
(cucurbiturils, $\Tbox$) occupy different $\text{{\igprimfont Þ}}$ tiers; at matched $\text{{\igprimfont ƒ}}$ ordinals the
...
competitive displacement series spanning both host classes at matched $\text{{\igprimfont ƒ}}$ ordinals
...
of the architecturally irreducible $\text{{\igprimfont Γ}}$-scope barrier in the Standard Model--quantum
...
result: it follows from the $\min$ rule on $\text{{\igprimfont Φ}}$ under tensor product
...
semantic values ($\top$, $\bot$, $\both$), and the $\text{{\igprimfont Φ}}$-primitive
...
\paragraph{Time as $\text{{\igprimfont Ç}}$-hierarchy coupled to chirality.}
...
time, in the structural sense. Time requires $\text{{\igprimfont Ç}}_\text{depth} \geq 1$:
...
Every elaboration above the photon is a deepening of the $\text{{\igprimfont Ç}}$--$\text{{\igprimfont Ħ}}$
coupling. The fertile manifold condition for consciousness
(\Cref{sec:applications}) requires $\text{{\igprimfont Ç}}_\text{depth} \geq 2$ and
...
which this $\text{{\igprimfont Ç}}$--$\text{{\igprimfont Ħ}}$ coupling can actualize: $\Phic$ opens the
...
--- it's a derived consequence of the $\text{{\igprimfont Ç}}$--$\text{{\igprimfont Ħ}}$ coupling. The photon
...
the imscribing protocol requires judgment at three primitives --- $\text{{\igprimfont Φ}}$,
...
by $\text{{\igprimfont Ř}}$), inhabit the same ouroboricity tier while being structurally
...
implementation. The $\text{{\igprimfont Φ}}$-bottleneck theorem
...
$\text{{\igprimfont Φ}}$-bottleneck theorem --- the proof that the lock can only be turned from the inside. This
...
$\texttt{ChiralityLE}$ with $\mathbf{N}/\mathbf{B}$ incomparability on the $\text{{\igprimfont Ħ}}$-axis.
...
structure. This is the $\text{{\igprimfont Φ}}$-bottleneck theorem in operational form: $\otimes(\text{{\igfont
...
character ($\text{{\igprimfont Ç}}$) and chirality/chirality ($\text{{\igprimfont Ħ}}$) had been conflated
...
`Ordering' axis had not yet separated into $\text{{\igprimfont Ħ}}$ (chirality/temporal
...
orthogonality --- an independent recovery of the grammar's $\text{{\igprimfont Þ}}$, $\text{{\igprimfont Φ}}$,
...
$\text{{\igprimfont Φ}}$), a directionality or geometry axis (mapping to $\text{{\igprimfont Þ}}$ and $\text{{\igprimfont Ð}}$), a
functional-outcome axis (mapping to $\text{{\igprimfont ⊙}}$), and a kinetic or
condition-sensitivity axis (mapping to $\text{{\igprimfont Ç}}$). These four dimensions
...
fidelity primitive $\text{{\igprimfont ƒ}}$ --- no binding energies, no computational
...
$\text{{\igprimfont Ħ}}$ &
  Absent; fused with $\text{{\igprimfont Ç}}$ under a ``temporal character" label &
  Chirality: \text{{\igfont 𐑓}}, \text{{\igfont 𐑒}}, \text{{\igfont 𐑖}}, \text{{\igfont 𐑫}} &
  \emph{Delamination.} $\text{{\igprimfont Ç}}$ encodes the rate of constraint change;
  $\text{{\igprimfont Ħ}}$ encodes the depth of temporal memory.
...
chirality ($\text{{\igprimfont Ħ}}$) were absent or fused. That partial output doesn't
...
this: it fused two primitives the first session had delaminated ($\text{{\igprimfont Ç}}$ and
...
\begin{enumerate}[noitemsep]
  \item \textbf{$\text{{\igprimfont Ð}}$}: Identify the character of the state space.
  ...
  \item \textbf{$\text{{\igprimfont Þ}}$}: Identify the connectivity topology.
    ...
    imscriptive closure.
  \item \textbf{$\text{{\igprimfont Ř}}$}: Identify the dominant relational mode.
    ...
    (peer coupling).
  \item \textbf{$\text{{\igprimfont Φ}}$}: Identify the symmetry class.
  ...
  \item \textbf{$\text{{\igprimfont ƒ}}$}: Identify information fidelity.
  ...
  \item \textbf{$\text{{\igprimfont Ç}}$}: Identify kinetic character.
  ...
  \item \textbf{$\text{{\igprimfont Γ}}$}: Identify the scope/granularity.
  ...
  \item \textbf{$\text{{\igprimfont Ħ}}$}: Identify chirality/chirality.
  ...
  \item \textbf{$\text{{\igprimfont Σ}}$}: Identify stoichiometry.
...
(\Cref{def:bottleneck}) requires that $\text{{\igprimfont Φ}}(X \otimes Y) =
\min(\text{{\igprimfont Φ}}_X, \text{{\igprimfont Φ}}_Y)$ and $\text{{\igprimfont ƒ}}(X \otimes Y) = \min(\text{{\igprimfont ƒ}}_X, \text{{\igprimfont ƒ}}_Y)$. Any
proposed imscription that would require a composite system to have
higher $\text{{\igprimfont Φ}}$ or $\text{{\igprimfont ƒ}}$ than its weakest component is internally
...
\begin{enumerate}
  ...
  \item \textbf{$\text{{\igprimfont Þ}} = \Tbw$}: Bowtie topology captures the
  ...
  \item \textbf{$\text{{\igprimfont Ř}} = \text{{\igfont 𐑽}}$}: Mutual implication between
    ...
    bivalent logic.
  \item \textbf{$\text{{\igprimfont Φ}} = \Ppms$}: The defining condition. The Liar requires
...
\begin{equation}
  ...
  \label{eq:liar_full}
\end{equation}
with $\text{{\igprimfont Φ}} = \Ppms$ and ouroboricity tier $\Oinf$ (R1). The assignment of
...
If the Liar imscribed at any $\text{{\igprimfont Φ}} < \Ppms$, the tensor product
...
is in $\text{{\igprimfont Ħ}}$ (unlimited meta-logical depth) and $\text{{\igprimfont Σ}}$ (many axioms, many
theorems):
...
G\"{o}del sentence $G'$. $\text{{\igprimfont Φ}} = \Ppms$ for the same reason as
...
differences in $\text{{\igprimfont Ħ}}$ and $\text{{\igprimfont Σ}}$ only.
