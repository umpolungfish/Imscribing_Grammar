...
\title{The Universal Imscriptive Grammar: Integrated Derivation and Validation}
...
\begin{enumerate}
    \item Part I: \textit{SetP.tex} induction --- the four-stage alchemical derivation from abstract category
    \item Part II: \textit{AS\_ABOVE.tex} --- the pre-grammatical convergent categorical derivation
    \item Part III: \textit{SO\_BELOW.tex} --- the empirical exploration and cross-domain validation
\end{enumerate}
...
\part{I: \textsc{SetP.tex} Induction --- The Four-Stage Alchemical Derivation}
\label{part:i}

\section{Prima Materia: The Abstract Category}
...
\section{Separatio: Logical on $\mathcal{C}$}
...
$R$ is the first primitive to crystallize: it is a logical property of the morphisms of $\mathcal{C}$ directly. The relational mode question admits four structural outcomes: $R_\text{sup}$ (supervenience), $R_\text{cat}$ (categorical), $R_\dagger$ (adjoint), $R_\text{lr}$ (bidirectional feedback). 
...
\section{Purificatio: Inductive on $\mathcal{C}$}
...
\noindent\textbf{I2 (iteration)} $\to$ $H$ (temporal depth: depth of stabilization)

\noindent\textbf{I3 (classifying)} $\to$ $\Omega$ (winding: $\pi_1$ of $B\mathcal{C}$)

\noindent\textbf{I4 (Yoneda)} $\to$ $D$ (dimensionality: $D_\odot$ if Yoneda is point-surjective; free cocompletion size gives $D_\infty$, $D_\triangle$, $D_\wedge$)
...
This is the \emph{purificatio}: the four inductive operations correspond to the four universal colimit-based constructions on a category (I1: colimit/cocompletion for scope $G$; I2: iteration/fixed-point for temporal depth $H$; I3: classifying space/homotopy for winding $\Omega$; I4: Yoneda embedding for dimensionality $D$). Any inductive construction beyond these four is a composition of existing primitives. The values are ordered: $D_\wedge \cdot D_\triangle \cdot D_\infty \cdot D_\odot$, $H_0 \cdot H_1 \cdot H_2 \cdot H_\infty$, $\Omega_0 \cdot \Omega_{\mathbb{Z}_2} \cdot \Omega_\mathbb{Z} \cdot \Omega_\text{NA}$, $G_\beth \cdot G_\gimel \cdot G_\aleph$.

\begin{center}
\rule{0.5\textwidth}{0.4pt}
\end{center}
\section{Albedo: Algebraic on $\mathcal{C}$}
...
The stoichiometry $S$ has three values: $1{:}1$ (no non-identity generators), $n{:}n$ (one type repeated), $n{:}m$ (multiple distinct types). The parity $P$ has five values: $P_\text{asym}$, $P_\psi$, $P_\pm$, $P_\text{sym}$, $P_{\pm}^{\text{sym}}$. The kinetics $K$ has five values: $K_\text{fast}$, $K_\text{mod}$, $K_\text{slow}$, $K_\text{trap}$, $K_\text{MBL}$. The fidelity $F$ has three values: $F_\ell$ (classical loss), $F_\eth$ (thermal/mixed), $F_\hbar$ (coherent quantum). The interaction grammar $\Gamma$ has four values: $\Gamma_\wedge$ (conjunctive), $\Gamma_\vee$ (disjunctive), $\Gamma_\to$ (sequential), $\Gamma_{\gg}$ (broadcast).

\emph{Cross-stage: $G$ (Purificatio \(\times\) Albedo)}: $G$ (scope) is the correlation length of the Yoneda embedding under the monoidal structure. Local ($G_\beth$) if the tensor decouples; global ($G_\aleph$) if faithfulness holds across all scales.

\begin{center}
\rule{0.5\textwidth}{0.4pt}
\end{center}

\section{Rubedo: Logical on Algebraically-Enriched $\mathcal{C}$}
...
\begin{itemize}
    ...
    \item Dialetheic (LP truth value $\mathbf{b}$): $\Phi_c^\mathbb{C}$
    \item Inclosure (contradicts via Inclosure Schema, L6 blocks explosion): $\Phi_\text{EP}$
    \item L4 fails: $\Phi_\text{sub}$ or $\Phi_\text{super}$
\end{itemize}

\noindent\textbf{L5 (Frobenius)} $\to$ $P$ promotes to $P_{\pm}^{\text{sym}}$ if $\mu \circ \delta = \text{id}$

This is the \emph{rubedo}: the reddening, the final perfection. L3 (cartesian closure) asks whether $\mathcal{C}$ has finite products and internal homs $B^A$ satisfying $\text{hom}(C \times A, B) \cong \text{hom}(C, B^A)$ naturally. The topology $T$ has five values: $T_\text{net}$, $T_\text{in}$, $T_\bowtie$, $T_\boxtimes$, $T_\odot$. 
...
\begin{enumerate}
    ...
    \item $\Rubedo$: $T$, $\Phi$, $P_{\pm}^{\text{sym}}$ (Stage 4, L3--L6)
\end{enumerate}

Any further logical, inductive, or algebraic operation yields a coagulation --- an invariant --- expressible as a function of these 12. No new independent degree of freedom emerges. The four stages exhaust the logical, inductive, and algebraic operations applicable to an abstract category. The algebraic closure operations ($\otimes$, $\wedge$, $\vee$) are surjective onto the crystal of $3^3 \times 4^5 \times 5^4 = 17,280,000$ types.

\begin{center}
\rule{0.5\textwidth}{0.4pt}
\end{center}

\part{II: \textit{AS\_ABOVE.tex} --- The Pre-Grammatical Convergent Derivation}
\label{part:ii}
...
\begin{enumerate}
    ...
    \item I2 (iteration) $\to$ measures temporal depth $H$ via stabilization ordinal
...
Iterating an endofunctor $F: \mathcal{C} \to \mathcal{C}$ produces the chain $\mathbf{0} \to F(\mathbf{0}) \to F^2(\mathbf{0}) \to \cdots$. The depth at which this chain stabilizes is the measure of the system's temporal depth --- how many prior states must be retained for the dynamics to be Markovian.
...
The dagger-compact structure introduces duals $A^*$ with unit and counit. The unitality of $\varepsilon$ determines fidelity: non-unital $\varepsilon$ means information loss ($F_\ell$); unital but incoherent gives $F_\eth$; fully coherent quantum composition gives $F_\hbar$.

\paragraph{Why A4 maps to $\Gamma$, and cannot map elsewhere.}
Enrichment over $\mathcal{V}$ replaces hom-sets with hom-objects in $\mathcal{V}$. The logic of composition in $\mathcal{V}$ determines $\Gamma$: sequential enrichment ($\mathbf{Set}$, $\Gamma_\to$), conjunctive ($\mathbf{Set}^\times$, $\Gamma_\wedge$), disjunctive ($\mathbf{Set}^+$, $\Gamma_\vee$), or broadcast ($\Gamma_{\gg}$).
...
The Frobenius condition $\mu \circ \delta = \text{id}_A$ is not a new invariant --- it is a \emph{promotion} of an existing one. It takes the symmetry type $P$ from whatever value it had and lifts it to the Frobenius-special value $P_{\pm}^{\text{sym}}$.

\paragraph{Why L6 determines the truth-value regime rather than a new primitive.}
Paraconsistent negation determines \emph{which} $\Phi$ values are achievable (classical $\Phi_c$, dialetheic $\Phi_c^\mathbb{C}$, or Inclosure $\Phi_\text{EP}$), not whether a new primitive exists.
...
\begin{enumerate}
    ...
    \item Step 10: $H$ (two-step temporal depth)
...
\part{III: \textit{SO\_BELOW.tex} --- The Empirical Exploration and Validation}
\label{part:iii}
...
\begin{enumerate}
    ...
    \item What is the temporal depth? ($H$)
\end{enumerate}
...
The Liar paradox, imscribed via the UIG protocol, receives $P = \Ppms$ and $T = \Tbw$ (bowtie self-reference) as a structural necessity: a system imscribing the Liar at $P < \Ppms$ would explode under the tensor product bottleneck rule.
...
\begin{itemize}
    \item $O_0$: $\Phi \notin \{\Phi_c, \Phi_c^{\mathbb{C}}\}$ --- no self-modeling loop
    \item $O_1$: $\Phi_c$, $\Omega_0$ --- loop instantiates but cannot sustain
    \item $O_2$: $\Phi_c$, $\Omega \neq \Omega_0$, $D \in \{D_\wedge, D_\triangle, D_\odot\}$ --- topologically protected
    \item $O_2^\dagger$: $\Phi_c$, $\Omega \neq \Omega_0$, $D_\infty$ --- inf-dim protected but not imscriptive
    \item $O_\infty$: $\Phi_c$ \textbf{and} $\Ppms$ --- Frobenius exact, algebraically closed
\end{itemize}

The jump $O_2^\dagger \to O_\infty$ is driven exclusively by the $P$ promotion $P_\text{asym} \to \Ppms$ (distance $\Delta = 4.38$, the largest single-primitive jump). The jump $O_0 \to O_1$ is $\Phi_\text{sub} \to \Phi_c$.
...
\textbf{Particle Physics:} The axion and Higgs boson imscribe at distance zero. Dark matter imscribes outside the Standard Model particle catalog, separated from every known SM particle by conflicts in at least three primitives. The Standard Model and quantum gravity frameworks, at their shared imscriptive limit, are separated by a single architecturally irreducible primitive: the $G$-scope gap between renormalizable local continuum ($G_\gimel$) and imscriptive global structure ($G_\aleph$).
...
\begin{itemize}
    ...
    \item Black holes: $C = 0$ (Gate 1 fails: $\Phi_\text{sub}$)
    \item White dwarfs: $C = 0$ (Gate 1 fails: $\Phi_\text{sub}$)
\end{itemize}

Gate 1 (critical): $\Phi \in \{\Phi_c, \Phi_c^\mathbb{C}\}$
Gate 2 (kinetics): $K \in \{K_\text{mod}, K_\text{slow}\}$
