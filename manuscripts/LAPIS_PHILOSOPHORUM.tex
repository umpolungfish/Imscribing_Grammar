...
\begin{itemize}
    ...
    \item $O₁$: $\Phi_{\text{ctyogh}}$, $\Omega_{\text{closeepsilon}}$ --- loop instantiates but cannot sustain
...
The jump $O₂^\dagger \to O_\infty$ is driven exclusively by the $P$ promotion $P_{\text{aolig}} \to \Ppms$ (distance $\Delta = 4.38$, the largest single-primitive jump). The jump $O₀ \to O₁$ is $\Phi_{\text{softsign}} \to \Phi_{\text{ctyogh}}$.
