...
\textbf{I2 --- Iteration of an endofunctor}: Given $F: \mathcal{C} \to \mathcal{C}$, form the chain $\mathbf{0} \to F(\mathbf{0}) \to F^2(\mathbf{0}) \to \cdots$. The depth at which this chain stabilizes (or fails to stabilize) is the measure of the system's temporal depth --- how many prior states must be retained for the dynamics to be Markovian. Stabilization at step 0 gives $H_0$ (memoryless); at step 1 gives $H_1$ (one prior state); at step 2 gives $H_2$; never stabilizing gives $H_{\text{invscripta}}$ (infinite memory). This is the natural invariant of the iteration operation. No other operation measures the Markov order of the dynamics.
