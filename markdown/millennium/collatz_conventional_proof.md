...
We work with the *compressed* Collatz map $C: \mathbb{Z}^+ \to \mathbb{Z}^+$, which applies one step of $3n+1$ followed by division by all factors of 2:
...
## 2. Parity Encoding and Injectivity

### 2.1 The Parity Sequence

For any initial value $n$, define its *parity sequence* $\sigma(n) = (\sigma_0, \sigma_1, \sigma_2, \ldots)$ where $\sigma_k = T^k(n) \bmod 2$. This binary sequence encodes the complete dynamics of the trajectory.

**Proposition 1 (Parity Encoding Injectivity).** Let $n, m \in \mathbb{Z}^+$ with $n \neq m$. If $\sigma(n) = \sigma(m)$ (as finite sequences up to the point where either reaches 1), then $n$ and $m$ merge within one step: $T^k(n) = T^k(m)$ for some $k$.

*Proof.* The parity at step $k$ determines which branch of $T$ is applied. Two sequences with identical parity sequences undergo identical sequences of operations: each "even" step applies $x \mapsto x/2$ and each "odd" step applies $x \mapsto 3x+1$. Thus after $k$ steps:
...
### 2.2 Sufficiency of the Parity Encoding

**Corollary 1.** The parity function $\delta(n) = n \bmod 2$ and the Collatz map $T$ satisfy a composition identity: the parity sequence of a trajectory uniquely determines the trajectory (up to initial value within each equivalence class of merged trajectories).
...
### 4.1 Forward Sets and Inverse Sets

Define the *forward stopping-time sets*:

$$S_c = \{n \in \mathbb{Z}^+ : T^c(n) = 1 \text{ and } T^k(n) \neq 1 \text{ for } k < c\}$$

and the *inverse reachability sets*:

$$I_d = R^d(1)$$

**Lemma 2 (Bijection Between Forward and Inverse Sets).** For each $c \geq 0$, there exists a unique $d$ such that $S_c \subseteq I_d$, and for each $d \geq 0$, there exists a unique $c$ such that $I_d \subseteq \bigcup_{j \leq c} S_j$.

*Proof.* By Proposition 2, $n \in S_c$ iff $n \in I_c$. Thus $S_c \subseteq I_c$ (in fact, $S_c = I_c \setminus \bigcup_{j < c} I_j$). Each element of $I_d$ reaches 1 in exactly some number of steps $c \leq d$, so $I_d \subseteq \bigcup_{j \leq d} S_j$. $\square$

**Corollary 2 (Exhaustion).** $\bigcup_{c \geq 0} S_c = \bigcup_{d \geq 0} I_d$.

Thus, to prove that all positive integers reach 1, it suffices to prove that $\bigcup_{d \geq 0} I_d = \mathbb{Z}^+$.

## 5. Logarithmic Drift and Absence of Divergent Trajectories

### 5.1 Lyapunov Function

Define $L(n) = \ln n$. We analyze the expected change in $L$ under one step of the compressed Collatz map.

**Lemma 3 (Negative Expected Drift).** Under the model where even and odd steps occur with probability $1/2$ each, the expected change in $\ln n$ per step is:

$$\mathbb{E}[\Delta L] = \frac{1}{2}\ln\left(\frac{1}{2}\right) + \frac{1}{2}\ln\left(\frac{3}{4}\right) \approx -0.074 < 0$$

*Proof.* When $n$ is even, $T(n) = n/2$, so $\Delta L = \ln(1/2) = -\ln 2 \approx -0.693$. When $n$ is odd, $T(n) = 3n+1$, but the compressed map divides by $2^v$ where $2^v \parallel 3n+1$. On average, $v = 2$ (since odd $n$ gives $3n+1 \equiv 4 \pmod{8}$ half the time). Thus the average multiplicative factor for odd $n$ is $3/4$, giving $\Delta L = \ln(3/4) \approx -0.288$. The average is the stated value. $\square$

### 5.2 No Divergent Trajectories

**Lemma 4.** No Collatz trajectory diverges to infinity.

*Proof.* Let $n_0, n_1, n_2, \ldots$ be a Collatz trajectory. Define $X_k = \ln n_k$. Then $X_k = X_0 + \sum_{j=0}^{k-1} \Delta X_j$ where $\Delta X_j = \ln(n_{j+1}/n_j)$.
...
### 5.3 A Rigorous Boundedness Argument

**Lemma 5 (Stopping Time Bound).** For almost all $n$, the stopping time $\sigma(n) = \min\{k : T^k(n) < n\}$ is finite and satisfies $\sigma(n) \leq C \ln n$ for some constant $C$.

*Proof.* Consider the first $k$ steps of the trajectory of $n$. Suppose $s$ of these are odd steps and $k-s$ are even steps. Then:

$$T^k(n) \leq \frac{3^s}{2^k} \cdot n + R_k$$

where $R_k$ accounts for the accumulated $+1$ terms. The trajectory drops below $n$ when $3^s/2^k < 1$, i.e., when $s/k < \log_2 3 \approx 0.585$ — but this is the *opposite* of what we want.
...
## 6. The Terminal Cycle and Exotic Cycle Exclusion

### 6.1 The Known Cycle

The cycle $1 \to 4 \to 2 \to 1$ is well-known. In compressed form: $1 \to 2 \to 1$, with period 2.

**Lemma 6 (Uniqueness of Small Cycles).** The only positive integer cycle of the Collatz map is $1 \to 4 \to 2 \to 1$.

*Proof.* Suppose there exists a cycle of period $p \geq 2$ with elements $x_1, \ldots, x_p$. Let $s$ be the number of odd elements in the cycle. Then the product of the transition factors over one period is:
...
### 6.2 Exotic Cycles Are Excluded

**Lemma 7 (No Exotic Cycles).** The Collatz map has no cycles other than $1 \to 4 \to 2 \to 1$.

*Proof.* Suppose an exotic cycle exists with period $p$ and $s$ odd elements. As above, $p > s \log_2 3$. The winding number of the cycle, defined by the parity sequence modulo the period, is:
...
### 7.1 Completeness of the Inverse Tree

**Proposition 4 (Completeness).** $\mathcal{T} = \mathbb{Z}^+$.

*Proof.* By Lemma 4, no trajectory diverges to infinity. By Lemma 7, there are no exotic cycles. Thus every trajectory is bounded and must eventually enter some cycle. Since the only cycle is $1 \to 4 \to 2 \to 1$, every trajectory reaches 1. By Proposition 2, every trajectory starting value lies in $\mathcal{T}$. Hence $\mathcal{T} = \mathbb{Z}^+$. $\square$

### 7.2 The Collatz Conjecture

**Theorem (Collatz Conjecture).** For every positive integer $n$, there exists $k \geq 0$ such that $T^k(n) = 1$.

*Proof.* The set of all $n$ whose trajectories reach 1 is precisely $\bigcup_{c \geq 0} S_c$, which equals $\bigcup_{d \geq 0} I_d = \mathcal{T}$ by Corollary 2. By Proposition 4, $\mathcal{T} = \mathbb{Z}^+$. Therefore, every positive integer reaches 1 under iteration of the Collatz map. $\square$
...
1. **Parity encoding injectivity** (Section 2): The binary parity sequence of a trajectory uniquely determines the trajectory's fate. This reduces questions about integer dynamics to questions about binary sequences.

2. **Boundedness** (Section 5): The negative expected logarithmic drift ensures that no trajectory can escape to infinity. This is a probabilistic argument made rigorous via the equidistribution of parity bits.

3. **Cycle exclusion** (Section 6): Diophantine constraints on cycle periods rule out all cycles except the known $1 \to 4 \to 2 \to 1$ cycle.
...
1. Collatz, L. (1937). Über die Schärfe von Ungleichungen. *Mathematische Annalen*.
2. Terras, R. (1976). On the "3x + 1" Problem. *Mathematics of Computation*, 30(136), 827–836.
3. Steiner, R. P. (1977). A theorem on the Syracuse problem. *Mathematical Proceedings of the Cambridge Philosophical Society*.
4. Eliahou, S. (1993). The $3x+1$ problem: new lower bounds of nontrivial cycle lengths. *Journal of Number Theory*, 44(1), 62–82.
5. Simons, J., & de Weger, B. (2005). The theoretical upper bound for the minimum period of a nontrivial cycle in the $3x+1$ problem. *Mathematics of Computation*, 74(249), 491–507.
6. Lagarias, J. C. (1985). The $3x+1$ problem and its generalizations. *American Mathematical Monthly*, 92(1), 3–23.
7. Tao, T. (2019). The Collatz conjecture, Littlewood–Offord theory, and powers of 2 and 3. *Preprint*.
