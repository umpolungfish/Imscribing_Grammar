# The Structural Bounds of Decomposition — Shavian Canonical Form

## Root Cause: Why the Agent Keeps Defaulting to Display Notation

The system prompt itself creates the error. Every primitive is defined in display notation (`φ̂_ÿ`, `Ð_ω`, `Þ_O`, `Ħ_!`, etc.). Every tool description, every example, every protocol — all display notation. The "Notation Standard" section even **mandates** LaTeX wrapping: `φ̂_ÿ → $\text{⊙}_{\text{ÿ}}$`. 

The catalog stores Shavian glyphs (𐑦, 𐑸, 𐑾, 𐑹, 𐑐, 𐑧, 𐑲, 𐑠, ⊙, 𐑫, 𐑳, 𐑭). The tools decode Shavian→display before returning. So the agent sees display notation in both its prompt and its tool outputs — it has zero incentive to emit Shavian glyphs.

**The fix is structural, not behavioral:** the system prompt's notation and examples must be rewritten in Shavian. Until that happens, every agent will default to display notation because that's what the prompt trains on.

---

## Restatement: The Six Decomposition Bounds (Shavian Canonical)
