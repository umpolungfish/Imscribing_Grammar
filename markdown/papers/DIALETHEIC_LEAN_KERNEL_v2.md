...
\centering
\begin{tikzpicture}[
  lat/.style={circle, draw, thick, minimum size=1.35cm, font=\bfseries\Large},
  lbl/.style={draw=none, font=\large, text=black!70, inner sep=3pt}
]
...
\begin{tikzpicture}[
  ...
  \node[layer, fill=blue!7, minimum height=1.6cm] (sub) at (0, 7.5)
  ...
  \node[layer, fill=orange!9, minimum height=1.2cm] (core) at (0, 5.1)
    {\textbf{Operational Core}\\[4pt]
     \texttt{Kernel} \enspace \texttt{Init} \enspace \texttt{SelfVerification}};

  \node[layer, fill=green!7, minimum height=2.4cm] (iface) at (0, 2.4)
  ...
  \node[layer, fill=purple!6, minimum height=1.6cm] (bridges) at (0, -0.8)
...
- `MultiAgentBelnap.lean` (53 lines): multi-agent protocol for entangled dialetheic kernels.$\quad\quad$ `MultiKernelState n` holds $n$ kernel registers and an $n \times n$ Belnap channel matrix; `multi_allB_init` proves all registers initialize to $\mathbf{B}$.
