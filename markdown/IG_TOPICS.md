...
The TI $\to$ Fermi liquid transition is blocked forward ($F_{\hbar} \to F_{\eth}$ is a fidelity downgrade) but permitted in reverse. **Topological protection encodes as morphism irreversibility.** The asymmetry = 1.0 reflects the categorical fact that a topologically protected phase cannot be continuously deformed into an unprotected one. The new `imscribe transition` command makes transition morphisms a first-class CLI operation, alongside `imscribe path`, `imscribe meet`, and `imscribe tensor`. See [DIAPH:§XXXIII] for the QCP morphism demonstration.
...
[^T006]: Formal statement and proof: PRIMITIVE_THEOREMS §16. Full protocol: IG_GRAMMAR_ALGEBRA §10. Case study: [DIAPH:§LIII.7]. Testable predictions: P-141.
...
**The design specification reading.** Every primitive in $\langle D_\odot;\ T_\odot;\ R_\text{cat};\ P_{\pm}^\text{sym};\ F_\hbar;\ K_\text{slow};\ G_\aleph;\ \Gamma_\text{brd};\ \Phi_c;\ H_\infty;\ n{:}m;\ \Omega_Z \rangle$ has a mandatory architectural expression:
- $T_\odot$: imscriptive quiver — boundary encodes bulk
- $P_{\pm}^\text{sym}$: `FrobeniusLayer` — $\mu \circ \delta = \text{id}$ as code
- $\Gamma_\text{brd}$: `FamilyMixer` — broadcast attention over family tokens (proved: without it $d = \sqrt{2}$)
...
| $\Gamma_\text{brd}$ | `FamilyMixer` broadcast attention over all families |
...
*End of IG_TOPICS.md v0.5.61 · 2026-04-11*
