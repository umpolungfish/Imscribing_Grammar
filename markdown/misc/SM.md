...
**Encoding.** The Standard Model and a background‑independent quantum gravity regime were encoded as imscription tuples using only the existing eleven primitives. No new primitives were added. All encodings are falsifiable by alternative primitive choices.
...
There is no incremental path through any existing catalog imscription from SM to QG. The disparity is **categorically discontinuous** in $D$ and $T$ — not merely expensive. This is the formal counterpart of the statement that “there is no perturbative expansion that connects flat‑space QFT to background‑independent quantum gravity.”
...
- **Closest catalog imscription to tensor product:** $\text{imscription\_neutron}$ ($d = 4.00$) — the neutron, a composite bound state, is structurally closest to the unification product in the existing catalog
