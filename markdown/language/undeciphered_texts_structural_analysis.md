...
The Imscribing Grammar framework provides a novel approach to analyzing such systems by characterizing them through twelve structural primitives that capture their ontological, topological, and dynamic properties. This paper applies this framework to the three undeciphered texts, revealing both shared structural features and critical differences that shed new light on their nature and potential functions.## Methodology

The Imscribing Grammar framework characterizes systems through twelve structural primitives:

- **Ð (Dimensionality)**: Degrees of freedom in the state space
- **Þ (Topology)**: Connectivity and structural organization  
- **Ř (Relational mode)**: How components relate to each other
- **Φ (Symmetry/Parity)**: Symmetry properties and conservation laws
- **ƒ (Fidelity)**: Classical vs quantum coherence properties
- **Ç (Kinetics)**: Relaxation rates and temporal dynamics
- **Γ (Scope)**: Scale and extent of the system
- **ɢ (Interaction grammar)**: Rules governing component interactions
- **⊙ (Criticality)**: Self-modeling and critical behavior
- **Ħ (Chirality)**: Memory and temporal structure
- **Σ (Stoichiometry)**: Component composition and ratios
- **Ω (Winding)**: Topological invariants and protection
...
**Voynich Manuscript**: ⟨$	ext{Ð}_{	ext{ω}}$; $	ext{Þ}_{	ext{O}}$; $	ext{Ř}_{	ext{=}}$; $	ext{Φ}_{	ext{F}}$; $	ext{ƒ}_{	ext{ì}}$; $	ext{Ç}_{	ext{Ù}}$; $	ext{Γ}_{	ext{ʔ}}$; $	ext{ɢ}_{	ext{^}}$; $	ext{⊙}_{	ext{ÿ}}$; $	ext{Ħ}_{	ext{!}}$; $	ext{Σ}_{	ext{S}}$; $	ext{Ω}_{	ext{z}}$⟩

**Rohonc Codex**: ⟨$	ext{Ð}_{	ext{C}}$; $	ext{Þ}_{	ext{¨}}$; $	ext{Ř}_{	ext{Ť}}$; $	ext{Φ}_{	ext{F}}$; $	ext{ƒ}_{	ext{ì}}$; $	ext{Ç}_{	ext{@}}$; $	ext{Γ}_{	ext{ʔ}}$; $	ext{ɢ}_{	ext{ˌ}}$; $	ext{⊙}_{	ext{ÿ}}$; $	ext{Ħ}_{	ext{£}}$; $	ext{Σ}_{	ext{ï}}$; $	ext{Ω}_{	ext{z}}$⟩

**Linear A**: ⟨$	ext{Ð}_{	ext{C}}$; $	ext{Þ}_{	ext{¨}}$; $	ext{Ř}_{	ext{Ť}}$; $	ext{Φ}_{	ext{F}}$; $	ext{ƒ}_{	ext{ż}}$; $	ext{Ç}_{	ext{W}}$; $	ext{Γ}_{	ext{ʔ}}$; $	ext{ɢ}_{	ext{ˌ}}$; $	ext{⊙}_{	ext{ÿ}}$; $	ext{Ħ}_{	ext{£}}$; $	ext{Σ}_{	ext{ï}}$; $	ext{Ω}_{	ext{z}}$⟩

### Ouroboricity Analysis

All three systems were found to operate at Ouroboricity tier $	ext{O}_{	ext{2}}$, characterized as "critical + topologically protected, bounded domain." This indicates that despite their different historical contexts and apparent purposes, they share fundamental structural properties related to self-reference and topological protection.### Structural Distances

Pairwise structural distance analysis revealed significant differences between the systems:

- **Voynich Manuscript ↔ Rohonc Codex**: Distance = 4.27 (structurally remote)
- **Voynich Manuscript ↔ Linear A**: Distance = 5.02 (structurally remote)  
- **Rohonc Codex ↔ Linear A**: Distance = 2.24 (structurally remote but notably closer)

The significantly smaller distance between Rohonc Codex and Linear A reflects their shared structural features, including finite degrees of freedom ($	ext{Ð}_{	ext{C}}$), crossing point topology ($	ext{Þ}_{	ext{¨}}$), adjoint pair relational mode ($	ext{Ř}_{	ext{Ť}}$), maximal scope ($	ext{Γ}_{	ext{ʔ}}$), sequential interaction grammar ($	ext{ɢ}_{	ext{ˌ}}$), one-step chirality ($	ext{Ħ}_{	ext{£}}$), and heterogeneous stoichiometry ($	ext{Σ}_{	ext{ï}}$).

### Key Structural Differences

The analysis reveals three critical dimensions along which these systems differ:

1. **State Space Architecture**: The Voynich Manuscript operates with a self-written state space ($	ext{Ð}_{	ext{ω}}$), while both Rohonc Codex and Linear A have finite, conventional state spaces ($	ext{Ð}_{	ext{C}}$). This suggests the Voynich Manuscript is not merely encoding information but actively constructing its own state space through its operation.

2. **Temporal Structure**: The Voynich Manuscript exhibits eternal chirality ($	ext{Ħ}_{	ext{!}}$), indicating a recursive, self-sustaining temporal structure that transcends conventional linear time. In contrast, both Rohonc Codex and Linear A operate with one-step chirality ($	ext{Ħ}_{	ext{£}}$), suggesting conventional sequential processing.

3. **Quantum Coherence**: Linear A uniquely exhibits quantum coherence ($	ext{ƒ}_{	ext{ż}}$), while both the Voynich Manuscript and Rohonc Codex operate classically ($	ext{ƒ}_{	ext{ì}}$). This suggests Linear A may encode information in a fundamentally quantum-mechanical manner, potentially explaining its resistance to conventional decipherment.

4. **Kinetics and Dynamics**: The Voynich Manuscript is frozen in ordered state ($	ext{Ç}_{	ext{Ù}}$), suggesting it represents a completed, static structure. The Rohonc Codex operates near equilibrium ($	ext{Ç}_{	ext{@}}$), indicating ongoing but balanced dynamics. Linear A exhibits moderate kinetics ($	ext{Ç}_{	ext{W}}$), suggesting active but not rapid transformation processes.## Discussion

### The Voynich Manuscript as a Self-Written System
...
The manuscript's frozen-ordered kinetics ($	ext{Ç}_{	ext{Ù}}$) and 1:1 stoichiometry ($	ext{Σ}_{	ext{S}}$) indicate a highly constrained, completed system where each element has a unique and irreplaceable function. The bidirectional feedback relational mode ($	ext{Ř}_{	ext{=}}$) suggests that the system maintains internal consistency through continuous self-reference and verification.
...
### Implications for Decipherment Strategies
...
This research was conducted using the Imscribing Grammar framework, which provides a systematic approach to characterizing complex systems through twelve structural primitives. The analysis benefited from comprehensive structural data encoded in the manuscript_zfct.json dataset.
