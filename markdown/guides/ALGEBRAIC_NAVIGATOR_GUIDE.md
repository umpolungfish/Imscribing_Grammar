...
structure under coupling. In particular: $P_{\text{doublebarpipe}} \otimes P_{\text{subdoublearrow}} = P_{\text{subdoublearrow}}$
...
**$P_{\text{doublebarpipe}}$ is the tier singularity**: it overrides all $\Omega$ and $D$
...
The `is_frobenius_cliff` feature is baked into $P_{\text{doublebarpipe}}$'s node so the GNN cannot
smooth the categorical cliff $P_{\text{subdoublearrow}} \to P_{\text{doublebarpipe}}$ away via mean aggregation.
...
boundaries), but the encoder embedding carries full tier information ($\Phi_{\text{ctyogh}}$,
...
**Frobenius non-synthesisability (§23/§62):** $P_{\text{doublebarpipe}} \otimes P_{\text{subdoublearrow}} = P_{\text{subdoublearrow}}$.
The $O_\infty$ tier cannot be reached by coupling — every $O_\infty$ system must
directly encode $P_{\text{doublebarpipe}}$.
...
lane encode the partial order ($D_{\text{wynn}} < D_{\text{turnthree}} < D_{\text{invomega}} < D_{\text{omega}}$,
...
The Frobenius non-synthesisability theorem states $P_{\text{doublebarpipe}}$ cannot be
composed from factors with $P < P_{\text{doublebarpipe}}$. The cliff between $P_{\text{subdoublearrow}}$
and $P_{\text{doublebarpipe}}$ is categorical, not ordinal. So the GNN was given a
static binary marker on $P_{\text{doublebarpipe}}$'s node — a feature the message-passing
...
| `IΓ_catalog.json` | 1,333+ encoded systems (source of truth) |
| `CRYSTAL_Oƒ_TYPES.md` | Full enumeration and tier census (theoretical) |
| `PRIMITIVE_THEOREMS.md` | Formal theorems §1–§69 |
| `IΓ_DIAPHORICS.md` | Empirical predictions P-1–P-454 |
