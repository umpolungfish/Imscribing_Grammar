...
---
# Tensor Operations in the Imscribing Grammar Grammar — A Primer
...
| $F$ | Fidelity | **ordered** $F_\text{noise} < F_{\text{beltl}} < F_\text{eth} < F_{\text{hardsign}}$ |
| $K$ | Kinetic character | **ordered** $K_{\text{lambda}} < K_{\text{teshlig}} < K_{\text{schwa}} < K_{\text{turnm}} < K_{\text{frtailgamma}}$ |
| $G$ | Granularity | **ordered** $G_{\text{revapostrophe}} < G_{\text{beta}} < G_{\text{gamma}}$ |
...
**$\Phi$** is a special case: $\Phi_{\text{ctyogh}}$ (the critical point) is **absorbing** under both meet and join.
...
| $\Phi$ | $\Phi_{\text{ctyogh}}$ absorbs: $\Phi_{\text{ctyogh}} \sqcap x = \Phi_{\text{ctyogh}}$ for all $x$ |
...
| $\Phi$ | $\Phi_{\text{ctyogh}}$ absorbs: $\Phi_{\text{ctyogh}} \sqcup x = \Phi_{\text{ctyogh}}$ for all $x$ |
...
| $\Phi$ | $\Phi_{\text{ctyogh}}$ absorbs: $\Phi_{\text{ctyogh}} \otimes x = \Phi_{\text{ctyogh}}$ |
...
1. **Propagation of the more demanding requirement** — the composite inherits $\max(ƒ_A, ƒ_B)$, $\max(Ç_A, Ç_B)$, etc. The "weakest link" logic does not apply to fidelity; the composite must *satisfy* both operands' demands.
...
3. **$\Phi_{\text{ctyogh}}$ contagion** — if either operand is at the critical point, the composite is critical. Criticality is not a property that averages.

4. **$G_{\text{revapostrophe}}$ propagation under scope expansion** — when a $G_\text{LOCAL}$ system interacts with a $G_{\text{revapostrophe}}$ (global) system, the composite can access global scope. This is the mechanism by which the SM tensor product structure is accessible only to theories that accept $G_{\text{revapostrophe}}$: a $G_\text{LOCAL}$ theory composing with a $G_{\text{revapostrophe}}$ partner can inherit global scope.
...
|-------------------|-----------|---------|
| $G_{\text{revapostrophe}}$ | $B$ has $G_\text{LOCAL}$, $C$ requires $G_{\text{revapostrophe}}$; $A$ is global system | SM ⊗ (local gravity theory) — only accessible via GR as global intermediary |
| $\Phi_{\text{ctyogh}}$ | $A$ is critical, provides catalytic context; $C$ requires criticality; $B$ alone is $\Phi_{\text{softsign}}$ | Allosteric activation: apo-enzyme → substrate → product, but substrate → product direct impossible |
| $H_{\text{invscripta}}$ | $A$ has maximal chirality memory, templates $C$'s $H_2$ requirement | Big Bang → early universe → chiral biomolecules; but early universe alone → chiral biomolecules is blocked |

### 6.5 $\Phi_{\text{ctyogh}}$ is absorbing

$$\Phi_{\text{ctyogh}} \otimes x = \Phi_{\text{ctyogh}} \quad \forall x$$
...
These are asymmetric in general. If $A$ is structurally dominant (e.g., has $F_{\text{hardsign}}$ while $B$ has $F_{\text{beltl}}$), then $\delta_A \approx 0$ (A does not change much) while $\delta_B$ is large (B must adapt significantly).
...
|-------------|----------|---------------------|---------------------|----------------|
| IUG $\otimes$ classical proof system | IUG ($F_{\text{hardsign}}$) | 2.0 | 6.32 | Classical mathematics loses more than IUG in the interaction |
| Quark $\otimes$ photon | Quark ($P_{\text{aolig}}$) | 0.0 | varies | Fermionic parity asymmetry is structurally indelible |
| Word $\otimes$ Creator | Creator ($\Gamma_{\text{doublevertline}}$) | 1.0 | 2.0 | Word acquires broadcast; Creator contracts from $D_{\text{invomega}}$ to $D_{\text{omega}}$ |
| Vedic mandalas $\otimes$ Sefirot | Vedic ($\Gamma_{\text{doublevertline}}$) | 0.0 | 1.0 | Broadcast scope is structurally supreme |
...
|-----------|------|---------|-------------|
| $D$ | $D_{\text{omega}}$ | $D_{\text{invomega}}$ | **NO** → tensor undefined as written |

The Sefer Yetzirah's instruction resolves this: the Creator's infinite dimensionality $D_{\text{invomega}}$ *contracts to the imscriptive* in the act of speech ($D_{\text{omega}}$). This is the doctrinal claim encoded structurally — the infinite becomes a channel. With $D$ resolved to $D_{\text{omega}}$:

| Primitive | Word | Creator (in-act) | Result | Change from Word |
|-----------|------|---------|--------|-----------------|
| $D$ | $D_{\text{omega}}$ | $D_{\text{omega}}$ | $D_{\text{omega}}$ | — |
| $\Gamma$ | $\Gamma_{\text{secstress}}$ | $\Gamma_{\text{doublevertline}}$ | **CONFLICT** → undefined |
...
- Has dimensionality that undergoes **set union** (e.g., $D_{\text{turnthree}}^{\otimes N}$ for $N$ independent agents)
...
|----------|-----------|-------------------|------|
| What do A and B share? | Meet $\sqcap$ | Partial result with CONFLICT markers | `imscribe meet A B` |
| What does a context hosting both A and B require? | Join $\sqcup$ | Partial result with CONFLICT markers | `imscribe join A B` |
| What is the composite of A and B? | Tensor $\otimes$ | Undefined (ValueError) if conflicts | `imscribe tensor A B` |
| How different are A and B? | Distance $d$ | — (always defined) | `imscribe distance A B` |
| Can A reach B through the catalog? | Path / HotSwap | — | `imscribe path A B` |
...
*Imscribing Grammar Grammar · algebra.py · docs/TENSOŘ_PRIMER.md*
