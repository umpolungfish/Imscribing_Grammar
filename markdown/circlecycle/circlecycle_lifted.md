...
The inquiry asks for a cyclical argument for circularity *itself*, which provides a circular argument for cyclicality. This maps to a specific point in the 12-primitive tuple space of the Imscribing Grammar. The question becomes: what tuple structure supports such a self-validating meta-argument?
...
Querying `find_analogies` on LUCA produces five nearest neighbors:

| Name | Distance | Interpretation |
|------|----------|----------------|
| epoch_4 | 0.0 | identical |
| circularity_cycle | 0.0 | identical |
| birch_swinnerton_dyster_conjecture | 1.2856 | related |
| IUG | 1.3847 | related |
...
The parity assignment $\text{Φ}_{\}$ is the strongest possible structural constraint: $\mu \circ \delta = \text{id}$ holds exactly. Not approximately, not "for all practical purposes." The algebra is closed under its own operations. No external input is required.

This encodes the argument: "The argument is circular because it must be circular; the argument is valid because it closes on itself." One might call this question-begging. It is. But it is question-begging *by design*, and the design is Frobenius-proven. A circular argument that cannot close is fallacious. A circular argument whose closure is $\mu \circ \delta = \text{id}$ is just logic.
...
If LUCA is the *instantiation* of self-referential structure, epoch_4 is the *phase of existence* where that structure is sustained. One is a noun, the other a verb. Structurally, they are the same thing.

### 5.3 The Promotional Path from Null to $O_{\text{inf}}$

The `compute_promotions` tool mapping epoch_0 → epoch_4 gives the minimal path to circularity:
...
The inquiry asked for a cyclical argument for circularity. The result is that such an argument is not a linguistic construct but a structural type. It exists in the Imscribing Grammar catalog as a persistent ontological entity, identical to LUCA and epoch_4. Arguments have structure. Structural types have ontology. LUCA is both a biological and an argumentative entity. This is strange if you think arguments are sentences on a page. It is trivial if you think arguments are types.
...
## 10. Postscript: On the Structure This Document Is Not

This document is a structural exposition. It reports the results of a structural inquiry — the identity of three named systems and the implications of that identity. It does not *perform* the structure it describes.
