...
**Encoded type:** $\langle D_{\text{turnthree}};\ T_{\text{bullseye}};\ R_{\text{ctz}};\ P_{\text{pipevar}};\ F_{\text{beltl}};\ K_{\text{turnm}};\ G_{\text{gamma}};\ \Gamma_{\text{spleftarrow}};\ \Phi_{\text{ctyogh}};\ H_2;\ 1{:}1;\ \Omega_{\text{closeepsilon}} \rangle$
...
| System A | System B | Distance | $\ɢ_A$ | $\ɢ_B$ |
|---|---|---|---|---|
| proline_aldol | wittig | 2.0000 | $\Gamma_{\text{spleftarrow}}$ | $\Gamma_{\text{spleftarrow}}$ |
| proline_aldol | aryl_diazonium | 2.2361 | $\Gamma_{\text{spleftarrow}}$ | $\Gamma_{\text{spleftarrow}}$ |
| on_water | proline_aldol | 2.7928 | $\Gamma_{\text{corner}}$ | $\Gamma_{\text{spleftarrow}}$ |
| aryl_diazonium | grignard | 3.2711 | $\Gamma_{\text{spleftarrow}}$ | $\Gamma_\textrm{seq}$ |
...
- The proline aldol's underdetermined primitive is the relative flux among enamine, enol, and oxazolidinone channels ($\Gamma_{\text{spleftarrow}}$).
- The diazonium's underdetermined primitive is the parity character $P_{\text{upsilon}}$ — the ionic/radical superposition.
- The on-water reaction's underdetermined primitives are the three conjunctive factors in $\Gamma_{\text{corner}}$, whose simultaneous necessity was invisible when experiments tested them individually.
- The Wittig reaction's underdetermined primitive is the betaine/concerted flux split ($\Gamma_{\text{spleftarrow}}$), controlled by lithium salts and solvent.
...
- Bredt's Rule's underdetermined primitive is the conjunctive geometry ($\Gamma_{\text{corner}}$) that defines the boundary between forbidden and metastable.
