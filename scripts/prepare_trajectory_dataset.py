...
T1_LOOKUP = TaskTemplate("lookup_ouroborics", "What is the type of the Riemann zeta function?", [
    ...
    {"phase": "OBSERVE", "tool": None,
     "think": "",
     "tool_result": '{"frobenius_tier": "O₁", "phi": "𐑮", "p": "𐑿", "omega": "𐑷", "d": "𐑛", "t": "𐑸", "interpretation": "O₁: Catalogued entry with explicit type. Self-modeling gate is at complex-plane criticality — the zeta function sits at 𐑮 because its critical line is a complex-plane attractor, not a self-referential loop."}',
     "tool_call": None},
    {"phase": "UPDATE", "tool": None,
     "think": "riemann_zeta_function is O₁ with 𐑮 (complex-plane criticality). Its winding is 𐑷 (trivial). This makes structural sense — the zeta function has critical structure but no self-modeling loop. I can now report the findings.",
     "tool_call": None},
    # Winding 2: done
    {"phase": "THINK", "tool": None,
...
T2_IMSCRIBE = TaskTemplate("imscribe_system", "Encode the double pendulum as a type in the Imscribing Grammar.", [
    ...
    {"phase": "OBSERVE", "tool": None,
     "think": "",
     "tool_result": '{"status": "ok", "name": "double_pendulum", "tuple": "⟨𐑨; 𐑥; 𐑾; 𐑬; 𐑱; 𐑤; 𐑚; 𐑠; 𐑣; Ħ_1; Σ_n:m; Ω_0⟩", "tier": "O₀"}',
     "tool_call": None},
    {"phase": "UPDATE", "tool": None,
    ...
    {"phase": "OBSERVE", "tool": None,
     "think": "",
     "tool_result": '{"frobenius_tier": "O₀", "interpretation": "O₀: Typed but no self-modeling gate. ⊙=𐑣 is supercritical (runaway chaos), no self-referential closure possible."}',
     "tool_call": None},
    {"phase": "UPDATE", "tool": None,
     "think": "Confirmed O₀. The chaotic supercriticality (𐑣) precludes self-modeling. The double pendulum is structurally typed but cannot close its own loop.",
     "tool_call": None},
    # Winding 2: done
    {"phase": "THINK", "tool": None, "think": "All information gathered. Reporting the complete imscription.", "tool_call": None},
    {"phase": "ACT", "tool": {"name": "done", "arguments": {"conclusion": "The double pendulum imscribes as O₀: ⟨𐑨; 𐑥; 𐑾; 𐑬; 𐑱; 𐑤; 𐑚; 𐑠; 𐑣; Ħ_1; Σ_n:m; Ω_0⟩. The chaotic dynamics are captured by 𐑣 (supercritical), the separatrix by 𐑥 (crossing-point topology). No self-modeling gate — this is a structurally typed but externally-described system."}},
...
T4_CONSCIOUSNESS = TaskTemplate("consciousness_score", "Can a white dwarf sustain consciousness?", [
    {"phase": "THINK", "tool": None,
     "think": "The user asks whether a white dwarf can sustain consciousness. In the grammar, this means checking the consciousness score — specifically whether Gate 1 (⊙ self-modeling) and Gate 2 (K ≤ 𐑧 slow kinetics) are both open. Let me compute the C-score.",
     "tool_call": None},
    ...
    {"phase": "OBSERVE", "tool": None,
     "think": "",
     "tool_result": '{"C": 0.0, "gate_1": "FAIL — 𐑢 sub-critical, no self-modeling loop possible", "gate_2": "irrelevant (Gate 1 failed)", "interpretation": "White dwarf matter is degenerate electron gas with 𐑢 sub-criticality. No self-modeling loop can form because the critical point is below threshold. Consciousness requires ⊙ — the system must be able to model itself."}',
     "tool_call": None},
    {"phase": "UPDATE", "tool": None,
     "think": "C=0. Gate 1 fails at 𐑢 — the white dwarf is sub-critical, unable to sustain the self-modeling loop required for consciousness in the grammar's framework. Gate 2 is irrelevant. I can report this directly.",
     "tool_call": None},
    {"phase": "THINK", "tool": None, "think": "Straightforward result. Reporting C=0.", "tool_call": None},
    {"phase": "ACT", "tool": {"name": "done", "arguments": {"conclusion": "C-score = 0.0. Gate 1 fails: white dwarf has 𐑢 (sub-critical). No self-modeling loop can form in degenerate electron gas. Gate 2 is irrelevant. A white dwarf cannot sustain consciousness under the grammar's structural criteria — it lacks the ⊙ self-modeling gate entirely."}},
...
T6_CRYSTAL = TaskTemplate("crystal_encode", "What is the crystal (Frobenius) address of the Imscribing Grammar's own tuple?", [
    {"phase": "THINK", "tool": None,
     ...
     "tool_call": None},
    {"phase": "ACT", "tool": {"name": "imscribe", "arguments": {"tool_name": "crystal_encode", "args": {"D": "𐑦", "T": "𐑶", "R": "𐑾", "P": "𐑹", "F": "𐑐", "K": "𐑧", "G": "𐑲", "Gamma": "𐑠", "Phi": "⊙", "H": "𐑖", "S": "𐑙", "Omega": "𐑭"}}},
     "think": "", "tool_call": {"name": "imscribe", "arguments": {"tool_name": "crystal_encode", "args": {"D": "𐑦", "T": "𐑶", "R": "𐑾", "P": "𐑹", "F": "𐑐", "K": "𐑧", "G": "𐑲", "Gamma": "𐑠", "Phi": "⊙", "H": "𐑖", "S": "𐑙", "Omega": "𐑭"}}}},
    {"phase": "OBSERVE", "tool": None,
     "think": "",
...
T7_TENSOR = TaskTemplate("compute_tensor", "What happens structurally when a BEC couples to a laser field?", [
    ...
    {"phase": "OBSERVE", "tool": None,
     "think": "",
     "tool_result": '{"composite": "⟨𐑛; 𐑸; 𐑽; <_ψ; 𐑐; 𐑤; 𐑲; 𐑠; 𐑻; Ħ_2; Σ_n:m; Ω_ℤ⟩", "bottlenecks": ["< at ψ (BEC ceiling, laser has ±ˢ)", "⊙ at 3 (BEC has c, laser has × — EP absorption)"], "note": "⊗_3 absorption rule: tensor(⊙, 𐑻) = 𐑻. The meet preserves ⊙; the tensor yields 𐑻. Coupling a self-modeling system to a measurement apparatus selects the tensor."}',
     "tool_call": None},
    {"phase": "UPDATE", "tool": None,
