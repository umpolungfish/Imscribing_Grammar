...
DESIGNS = [
    ...
    },
    {
        ...
        "description": "Epigenetic reprogramming therapeutic that writes persistent chromatin state changes. Supervenience coupling (>=𐑩) — the epigenetic layer supervenes on the genetic substrate. Eternal chirality (⊥=𐑫) ensures reprogramming persists through cell division. Frobenius-closed (<=𐑹) ensures the rewrite operation is idempotent. Quantum coherent (⋈=𐑐) manipulation of methylation and histone patterns.",
        "tuple": {k:v for k,v in zip(CAT_KEYS, ['𐑦','𐑸','𐑩','𐑹','𐑐','𐑧','𐑲','∋^∧','⊙','𐑫','𐑳','𐑭'])}
    },
    {
        "name": "universal_antidote",
        "description": "Platform therapeutic that self-selects against any disease signature. All-simultaneous detection (∋=∋^∧) — every pathogen feature is processed at once via parallel molecular recognition. Stoichiometry is many-identical (Σ=𐑕) because all disease patterns share a universal grammar. Eternal memory (⊥=𐑫) means once a threat is recognized, it is never forgotten.",
        "tuple": {k:v for k,v in zip(CAT_KEYS, ['𐑦','𐑸','𐑾','𐑹','𐑐','𐑧','𐑲','∋^∧','⊙','𐑫','𐑕','𐑭'])}
    },
    {
        "name": "ouroboric_composite",
        "description": "Structural material that senses damage and self-heals via an internal ouroboric loop. Self-referential topology (⊣=𐑶) — the material's response is a function of its own damage state. Trapped-ordered kinetics (⊤=𐑺) — healing agents are stored in an ordered but activatable reservoir. Eternal memory (⊥=𐑫) ensures the material remembers every damage event across its lifetime.",
        "tuple": {k:v for k,v in zip(CAT_KEYS, ['𐑦','𐑶','𐑾','𐑹','𐑐','𐑺','𐑲','∋^∧','⊙','𐑫','𐑳','𐑭'])}
    },
    {
    ...
    },
    {
        ...
        "description": "Smart fabric that weaves all functions — sensing, actuation, communication, energy harvesting — simultaneously into a single textile. All-simultaneous composition (∋=∋^∧). Moderate kinetics (⊤=𐑤) — the fabric responds at human-relevant timescales. Self-written microarchitecture (⊢=𐑦) — each thread contains its own knitting pattern generator.",
        "tuple": {k:v for k,v in zip(CAT_KEYS, ['𐑦','𐑸','𐑾','𐑹','𐑐','𐑤','𐑲','∋^∧','⊙','𐑫','𐑳','𐑭'])}
    },
    {
        "name": "ouroboric_cell",
        "description": "Synthetic cell whose genome writes and rewrites itself in response to environmental signals. Self-referential topology (⊣=𐑶) — the genome encodes its own modification machinery. Frobenius-closed rewrite (<=𐑹) — each genome edit is a fixed point of the edit operation. Self-written dimensionality (⊢=𐑦) — the genome is its own state space. Eternal memory (⊥=𐑫) — no information is lost across generations.",
        "tuple": {k:v for k,v in zip(CAT_KEYS, ['𐑦','𐑶','𐑾','𐑹','𐑐','𐑧','𐑲','∋^∧','⊙','𐑫','𐑳','𐑭'])}
    },
    {
        "name": "quantum_bioelectric_tissue",
        "description": "Engineered tissue that uses quantum-coherent bioelectric fields to guide regeneration. Bidirectional feedback (>=𐑾) — electric field and tissue state mutually determine each other. Quantum coherent ion channels (⋈=𐑐) maintain coherence at tissue scale. Eternal chirality (⊥=𐑫) ensures the bodyplan blueprint is never lost during regeneration.",
        "tuple": {k:v for k,v in zip(CAT_KEYS, ['𐑦','𐑸','𐑾','𐑹','𐑐','𐑧','𐑲','∋^∧','⊙','𐑫','𐑳','𐑭'])}
    },
    {
        "name": "universal_symbiont",
        "description": "Consortium of 12 engineered microbial strains that collectively provide all metabolic support functions. Many-heterogeneous stoichiometry (Σ=𐑳) — 12 distinct strains with complementary functions. All-simultaneous composition (∋=∋^∧) — all metabolic pathways active at once. Long-range signaling (∈=𐑲) via quorum-sensing molecules that coordinate across the entire consortium.",
        "tuple": {k:v for k,v in zip(CAT_KEYS, ['𐑦','𐑸','𐑾','𐑹','𐑐','𐑧','𐑲','∋^∧','⊙','𐑫','𐑳','𐑭'])}
    },
    {
