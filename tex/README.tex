...
\title{Imscribing Grammar --- Lean 4 Formalization}
\date{\today}
...
Defines the 12 Imscribing Grammar primitives as inductive types with \texttt{deriving DecidableEq, Repr, Ord},
...
B & \texttt{\Omega \textgreater{}= \Omega\_Z -\textgreater{} H \textgreater{}= Ħ_A} (integer winding number requires persistent chirality) \\
C & \texttt{D\_odot \textless{}-\textgreater{} T\_odot} (imscriptive dimensionality iff imscriptive topology) \\
D & \texttt{\Omega\_NA -\textgreater{} D\_odot} (non-Abelian anyonic protection requires imscriptive substrate) \\
\bottomrule
\end{tabularx}
\end{table}
...
between the two files. The connection is \textbf{conceptual and meta-level}: the Imscribing Grammar
...
\begin{tabularx}{\textwidth}{>{\centering\arraybackslash}X >{\centering\arraybackslash}X}
\toprule
\rowcolor{gray!25}\textbf{OPN concept} & \textbf{Imscribing Grammar analogue} \\
\midrule
\rowcolors{1}{white}{gray!10}
...
The four \texttt{axiom} declarations are \textbf{foundational postulates} of the Imscribing Grammar framework,
...
no path to deriving, say, \texttt{D\_odot\_iff\_T\_odot} from first principles, because the types carry
...
\emph{imscription} --- a tuple in the 12-dimensional primitive space --- and the constraint propagation
