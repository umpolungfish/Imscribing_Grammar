...
The load-bearing primitive is $\Omega_{\text{dzlig}}$. It is the only temporal primitive whose removal degrades ouroboricity tier --- from $O₂^\dagger$ to $O₁$ --- and the only one whose removal produces a large C-score drop (0.828 $\rightarrow$ 0.644, a 22\% reduction). Its diagonal distance to time is the \emph{smallest} of all five peels (1.67), yet its Mahalanobis distance (3.59) is large because of the $\Omega$-metric weight. $\Omega_{\text{dzlig}}$ is the primitive that most quietly removes the most: the one that shatters tier membership with the smallest naïve footprint.
...
TIME.md \pilcrow{}8 identified the wall from $O₂^\dagger$ to $O_{\infty}$ as the $P_{\text{aolig}} \to P_{\text{doublebarpipe}}$ promotion: distance 4.38, $\delta = 4$, the largest jump in the entire tier gap ladder. The partial-type analysis adds the mechanism that makes this wall permanent.
...
\emph{Crystal address 3,928,019. $O₂^\dagger$. Five primitives. One fixed point.}
