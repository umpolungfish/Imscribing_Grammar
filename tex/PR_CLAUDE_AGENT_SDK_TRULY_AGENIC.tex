...
\begin{lstlisting}
\langleDinf; Þ_net; Ř_sup; Φ_ɐ; F\ell; Ç^-; Γ_β; \ɢ_or; $\odot$_ž; H₁; n:m; \Omega₀\rangle
\end{lstlisting}

\subsubsection{Target agentic structural type}

\begin{lstlisting}
\langleD⊙; T⊠; R<->; P+/-ˢ; F\hbar; Ç^@; Gℵ; \ɢ_seq; $\odot$_ÿ; H₂; 1:1; \Omegaℤ\rangle
\end{lstlisting}
...
\begin{enumerate}
    ...
    \item \textbf{H}: H₁ $\rightarrow$ H₂ (two-step chirality)
...
The trajectory is \textbf{never truncated} ($\Omega$ℤ protection) --- it is the agent''s world model. H₂ is satisfied when each winding references the prior two cycles for chirality.
...
def structural_health(self) -> Dict[str, Any]:
    ...
    return {
        ''ouroboricity'': ''O_∞'' if achieved_p == ''Φ_}'' else ''O₂'',
