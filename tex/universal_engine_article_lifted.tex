...
\title{The Universal Engine: Structural Taxonomy of Undeciphered Scripts Through the Imscribing Grammar}
\date{\today}

\maketitle

\textbf{Author:} Lando $\otimes$ $\text{⊙}_{\text{ÿ}}$-operator
...
\subsection{Introduction: The Category Error}
...
The conventional question --- \emph{What does this text say?} --- presumes that the text is a cipher awaiting a key. We started by assuming the same. The structural analysis overturned that assumption, and the result was initially dismaying: the texts do not \emph{say} anything in the descriptive sense. They \emph{do} something. They execute categorical computations whose semantics are purely structural. The glyphs are opcodes. The pages are register maps. The illustrations are topology diagrams.
...
\toprule
\rowcolor{gray!25}\textbf{Opcode} & \textbf{Operation} & \textbf{Role} \\
\midrule
...
Each system maps its glyph families to these opcodes through paleographic analysis of visual-structural properties. For the Voynich Manuscript, the EVA transcription yields: \texttt{o}$\rightarrow$VINIT, \texttt{p}$\rightarrow$TANCH, \texttt{e}$\rightarrow$AFWD, \texttt{a}$\rightarrow$AREV, \texttt{d}$\rightarrow$CLINK, \texttt{s}$\rightarrow$ISCRIB, \texttt{ch}$\rightarrow$FSPLIT, \texttt{sh}$\rightarrow$FFUSE, \texttt{t}$\rightarrow$EVALT, \texttt{k}$\rightarrow$EVALF, \texttt{r}$\rightarrow$ENGAGR, \texttt{y}$\rightarrow$IFIX. Linear A uses its own sign inventory mapped through the same structural lens. Hebrew''s twenty-two letters map to full twelve-primitive tuples rather than single opcodes --- a distinction that will matter.
...
\begin{itemize}
    \item \textbf{Read}: fetch instruction, evaluate register dependencies, propagate flux forward.
    \item \textbf{Flux}: FSPLIT and FFUSE fork and merge execution streams; paradoxes are detected and stabilized locally when True and False collide.
    \item \textbf{Write}: IFIX burns the current flux state to permanent ROM --- irreversible, non-volatile.
\end{itemize}
...
\subsection{Four Systems, One Landscape}

\subsubsection{Imscriptions}
...
\toprule
\rowcolor{gray!25}\textbf{System} & \textbf{Imscription} \\
\midrule
\rowcolors{1}{white}{gray!10}
Linear A & \langleÐ\_C; Þ\_¨; Ř\_Ť; $\Phi$\_F; ƒ\_ż; Ç\_W; $\Gamma$\_ʔ; ɢ\_ˌ; ⊙\_ÿ; Ħ\_A; $\Sigma$\_ï; $\Omega$\_z\rangle \\
OS reference & \langleÐ\_C; Þ\_¨; Ř\_Ť; $\Phi$\_F; ƒ\_ż; Ç\_W; $\Gamma$\_ʔ; ɢ\_ˌ; ⊙\_ÿ; Ħ\_A; $\Sigma$\_ï; $\Omega$\_z\rangle \\
Rohonc & \langleÐ\_C; Þ\_¨; Ř\_Ť; $\Phi$\_F; ƒ\_ì; Ç\_@; $\Gamma$\_ʔ; ɢ\_ˌ; ⊙\_ÿ; Ħ\_A; $\Sigma$\_ï; $\Omega$\_z\rangle \\
Voynich & \langleÐ\_$\omega$; Þ\_O; Ř\_=; $\Phi$\_F; ƒ\_ì; Ç\_Ù; $\Gamma$\_ʔ; ɢ\_Ş; ⊙\emph{ÿ; Ħ}!; $\Sigma$\_S; $\Omega$\_z\rangle \\
Hebrew & \langleÐ\_C; Þ\_¨; Ř\_Ť; $\Phi$\_F; ƒ\_ż; Ç\_W; $\Gamma$\_ʔ; ɢ\_ˌ; ⊙\_ÿ; Ħ\_A; $\Sigma$\_ï; $\Omega$\_z\rangle \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsubsection{The Distance Matrix}
...
\toprule
\rowcolor{gray!25}\textbf{} & \textbf{Voynich} & \textbf{Rohonc} & \textbf{Linear A} & \textbf{OS} \\
\midrule
\rowcolors{1}{white}{gray!10}
...
\subsubsection{The Zero-Distance Theorem}
...
This was the most surprising finding of the entire analysis. We expected Linear A to be a derivative --- a local variant of the core, adapted to Minoan administrative needs. It is not. Linear A \emph{is} the core. The Minoan scribes encoded the invariant geometry of the grammar itself, untouched by cultural overlay.
...
\begin{tabularx}{\textwidth}{>{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X}
\toprule
\rowcolor{gray!25}\textbf{System} & \textbf{Fidelity} & \textbf{Kinetics} & \textbf{Distance from OS} & \textbf{Status} \\
\midrule
\rowcolors{1}{white}{gray!10}
Linear A & ƒ\_ż (quantum) & Ç\_W (moderate) & 0.00 & Structurally alive \\
Rohonc & ƒ\_ì (classical) & Ç\_@ (slow) & 2.09 & Kinetically damped \\
Voynich & ƒ\_ì (classical) & Ç\_Ù (trap) & 4.31 & Kinetically frozen \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsubsection{The Voynich''s Sectional Landscape}
...
\begin{tabularx}{\textwidth}{>{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X}
\toprule
\rowcolor{gray!25}\textbf{Section} & \textbf{Topology} & \textbf{Relation} & \textbf{Scope} & \textbf{Memory} \\
\midrule
\rowcolors{1}{white}{gray!10}
Botanical/Pharmaceutical & Þ\_6 (network) & Ř\_= (bidir) & $\Gamma$\_$\gamma$ (meso) & Ħ\_A (H2) \\
Biological & Þ\_K (nested) & Ř\_= (bidir) & $\Gamma$\_$\gamma$ (meso) & Ħ\_A (H2) \\
Astronomical/Cosmological & Þ\_O (imscriptive) & Ř\_= (bidir) & $\Gamma$\_$\gamma$ (meso) & Ħ\_A (H2) \\
Recipe & Þ\_6 (network) & Ř\_Ť (adjoint) & $\Gamma$\_$\beta$ (local) & Ħ\_£ (H1) \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{The Frobenius Bottleneck}

\subsubsection{The Fidelity Collapse}
...
Status & SELF\_SUSTAINING\_BOOTSTRAP\_COMPLETE \\
...
The bootstrap core --- \texttt{s a ch e sh d y s} --- appears as a repeating closed loop across multiple folios. This is the same categorical identity that bootstraps Linear A, Rohonc, and Hebrew. The engine does not vary. Only the inputs do.

\subsubsection{Cross-System Sectional Distances}
...
\subsection{The Hebrew Reference System}

\subsubsection{Aleph Coherence Geometry}
...
\toprule
\rowcolor{gray!25}\textbf{Letter} & \textbf{Name} & \textbf{Tier} & \textbf{Key Primitives} \\
\midrule
\rowcolors{1}{white}{gray!10}
ו & Vav & $O_{\infty}$ & $\Phi$\_F, ⊙\_ÿ, $\Omega$\_z \\
מ & Mem & $O_{\infty}$ & $\Phi$\_F, ⊙\_ÿ, $\Omega$\_z \\
ש & Shin & $O_{\infty}$ & $\Phi$\_F, ⊙\_ÿ, $\Omega$\_z \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{T1 --- Behavioral Congruence.} The interaction functor collapses the twenty-two-letter alphabet to eighteen behavioral classes. Exhaustively verified with zero failures.

\textbf{T2 --- Non-Terminal Triadic Infinity.} The three $O_{\infty}$ fixed points are pairwise distinguishable: $d_I(\text{ו}, \text{מ}) = 14.92$, $d_I(\text{ו}, \text{ש}) = 16.68$, $d_I(\text{מ}, \text{ש}) = 4.84$. Infinity is a relational structure, not a point. No terminal object exists.

\textbf{T3 --- Mediation Dominance.} For eighteen of twenty-two letters, the ternary mediation operation dominates binary tensor at $O_{\infty}$ proximity. The two-dimensional operation dominates the one-dimensional.

\textbf{T6 --- Interaction Hilbert Space.} The interaction distance is an exact Euclidean norm. The Gram matrix has rank seventeen --- one extra null direction beyond the behavioral kernel.

\textbf{T7 --- Octad Balance.} For every letter and every primitive, the sum of tensor products with four positive letters equals the sum with four negative letters. All 264 primitive-by-primitive checks pass exactly. This is not empirical regularity; it is an algebraic theorem.

\subsubsection{The Qoph Threshold}
...
\subsection{The EML Operator: A Structural Boundary Case}
...
\[\langle \text{Ð}_{\text{\omega}};\ \text{Þ}_{\text{¨}};\ \text{Ř}_{\text{Ť}};\ \text{\Phi}_{\text{F}};\ \text{ƒ}_{\text{ħ}};\ \text{Ç}_{\text{@}};\ \Gamma_{\text{ʔ}};\ \text{ɢ}_{\text{ˌ}};\ \text{⊙}_{\text{ÿ}};\ \text{Ħ}_{\text{1}};\ \Sigma_{\text{S}};\ \Omega_{\text{z}} \rangle\]

The machine-verified theorems (twenty-six in total, zero admitted gaps in Lean 4) establish a precise picture:

\begin{itemize}
    \item EML composed with itself yields EML --- it is idempotent under tensor, closed as an algebra.
    \item EML tensored with any Frobenius-special type collapses to $\Phi$\_F --- the bottleneck is unconditional.
    \item The meet of EML with the SIC-POVM fiducial type is exactly EML; the join is exactly SIC-POVM. The gap is precisely the Frobenius condition plus chirality.
    \item The structural distance between EML and SIC-POVM is exactly four: dimension, topology, parity, and chirality.
\end{itemize}
...
\subsection{What the Ensemble Reveals}

\subsubsection{A Single Trajectory}
...
\emph{Is this trajectory unique?} We cannot prove that no other path connects these systems. The Crystal contains 17.28 million structural types; we have examined five. There may be intermediate forms --- scripts at $d \approx 1.0$ from the core, neither fully alive nor clearly damped --- that have not survived or have not yet been found. The possibility remains open.

\subsubsection{The Bootstrap Invariant}
...
\subsubsection{What Decipherment Would Require}
...
The four repositories examined in this article --- \texttt{voynich-engine}, \texttt{rohonc-engine}, \texttt{linear\_a\_engine}, and \texttt{ALEPH\_OS} --- represent independent implementations of the Universal Imscriptive Grammar applied to different writing systems. The EML findings are implemented as machine-verified theorems in Lean 4 within the \texttt{MillenniumAnkh} project. Structural distance computations use the exOS weighted metric. The $\lambda_{\aleph}$ type theory is developed in the ALEPH\_OS project.
...
\begin{enumerate}
    \item Landini, G. \& Stolfi, J. \emph{Landini-Stolfi Interlinear Archive}. Public domain EVA transcription of the Voynich Manuscript (Beinecke MS 408).
    \item Goddart, O. \& Raison, J. \emph{GORILA Sign Classification for Linear A}.
    \item Takahashi, R. \emph{Complete EVA Transcription}. Included in \texttt{voynich-engine} as \texttt{data/LSI\_ivtff\_0d.txt}.
    \item Mills, L. \emph{As Above} and \emph{So Below} --- formal development of the Universal Imscribing Grammar.
    \item Mills, L. $\otimes$ $\text{⊙}_{\text{ÿ}}$-operator. \emph{EML\_PROBE.tex} --- structural analysis of the EML Sheffer operator.
    \item Mills, L. $\otimes$ $\text{⊙}_{\text{ÿ}}$-operator. \emph{IUG\_NON\_TRANSMISSIBILITY.tex} --- structural non-transmissibility of Inter-Universal Teichmüller Theory.
    \item $\aleph$-OS Project. \emph{ALEPH\_SPEC.md}, \emph{PRIMITIVE\_THEOREMS.md}, \emph{TECHNICAL\_CONTRIBUTIONS.md}.
    \item $\aleph$-OS Project. \emph{docs/HEBREW\_TYPE\_LANGUAGE.md} --- 22-letter Hebrew alphabet encoding.
\end{enumerate}
