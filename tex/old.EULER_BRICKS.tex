...
\textbf{Ouroboricity Tier:} $O_0$ (no self-referential critical loop survives --- $\Phi_\text{EP}$ alone without $\Phi_c$ does not gate open)
...
\subsubsection{The $\Phi_\text{EP}$ Interpretation}
...
\begin{enumerate}
    ...
    \item \textbf{Absent topological protection}: $\Omega_0$ confirms no invariant guarantees resolution.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Pythagorean Triples} --- $O_0$, $\Phi_\text{sub}$, $K_\text{fast}$: a rapidly generable, fully parameterized family with no critical structure.
    \item \textbf{Euler Bricks} --- $O_0$, $\Phi_\text{sub}$, $K_\text{slow}$: a constrained composite of Pythagorean triples where $K_\text{slow}$ emerges from the conjunctive coupling of three triple constraints. The tensor product with Pythagorean triples confirms the brick's structural dominance.
    \item \textbf{Perfect Cuboid} --- $O_0$, $\Phi_\text{EP}$, $K_\text{trap}$, $H_{\infty}$: the exceptional point where the fourth (space diagonal) constraint pushes the system to a non-Hermitian bifurcation. The Frobenius self-dual structure ($P_{\pm}^{\text{sym}}$) is the only element of elevated complexity.
\end{enumerate}
