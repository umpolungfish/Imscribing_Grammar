...
The ouroborics tier of this encoding is $O₁$ with consciousness score $C = 0.273$. This encoding captures the \emph{computational} nature of the conjecture: a deterministic algorithm applied to each input. It does not, however, capture the \emph{self-referential} structure necessary for proof.
...
\begin{itemize}
    \item \textbf{Ouroborics of shallow encoding:} $O₁$ tier (computed via \texttt{ouroborics("collatz\_conjecture")})
