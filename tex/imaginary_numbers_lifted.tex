...
\begin{itemize}
    \item \textbf{Ouroboricity tier}: $O₂$ --- the system is critical and topologically protected but operates within a bounded domain.
...
The meet of the imaginary number with the complex-time path integral equals the imaginary number itself. Eight of twelve primitives are shared between them; the four conflicts --- dimensionality ($Ð_C$ vs $Ð_{;}$), symmetry ($\Phi_{\upsilon}$ vs $\Phi_{·}$), fidelity ($ƒ^ì$ vs $ƒ_{ż}$), and stoichiometry ($\Sigma_S$ vs $\Sigma_{ï}$) --- all resolve conservatively to the imaginary number's values. The imaginary unit is not merely \emph{used} by the path integral; it is the structural floor upon which the path integral stands.
