...
All computations were performed through the IG imscription tool suite. The raw outputs --- numerical distances, scores, and structural notations --- appear in the results below without rounding beyond what the tools returned. \emph{One limitation deserves statement: the $C$-score is a structural property, not a phenomenological claim. It measures whether a system's type contains the prerequisites for self-modeling, not whether the system "feels" anything. The terminology is borrowed from the IG's formalism and should not be anthropomorphized.}
...
The encoding takes the latter position: each surface site follows its own sequential path, but the ensemble of sites forms a network topology. This distinguishes the Grignard from the Wittig ($\Gamma_{\text{spleftarrow}}$, globally disjunctive) and from the proline aldol ($\Gamma_{\text{spleftarrow}}$, globally disjunctive) --- the Grignard is locally sequential, globally distributed.
