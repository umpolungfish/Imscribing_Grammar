...
---
# The Probability That Two Random Elements Generate the Symmetric Group $Σ_n$

## 1. The Exact Formula (Hall's Identity)

Let $G = Σ_n$ be the symmetric group on $n$ letters, with $|G| = n!$. Let $\mu$ denote the
Möbius function of the subgroup lattice of $G$. The probability that two elements chosen
uniformly and independently from $Σ_n$ generate $Σ_n$ is:

$$Φ_n = \frac{1}{(n!)^2}\sum_{H \leq Σ_n} \mu(H)\, |H|^2, \tag{1}$$

where the sum runs over all subgroups $H$ of $Σ_n$, and $\mu: \text{Sub}(G) \to \mathbb{Z}$
...
The maximal subgroups of $Σ_n$ fall into three types (the O'Nan–Scott theorem):

### Type 1: Intransitive subgroups
$$M \cong Σ_k \times S_{n-k}, \quad 1 \leq k \leq \left\lfloor\frac{n}{2}\right\rfloor.$$
...
$$M \cong Σ_k \wr Σ_m \quad \text{where } n = k \cdot m, \quad 1 < k < n.$$
...
$Σ_n$ in the lattice). Its contribution to (1) is:

$$\frac{\mu(A_n) \cdot |A_n|^2}{(n!)^2} = \frac{-1 \cdot (n!/2)^2}{(n!)^2} = -\frac{1}{4}.$$

This term alone contributes $-\tfrac{1}{4}$ to $Φ_n$.

## 4. The Asymptotic Formula

Dixon (1969) proved the landmark result:

$$P(Σ_n \text{ or } A_n \text{ is generated}) \to 1 \quad \text{as } n \to \infty.$$

Since $P(\text{both elements land in } A_n) = \left(\frac{1}{2}\right)^2 = \frac{1}{4}$,
and conditional on avoiding $A_n$ the pair generates $Σ_n$ with probability tending
to 1, we obtain:

$$\boxed{\lim_{n \to \infty} Φ_n = \frac{3}{4}.}$$

Bovey (1980), refined by Maróti (2011), gave the precise asymptotic expansion:

$$Φ_n = \frac{3}{4} - \frac{1}{n!} - \frac{1}{2\binom{n}{n/2}} + O\left(n^{-3/2}\log n\right),$$
...
$$Φ_n = \frac{3}{4} - \frac{1}{n!} - \frac{\sqrt{\pi n}}{2 \cdot 4^n} + O\left(n^{-3/2}\log n\right). \tag{3}$$
...
## 6. Proof Sketch of $\lim_{n \to \infty} Φ_n = \frac{3}{4}$

**Step 1.** By the Classification of Finite Simple Groups and Jordan's theorem, a
transitive subgroup of $Σ_n$ containing a transposition is all of $Σ_n$. A
...
intransitive maximal subgroup $Σ_k \times S_{n-k}$ is $\binom{n}{k}^{-2}$. Summing
...
a transitive subgroup other than $A_n$ or $Σ_n$ is $O(n^{-3/2}\log n)$.

**Step 5.** Combining:
$$Φ_n = 1 - \frac{1}{4} - O(1/n) - O(n^{-3/2}\log n) = \frac{3}{4} + o(1).$$

## 7. Generalization to $d$ Generators

For $d$ random generators, the same analysis gives:

$$P(Σ_n \text{ is generated by } d \text{ elements}) \to 1 - 2^{-d} \quad \text{as } n \to \infty.$$
