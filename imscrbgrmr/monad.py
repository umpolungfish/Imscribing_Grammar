"""
ImscriptionM — the Imscribing Grammar monad transformer stack.
...
Strategy combinators:
    ...
    optimize(imscription, strats) first-success search (asum)

See IG_LANG.md §Phase 3a for design rationale.
"""
...
@dataclass
class ImscriptionM(Generic[A]):
    """
    The Imscribing Grammar monad.
    ...
    def bind(self, f: "Callable[[A], ImscriptionM[B]]") -> "ImscriptionM[B]":
        ...
        result: ImscriptionM[B] = f(self.value)
        return ImscriptionM(
            value=result.value,
    ...
    def __rshift__(self, f: "Callable[[A], ImscriptionM[B]]") -> "ImscriptionM[B]":
        """Operator alias: m >> f  ≡  m.bind(f)."""
        return self.bind(f)

    @classmethod
    def mzero(cls) -> "ImscriptionM[A]":
    ...
    def mplus(self, other: "ImscriptionM[A]") -> "ImscriptionM[A]":
        """
    ...
    def __or__(self, other: "ImscriptionM[A]") -> "ImscriptionM[A]":
...
def _load(name: str) -> Imscription:
    """Load a imscription from the global catalog by name. Raises KeyError if absent."""
...
def _imscription_from_lattice(result, name: str) -> Imscription:
    """
    Build a Imscription from a LatticeResult.
    Fields that are the CONFLICT sentinel are replaced with the source imscription's value.
    """
    ...
    # get a fallback imscription (s1) from the result
    try:
...
def _imscription_from_tensor(result, name: str) -> Imscription:
    """Build a Imscription from a TensorResult."""
    return Imscription(
        name=name,
...
def join_m(other_name: str) -> "Callable[[Imscription], ImscriptionM[Imscription]]":
    """Lift algebra.join into ImscriptionM."""
    def _step(imscription: Imscription) -> ImscriptionM[Imscription]:
        from .algebra import join
        try:
            other = _load(other_name)
            result = join(imscription, other)
            if result.conflicts:
                rec = StepRecord(
            ...
            new_s = _imscription_from_lattice(result, f"join({imscription.name},{other_name})")
            ...
            return ImscriptionM(value=new_s, cost=0.0, context=ctx, log=[rec])
...
def meet_m(other_name: str) -> "Callable[[Imscription], ImscriptionM[Imscription]]":
    """Lift algebra.meet into ImscriptionM."""
    def _step(imscription: Imscription) -> ImscriptionM[Imscription]:
        from .algebra import meet
        try:
            other = _load(other_name)
            result = meet(imscription, other)
            if result.conflicts:
                rec = StepRecord(
                ...
                return ImscriptionM(value=None, cost=0.0, context=Context(), log=[rec])
            new_s = _imscription_from_lattice(result, f"meet({imscription.name},{other_name})")
            rec = StepRecord(
                "meet", other_name, "PASS", 0.0,
                f"Meet succeeded  notes: {'; '.join(result.notes[:2])}"
            )
            return ImscriptionM(value=new_s, cost=0.0, context=Context(step_count=1), log=[rec])
        except Exception as e:
...
def tensor_m(
    other_name: str,
    lambda_: float = 0.3,
) -> "Callable[[Imscription], ImscriptionM[Imscription]]":
    """Lift algebra.tensor into ImscriptionM."""
    def _step(imscription: Imscription) -> ImscriptionM[Imscription]:
        from .algebra import tensor
        try:
            other = _load(other_name)
            result = tensor(imscription, other, lambda_=lambda_)
            new_s = _imscription_from_tensor(result, f"tensor({imscription.name},{other_name})")
            ...
            fidelity_name = getattr(new_s.fidelity, "value", str(new_s.fidelity)) if new_s.fidelity else None
            ctx = Context(f_floor=fidelity_name, step_count=1)
            return ImscriptionM(value=new_s, cost=xi_ens, context=ctx, log=[rec])
        except Exception as e:
...
def lift_m(target: str) -> "Callable[[Imscription], ImscriptionM[Imscription]]":
    """Lift algebra._LIFT_MAP[target] into ImscriptionM."""
    def _step(imscription: Imscription) -> ImscriptionM[Imscription]:
        ...
        try:
            ...
            if not result.applicable:
                ...
                return ImscriptionM(value=None, cost=0.0, context=Context(), log=[rec])
            new_s = result.imscription or imscription
            ctx = Context(
...
def path_m(
    ...
    """Lift algebra.find_path into ImscriptionM."""
    def _step(imscription: Imscription) -> ImscriptionM[Imscription]:
...
def assert_m(
    predicate: "Callable[[Imscription], bool]",
    message: str = "",
) -> "Callable[[Imscription], ImscriptionM[Imscription]]":
    """
    Inline proof obligation.
    Passes the imscription through unchanged if predicate(imscription) is True.
    Fails (value=None) otherwise — the computation is blocked.
    """
    def _step(imscription: Imscription) -> ImscriptionM[Imscription]:
        try:
            ok = bool(predicate(imscription))
        except Exception as e:
        ...
        if ok:
            ...
            return ImscriptionM(value=imscription, cost=0.0, context=Context(step_count=1), log=[rec])
        else:
            rec = StepRecord(
...
# A DesignStrategy is a function from Imscription to a monadic computation.
DesignStrategy = Callable[[Imscription], ImscriptionM[Imscription]]


def strategy_then(s1: DesignStrategy, s2: DesignStrategy) -> DesignStrategy:
    """Sequential composition: s1 then s2  (do { x <- s1; s2 x })."""
    return lambda imscription: ImscriptionM.return_(imscription).bind(s1).bind(s2)


def strategy_or(s1: DesignStrategy, s2: DesignStrategy) -> DesignStrategy:
    """Alternative: try s1; fallback to s2 if s1 fails  (s1 <|> s2)."""
    return lambda imscription: (
        ImscriptionM.return_(imscription).bind(s1)
        .mplus(ImscriptionM.return_(imscription).bind(s2))
    )
...
def optimize(
    ...
    """
    result: ImscriptionM[Imscription] = ImscriptionM.mzero()
    for strat in strategies:
        result = result.mplus(ImscriptionM.return_(imscription).bind(strat))
