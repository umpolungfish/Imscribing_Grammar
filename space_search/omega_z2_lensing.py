"""
𐑴 Lensing Signature Test.

Tests for topological-protection (𐑴) modifications to the weak
lensing shear field. Standard lensing (𐑷) predicts:
  - Curl-free shear (E-mode only, B-mode = 0 from systematics)
  - Parity-symmetric B-modes

𐑴 modification predicts:
...
def simulate_shear_catalog(n_galaxies: int = 50000,
    ...
    omega_z2_signal: amplitude of injected 𐑴 B-mode signal (0 = null).
    ...
    # 𐑴 B-mode injection: curl component with Z2 parity
...
def compute_eb_aperture(cat: dict, theta_min: float = 0.5,
    ...
    B-modes from 𐑴: correlated pattern, non-zero parity asymmetry.
...
def parity_test(cat: dict) -> dict:
    """
    Parity asymmetry test for 𐑴 signature.

    Under a 45° rotation, spin-2 shear transforms as e1 → -e2, e2 → e1.
    For a true 𐑴 B-mode: B_rot ≈ -B_orig  (anti-correlated).
    ...
    Uses scipy t-test on B_sum = B_orig + B_rot:
      𐑴 → B_sum ≈ 0, t-test p >> 0.05 (consistent with zero).
    ...
    # Anti-correlation: r(B_orig, -B_rot) — target +1 for 𐑴
    ...
    # 𐑴 candidate: strong anti-correlation + B_sum consistent with 0
...
def run_lensing_test(near_pulsars: bool = True,
    ...
    """
    Run the 𐑴 lensing test.
    near_pulsars: test fields centered on P-74 neutron star positions.
    inject_signal: inject synthetic 𐑴 signal for validation (0 = real test).
    """
    print("=== 𐑴 Lensing Test ===")
    if inject_signal > 0:
        print(f"  [VALIDATION MODE] Injecting 𐑴 signal = {inject_signal}")
    ...
    for name, ra, dec, notes in fields:
        ...
        flag = "*** 𐑴 CANDIDATE ***" if candidate else ""
    ...
    print(f"\n  𐑴 candidates: {n_candidates}/{len(field_results)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inject", type=float, default=0.0,
                        help="Inject synthetic 𐑴 signal (e.g. 0.01)")
