...
*   **Framework Tools:** `imscribe analogies --stoichiometry-aware --allow-defect-fraction 0.25`; `imscribe criticality-probe` to confirm $\Phi_{\text{sub}}$ before swap.
...
3.  Do not attempt to hotswap speculative synthons into grounded molecular systems. The semantic contamination risk (Fix 5 in IG_FIXES.MD) may corrupt catalog integrity and prediction accuracy.
