...
**IG_ENSEMBLER** verifies the compatibility of multi-synthon systems. Every case in the standard validation table is a *single* synthon. Real systems are composed: a rotaxane is $R_{\Leftrightarrow}$ *plus* an H-bonding station *plus* a stopper.
...
3.  Do not attempt to assemble speculative ensembles with grounded molecular systems. The semantic contamination risk (Fix 5 in IG_FIXES.MD) may corrupt catalog integrity and prediction accuracy.
...
*   **Integration:** Multi-component systems must pass Ensembler pairwise check before IG_HOTSWAP.md candidate screening. Emergent $\Phi_c$ (score ≥ 0.70) at ensemble level requires Varma probe on the *assembled* system, not individual components.
