#!/usr/bin/env python3
"""
ruleset_dialect.py — Alternate dialect explorer for the Imscribing Grammar.
...
distinct dialects over the same 17,280,000-address Crystal of Types.
...
Usage
    uv run ruleset_dialect.py profile           # profile all predefined rulesets
    uv run ruleset_dialect.py profile --name canonical
    uv run ruleset_dialect.py compare           # side-by-side fingerprint table
"""
...
# Canonical T-constitution: T = lim(Φ, ⋈, ⊤, ⊥, Ω)
...
_T_CANONICAL: Dict[str, Tuple[str, bool]] = {
    ...
    "◻": ("𐑭", False),  # winding must reach integer level
}
...
_DEFAULT_ABSORPTION: Tuple[AbsorptionRule, ...] = (
    ...
    AbsorptionRule("⊞", "𐑳", ("tensor",)),                   # n:m absorbs under tensor
)
...
class Ruleset:
    """
    A complete composition ruleset defining a dialect over the Crystal of Types.

    g1/g2/g3        — three gate specs (GateSpec)
    gate_ordering   — True: G2 requires G1, G3 requires G2 (sequential stages)
                      False: all three gates independent (parallel dialect)
...
RULESETS: List[Ruleset] = [
    ...
    Ruleset(
        ...
        g2=GateSpec("<", 5.0),   # Φ=𐑹 = Frobenius
        g3=GateSpec("◻", 3.0),   # Ω=𐑭 = winding seal
    ),

    Ruleset(
        name="no_ordering",
        description="All three gates fully independent — parallel dialect. "
    ...
    Ruleset(
        ...
        description="Time constituted by structural/geometric primitives: "
                    "T=lim(⊢,⊣,Ř,ɢ,⊙) instead of the dynamic T=lim(Φ,ƒ,Ç,Ħ,Ω). "
...
def dialect_profile(ruleset: Ruleset, catalog: List[dict]) -> dict:
    """Compute structural fingerprint of a dialect over the given catalog."""
...
def print_profile(prof: dict, verbose: bool = True) -> None:
    ...
    if verbose and prof["o_inf_entries"]:
        ...
        print("  O_∞ entries: none — O_∞ is structurally inaccessible in this dialect")


def print_comparison(profiles: List[dict]) -> None:
    print(f"\n{'Dialect':<22} {'plain':>5} {'frob':>5} {'traced':>6} {'O_∞':>5}  "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Alternate dialect explorer — permute IG composition rulesets"
    )
    ...
    if args.cmd == "profile":
        ...
        if args.name:
            ...
            if not targets:
                ...
                sys.exit(1)
        for r in targets:
            prof = dialect_profile(r, catalog)
            print_profile(prof)
        print()

    elif args.cmd == "compare":
        profiles = [dialect_profile(r, catalog) for r in RULESETS]
