#!/usr/bin/env python3
"""
quantum_operator.py — Quantum Computation ⊙perator (Fibonacci anyon QC, SIC-POVM, IMASM↔Quantum translation).

Launches a TrueAgenticAgent with the quantum-computation-specialist system prompt.
...
Usage:
    ...
    uv run agents/specialists/quantum_operator.py --model claude-opus-4 "Verify the d=2048 Stark unit via Gauss sums"
    uv run agents/specialists/quantum_operator.py --continue "Extend the Jones polynomial cross-check"
    uv run agents/specialists/quantum_operator.py --session-id 20250315T120000Z-abc12345 "Analyze the Belnap QFT finding"
    uv run agents/specialists/quantum_operator.py --list-sessions
    uv run agents/specialists/quantum_operator.py --max-windings 50 "Compute the winding spectrum for H T S X circuit"
"""
...
from specialists import QUANTUM_SPECIALIST_PROMPT
from session_db import get_session_db


class QuantumOperator(TrueAgenticAgent):
    """Quantum Computation-focused ⊙perator — bridges IMASM grammar and quantum computation.
    Session-aware: auto-saves trajectory and messages."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._custom_system_prompt = (
            QUANTUM_SPECIALIST_PROMPT + _GRAMMAR_FIRST_RIDER + _PARTNERSHIP_RIDER
        )
        self._session_id: str | None = None

    async def run(self, task: str) -> str:
        """Run with the quantum-specialist system prompt injected."""
...
def main():
    ...
    p = argparse.ArgumentParser(
        prog="quantum_operator",
        description="Quantum Computation ⊙perator — Fibonacci anyon QC, SIC-POVM, IMASM↔Quantum translation",
    )
    ...
    if args.list_sessions:
        sessions = QuantumOperator.list_sessions()
        if not sessions:
    ...
    if args.continue_:
        sessions = QuantumOperator.list_sessions(limit=1)
        if sessions:
            restored_sid = sessions[0]["id"]
            _meta, preloaded_msgs, winding_dicts = QuantumOperator.load_session(restored_sid)
    ...
    if args.interactive or not args.task:
        print("═" * 72)
        print("  QUANTUM COMPUTATION ⊙PERATOR — Interactive Mode (session-persistent)")
        print(f"  Model: {args.model}  |  Max windings: {args.max_windings}")
        print(f"  Session: {restored_sid[:24] + '…' if restored_sid else 'new'}")
        print("  Domains: Fibonacci anyon QC, SIC-POVM, Jones polynomials, Braid compilation,")
        print("           IMASM ring walks, Belnap ParaASM, mOMonadOS kernel, web research")
        print("  Enter task → blank line submits. 'quit' or Ctrl-D exits.")
        print("═" * 72)

        agent = QuantumOperator(
            model=args.model,
        ...
        try:
            ...
            print("\n[quantum operator session ended]")
        return
    ...
    if not args.no_save:
        sid = agent.save_session(args.task, tags=["quantum"])
        print(f"  Session saved: {sid}")

    if args.output:
        import json as _json
        payload = {
            "specialist": "quantum_operator",
