#!/usr/bin/env python3
"""
chembio_operator.py — Chemistry/Biology/Materials/Plasmas ⊙perator (red-hot_rebis + p4rakernel).

Launches a TrueAgenticAgent with the chem-bio-materials specialist system prompt.
...
Usage:
    uv run agents/specialists/chembio_operator.py "Design a synthesis protocol for the optimal phenethylamine salt"
    uv run agents/specialists/chembio_operator.py --model claude-opus-4 "Analyze the rebis furnace phase diagram"
    uv run agents/specialists/chembio_operator.py --continue "Optimize the yield from yesterday's calculation"
    uv run agents/specialists/chembio_operator.py --list-sessions
    uv run agents/specialists/chembio_operator.py --session-id 20250315T120000Z-abc12345 "Follow up on prior results"
"""
...
class ChemBioOperator(TrueAgenticAgent):
    """Chemistry/Biology/Materials/Plasmas-focused ⊙perator with session persistence."""
...
def main():
    ...
    p = argparse.ArgumentParser(
        prog="chembio_operator",
        description="Chemistry/Biology/Materials/Plasmas ⊙perator — red-hot_rebis + p4rakernel",
    )
    ...
    if args.list_sessions:
        sessions = ChemBioOperator.list_sessions()
        if not sessions:
    ...
    if args.continue_:
        sessions = ChemBioOperator.list_sessions(limit=1)
        if sessions:
            restored_sid = sessions[0]["id"]
            _meta, preloaded_msgs, winding_dicts = ChemBioOperator.load_session(restored_sid)
            preloaded_traj = winding_dicts  # TrueAgenticAgent accepts dict-based trajectory
    ...
    if args.interactive or not args.task:
        ...
        print("  Domains: chemistry synthesis, biology, materials, plasmas, rebis furnace")
        print("  Enter task → blank line submits. 'quit' or Ctrl-D exits.")
        print("═" * 72)

        agent = ChemBioOperator(
            model=args.model,
        ...
        try:
            while True:
                ...
                if not args.no_save:
                    sid = agent.save_session(
                        ...
                        tags=["chembio", "interactive"],
                    )
                    print(f"  Session: {sid}")

                # Reset preloaded state so next turn continues from THIS session
            ...
            print("\n[chembio operator session ended]")
        return
    ...
    # ── Auto-save ──
    if not args.no_save:
        sid = agent.save_session(args.task, tags=["chembio"])
