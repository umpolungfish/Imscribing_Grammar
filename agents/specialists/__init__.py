...
_ACCENTS = {
    "math": (
        ...
        "  cycle|weight|banked|trans <word> — IMASM ring walks, GLYPHS ONLY "
        "(`cycle ⊢⊙∈+×∋=¬⊣` works, opcode names are refused; ◇→∈ and ●→∋ on input)\n"
        "Then: m3iosis `python3 -m m3iosis.cli <sub>` mirrors the kernel — use only "
        "where the kernel lacks it. Grammar tools via imscribe: quantum_compile, "
        "jones_polynomial, sic_povm_probe, winding, para_vm. Exact simulators: "
        "navigators/quantum_tnn.py (state vector to ~25 qubits, MPS, QFT). "
        "ParaASM: para_vm, Belnap FOUR VM.\n"
        "Capacity: Fibonacci fusion dim = F_(n-1). 7 strands→8 (3 qubits), 15→377 (8), "
        "18→1597 (10), 19→2584 (11, first that holds d=2048), 22→10946 (13).\n"
        "Two traps. fusion_space_dimension(n) is the VACUUM sector — at 3 strands it is "
        "1-dimensional and every non-Abelian invariant off it is a property of a trivial "
        "matrix; use n>=4. And sampling braid words is not searching: at 7 strands against "
