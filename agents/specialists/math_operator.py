#!/usr/bin/env python3
"""
math_operator.py — Mathematics ⊙perator (MoDoT, m3iosis, grammar↔conventional translation).

Launches a TrueAgenticAgent with the mathematics-specialist system prompt.
...
Usage:
    uv run agents/specialists/math_operator.py "Derive the SIC-POVM functor in categorical terms"
    uv run agents/specialists/math_operator.py --model claude-opus-4 "Prove the Zauner embedding for d=12"
    uv run agents/specialists/math_operator.py --continue "Extend the derivation from last session"
    uv run agents/specialists/math_operator.py --session-id 20250315T120000Z-abc12345 "Verify the Lean proof"
    uv run agents/specialists/math_operator.py --list-sessions
    uv run agents/specialists/math_operator.py --max-windings 50 "Translate ZFC set theory to grammar tuple"
"""
...
class MathOperator(TrueAgenticAgent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._custom_system_prompt = (
            MATH_SPECIALIST_PROMPT + _PARTNERSHIP_RIDER
        )
        self._session_id: str | None = None

    async def run(self, task: str) -> str:
        """Run with the math-specialist system prompt injected."""
...
def main():
    ...
    p = argparse.ArgumentParser(
        prog="math_operator",
        description="Mathematics ⊙perator — MoDoT, m3iosis, grammar↔conventional translation",
    )
    ...
    p.add_argument("--model", default=_ig_model,
                   help="Model: grok-4, deepseek-r1, deepseek-chat, deepseek:deepseek-chat (direct API), claude-opus-4, etc. Env: IG_MODEL, IG_PROVIDER")
    ...
    if args.list_sessions:
        sessions = MathOperator.list_sessions()
        if not sessions:
    ...
    if args.continue_:
        sessions = MathOperator.list_sessions(limit=1)
        if sessions:
            restored_sid = sessions[0]["id"]
            _meta, preloaded_msgs, winding_dicts = MathOperator.load_session(restored_sid)
    ...
    if not args.task:
        print("═" * 72)
        print("  MATHEMATICS ⊙PERATOR — Interactive Mode (session-persistent)")
        print(f"  Model: {args.model}  |  Max windings: {args.max_windings}")
        print(f"  Session: {restored_sid[:24] + '…' if restored_sid else 'new'}")
        print("  Domains: MoDoT, m3iosis, SIC-POVM, Lean 4, grammar↔conventional")
    ...
    if not args.no_save:
        sid = agent.save_session(args.task, tags=["math"])
        print(f"  Session saved: {sid}")

    if args.output:
        import json as _json
        payload = {
            "specialist": "math_operator",
