#!/usr/bin/env python3
"""
momonados_operator.py — mOMonadOS ⊙perator (the kernel itself: every command it exposes, present and future).

Launches a TrueAgenticAgent with the mOMonadOS-specialist system prompt. The
specialist does not carry a command list: it reads the live surface from
src/menu.rs and src/repl.rs and confirms it by booting, because a list written
into a prompt is stale the first time a command is added.
...
Usage:
    uv run agents/specialists/momonados_operator.py "What does the kernel expose for fixed-point work?"
    uv run agents/specialists/momonados_operator.py "Which REPL commands are unreachable from the menu, and why each?"
    uv run agents/specialists/momonados_operator.py "Wire a command that reports the crystal tier of a named entry"
    uv run agents/specialists/momonados_operator.py --continue "Now check it against the Lean ordinal table"
    uv run agents/specialists/momonados_operator.py --list-sessions
"""
...
from specialists import MOMONADOS_SPECIALIST_PROMPT
from session_db import get_session_db


class MomonadosOperator(TrueAgenticAgent):
    """mOMonadOS-focused ⊙perator — expert in the kernel's command surface.
    Session-aware: auto-saves trajectory and messages."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._custom_system_prompt = (
            MOMONADOS_SPECIALIST_PROMPT + _GRAMMAR_FIRST_RIDER + _PARTNERSHIP_RIDER
        )
        self._session_id: str | None = None

    async def run(self, task: str) -> str:
        """Run with the mOMonadOS-specialist system prompt injected."""
...
def main():
    ...
    p = argparse.ArgumentParser(
        prog="momonados_operator",
        description="mOMonadOS operator — the kernel command surface, read live and confirmed by booting",
    )
    ...
    if args.list_sessions:
        sessions = MomonadosOperator.list_sessions()
        if not sessions:
    ...
    if args.continue_:
        sessions = MomonadosOperator.list_sessions(limit=1)
        if sessions:
            restored_sid = sessions[0]["id"]
            _meta, preloaded_msgs, winding_dicts = MomonadosOperator.load_session(restored_sid)
    ...
    if args.interactive or not args.task:
        print("═" * 72)
        print("  mOMonadOS ⊙PERATOR — Interactive Mode (session-persistent)")
        print(f"  Model: {args.model}  |  Max windings: {args.max_windings}")
        print(f"  Session: {restored_sid[:24] + '…' if restored_sid else 'new'}")
        print("  Surface: read live from src/menu.rs and src/repl.rs, confirmed by booting.")
        print("           Batch commands — the QEMU boot dominates. No timeout, no caps.")
        print("           Rebuild with `make image` before running after any source change.")
        print("  Enter task → blank line submits. 'quit' or Ctrl-D exits.")
        print("═" * 72)

        agent = MomonadosOperator(
            model=args.model,
        ...
        try:
            ...
            print("\n[mOMonadOS operator session ended]")
        return
    ...
    if not args.no_save:
        sid = agent.save_session(args.task, tags=["momonados"])
        print(f"  Session saved: {sid}")

    if args.output:
        import json as _json
        payload = {
            "specialist": "momonados_operator",
